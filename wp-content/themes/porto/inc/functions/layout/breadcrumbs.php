...
function porto_breadcrumbs() {
	...
	/* if ( class_exists( 'WooCommerce' ) && ( ! isset( $porto_settings['breadcrumbs-shop-link'] ) || $porto_settings['breadcrumbs-shop-link'] ) && ( ( is_woocommerce() && is_archive() && ! is_shop() ) || is_cart() || is_checkout() || is_account_page() ) ) {
		$output .= porto_breadcrumbs_shop_link();
	} */
	
	if ( class_exists( 'WooCommerce' ) && ( ! isset( $porto_settings['breadcrumbs-shop-link'] ) || $porto_settings['breadcrumbs-shop-link'] ) && is_product() ) {
		$getProductBrandName = $product->get_attribute( 'pa_brand' );
		
		$output .= '<li><a href="'.site_url() .'/brand/?filter_brand=' . strtolower($getProductBrandName). '">' . strtoupper($getProductBrandName) . '</a><i class="delimiter delimiter-2"></i></li>';
	}
		...
		if ( is_post_type_archive() ) {
			...
			if ( is_tag() ) {
				...
				if(isset($_GET["from"]) && $_GET["from"]=="landing"){
					$output .='<li><a href="/presale"><span >PRESALE COLLECTIONS</span></a> <i class="delimiter delimiter-2"></i></li>';
				}
				$output .= $html;
			}
		} elseif ( is_date() ) {
...
function porto_breadcrumbs_terms_link() {
	...
	{
		$getProductBrandName = $product->get_attribute( 'pa_brand' );
		$a = "?filter_brand=".strtolower($getProductBrandName);
	}
	else
	{
		$a = '';
	...
	if ( count( $terms ) == 1 ) {
		...
		if ( $term_parent ) {
			...
			foreach ( $term_tree as $term_id ) {
				...
				if ( 0 == $i++ ) {
					...
					$output .= porto_breadcrumbs_simple_link( $term_object->name, get_term_link( $term_object ) . $a);
				} else {
					$output .= ', ' . porto_breadcrumbs_simple_link( $term_object->name, get_term_link( $term_object ) . $a);
				}
			}
		...
		$output = porto_breadcrumbs_simple_link( $terms[0]->name, get_term_link( $terms[0] ) . $a);
		array_shift( $terms );
		foreach ( $terms as $term ) {
			$output .= ', ' . porto_breadcrumbs_simple_link( $term->name, get_term_link( $term ) . $a);
		}
		$output = porto_breadcrumbs_links( $output );
	}
