...
$splitArr = array_chunk($allProducts, 50);
    ...
    foreach($sv as $value)
    {
    	...
    	if( has_term( 'summer-spring-22' , 'product_cat' ,  $parentID) ){
    		...
    		else
    		{
    			...
    			$result->setAttribute('Producto', $parentSKU . $colorName);
    ...
    if($ak != ""){
		...
		$root->appendChild($result);
			...
			if($dom->save('IMG_' . $ak . '.xml'))
			{
				...
				echo "FEXpro Style Images XML is created please click <a href='". site_url() ."/wp-content/themes/porto-child/ss22_images/mexico/IMG_" . $ak . ".xml' target='_blank'>Click Here " . $ak ."</a> <br><hr>"; 
			}
			else
			{
