...
foreach($allProducts as $value)
{
	...
	else
	{
		...
		echo $thumb_image[0] . "<br>";
		$ak =  $thumb_image[0];
		$tokens2 = explode('/', $ak);
		$str2 = trim(end($tokens2));
		...
		if ( ! empty( $gallery ))
		{
			foreach($gallery as $gvalue)
			{
				...
				$abc = strstr($thumb_image1[0], 'https://shop.fexpro.com/');
				...
				echo "File downloaded! " . $str1 . "<br>";
			}
		}
		
		$fp1 = $str2;
		$data1 = file_get_contents_curl($ak);
		file_put_contents( $fp1, $data1 );
	}
