...
if (!empty($term_id)) {
    ...
    $get_total_records = $wpdb->get_results("SELECT v_id as ID, p_id FROM wp_ss22_bulk_filter_brand_custom_filter WHERE brand_id IN ($term_id) ");
...
if (!empty($_GET['attr_pa_team'])) {
    ...
    foreach ($explodeTeamArr as $team_key => $team_value) {
        ...
        $output .= '<div class="wcpt-clear-filter" data-wcpt-filter="attribute" data-wcpt-taxonomy="pa_team" data-wcpt-meta-key="" data-wcpt-value="' . $team_value . '"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>';
        $output .= '<span class="wcpt-filter-label">Team : ' . $team_value . '</span>';
        $output .= '</div>';
    }
    $output .= '</div>';
}
