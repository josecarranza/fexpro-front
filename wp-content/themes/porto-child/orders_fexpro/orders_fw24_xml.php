...
$base_path="/var/www/vhosts/fexpro.com/shop.fexpro.com/";
include($base_path.'wp-load.php');
global $wpdb;
...
foreach($orders as $order_id)
{
	...
	else
	{
		...
		$result->setAttribute('Nombre', $company);
		//$result->setAttribute('Company',$company);
		$result->setAttribute('Fecha', $orderCreateDate);
		$result->setAttribute('Tipo', "1");
		$result->setAttribute('Fecha_Entrega', "15/05/2024");
		...
		$result->setAttribute('ResponsablePedido', $userFirstLastname);
		...
		/* foreach($getMarca as $ak)
		{
			...
			if($dom->save('wc-fw24-q3/PED_' . $order_id . '_243.xml'))
			{
			   echo "FW24-Q3 Panama Order XML is created please click <a href='". site_url() ."/wp-content/themes/porto-child/orders_fexpro/wc-fw24-q3/PED_" . $order_id . "_243.xml' target='_blank'>Click Here " . $order_id . " </a> <br>"; 
			}
			else
			{
