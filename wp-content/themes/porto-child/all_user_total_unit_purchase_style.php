   ...
   <thead>
	...
	<tbody id="myTable">
		<?php 
			...
			foreach($merge as $key => $value)
			{
				$getOrderNumber = $wpdb->get_row( "SELECT `forderid` FROM {$wpdb->prefix}factory_order_confirmation_list WHERE vid = $key" );
				$_product =  wc_get_product( $key); 
				$main_product = wc_get_product( $_product->get_parent_id() );
				...
				echo "<tr>";
					echo "<td class='".$_product->get_sku()."'>" . $getOrderNumber->forderid . "</td>";
...
jQuery(document).ready(function() {
	...
	setTimeout(function() {
		...
		jQuery("table#demo tbody tr> td[data-customer_id*=' ']").each(function() {
			var getCustomer = jQuery(this).data("customer_id");
			jQuery(this).data("customer_id").each(function() {
				c += Number($(this).text());
			});
			
			console.log(getCustomer + " : " + c);
		});
	},500); */
});
