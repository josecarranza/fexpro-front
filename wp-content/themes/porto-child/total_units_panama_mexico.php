...
foreach($orders as $order_id)
{
	...
	$getCountry = $order->get_billing_country();
	...
	$a = array();
	   ...
	   $variation_id = $item->get_variation_id();
	    if(!empty($product_id) && !empty($variation_id))
	    {
			...
			if($getCountry == 'MX')
			{
				$final_result1[$variation_id][$getCountry][] = $item_id;			
			}
			else
			{
				$final_result1[$variation_id]['PA'][] = $item_id;			
			}
		...
		foreach($value5 as $key4 => $abc)
		{
			...
			$c1 = 0;
				...
				foreach ($variation_size as $key => $size) 
				{
					...
					$merge5[$key3][$key5][] = $ap * $size['value'];
				}
...
<h2>Panama/Mexico Purchase List</h2>
...
<table class="table table-bordered" id="demo">
   <thead>
        <tr>
          ...
          <th  style="vertical-align : middle;text-align:center;">Panama Qty</th>
          <th  style="vertical-align : middle;text-align:center;">Mexico Qty</th>
	...
	<tbody id="myTable">
		...
		foreach($merge as $key => $value)
		{
			...
			$row3 .= "</div>";
			$row3 .= "</div>";
			if(empty($merge5[$key]['PA']))
			{
				$pa = '';
			}
			else
			{
				$pa = array_sum($merge5[$key]['PA']);
			}
			
			if(empty($merge5[$key]['MX']))
			{
				$mx = '';
			}
			else
			{
				$mx = array_sum($merge5[$key]['MX']);
			}
			echo "<tr>";
				...
				echo "<td class='".$_product->get_sku()."'>" . $pa . "</td>";
				echo "<td class='".$_product->get_sku()."'>" . $mx . "</td>";
