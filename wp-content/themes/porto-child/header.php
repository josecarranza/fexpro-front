...
<head>
	...
	<script async src="https://www.googletagmanager.com/gtag/js?id=UA-174545889-1"></script>
	<!-- <script>
	...
	<!-- <script async src="https://www.googletagmanager.com/gtag/js?id=UA-200032749-1"></script>
	<script>
		window.dataLayer = window.dataLayer || [];
...
<body custom="1" <?php body_class(array($body_class)); ?><?php echo !$showing_overlay ? '' : ' data-loading-overlay'; ?>>
	...
	<div class="page-wrapper<?php echo !$header_is_side ? '' : ' side-nav', isset($porto_settings['header-side-position']) && $porto_settings['header-side-position'] ? ' side-nav-right' : ''; ?>">
		...
		if ('before_header' == $porto_banner_pos) {
			custom_porto_banner('banner-before-header');
		}
		...
		<?php if (porto_get_meta_value('header', true) && 'hide' != $porto_settings['header-view']) : ?>
			...
			<div class="<?php echo esc_attr($header_wrapper_class_escaped); ?>">
				<?php if (porto_get_wrapper_type() != 'boxed' && 'boxed' == $porto_settings['header-wrapper']) : ?>
					...
					<?php
					$ids_fifa = [265304];
					$isFifa = in_array(get_the_ID(),$ids_fifa);
					if(!$isFifa){	
						get_template_part('header/header_8');
					}else{
						get_template_part('header/header_fifa');
					}
					?>
		...
		<?php if ('side' == porto_get_header_type()) : ?>
			...
			if ('before_header' != $porto_banner_pos) {
				custom_porto_banner(('fixed' == $porto_banner_pos && 'boxed' !== $wrapper) ? 'banner-fixed' : '');
			}
			...
			<div id="main" class="<?php echo esc_attr(implode(' ', $main_class)); ?>">
				<!-- main -->
