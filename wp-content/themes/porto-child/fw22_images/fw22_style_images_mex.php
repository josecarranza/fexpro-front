...
foreach($splitArr as $sk => $sv){
	...
	if(!file_exists($self_path."/mex/images/".$zip_folder_name)) {
		mkdir($self_path."/mex/images/".$zip_folder_name, 0777);
	}
    ...
    foreach($sv as $value)
    {
    	...
    	if( has_term( 'PRESALE' , 'product_cat' ,  $parentID) ){
    		...
    		$ak = str_replace("-","",str_replace(' ', '-', strtolower($getSKU)));
    		...
    		else
    		{
    			...
    			if($str != 'default.png')
    			{
					...
					@copy($server_path,$self_path."/mex/images/".$zip_folder_name."/".$str );
    			}
    			...
    			if ( ! empty( $gallery ))
    			{
    				foreach($gallery as $gvalue)
    				{
    					...
    					$str1 =  str_replace(' ', '-',trim(end($tokens1)));
						...
						@copy($server_path1,$self_path."/mex/images/".$zip_folder_name."/".$str1 );
    				}
    			}
		...
		$root->appendChild($result);
		if($dom->save('mex/IMG_' . $ak . '.xml'))
		{
			$zip_file = 'IMG_' . $ak.".zip";
			$command = "cd ".$self_path."/mex/images/".$zip_folder_name." && zip -r  ../".$zip_file." *";
			shell_exec($command);
			shell_exec("cd ".$self_path."/mex/images/ && rm -rf ".$zip_folder_name);
			echo "FEXpro Style Images XML is created please click <a href='". site_url() ."/wp-content/themes/porto-child/fw22_images/mex/IMG_" . $ak . ".xml' target='_blank'>Click Here " . $ak ."</a> | <a href='". site_url() ."/wp-content/themes/porto-child/fw22_images/mex/images/" . $zip_file . "'>".$zip_file."</a><br><hr>"; 
		}
		else
		{
