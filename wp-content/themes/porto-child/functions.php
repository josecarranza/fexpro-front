...
function porto_child_css() {
   ...
   wp_enqueue_style( 'styles-fifa', esc_url( get_stylesheet_directory_uri() ) . '/assets/css/style-fifa.css', false, $theme_version . $random_number ); 
   ...
   wp_enqueue_script( 'template-new-design' , esc_url( get_stylesheet_directory_uri() ). "/assets/js/template-new-design.js?v=1.5" );
...
'id' => 'supplier_code[' . $loop . ']',

'class' => 'supplier_code',

'label' => __( 'SUPPLIER CODE: ', 'woocommerce' ),

'value' => get_post_meta( $variation->ID, 'supplier_code', true )
...
woocommerce_wp_text_input( array(
	...
	'label' => __( '<b>PI#:</b><br/>', 'woocommerce' ),
	
	'value' => get_post_meta( $variation->ID, 'pi_numeral', true )
	
	) ); 

woocommerce_wp_text_input( array(

	'id' => 'sourcing_office[' . $loop . ']',
	
	'class' => 'sourcing_office',
	
	'label' => __( '<b>Sourcing Office:</b><br/>', 'woocommerce' ),
	
	'value' => get_post_meta( $variation->ID, 'sourcing_office', true )
...
woocommerce_wp_text_input( array(
	...
	'label' => __( 'SAT', 'woocommerce' ),
	
	'value' => get_post_meta( $variation->ID, 'codigo_sat', true )
...
woocommerce_wp_text_input( array(
	...
	'label' => __( 'Bodega', 'woocommerce' ),
	
	'value' => get_post_meta( $variation->ID, 'warehouse', true )
	
	) );

   echo '
      <div class="meta-medidas">
         <table>
            <tr>
               <td>
                  <p class="form-field">
                     <label>Medida 1</label>
                     <input name="medidas_1['.$loop.']" value="'.get_post_meta( $variation->ID, 'medidas_1', true ).'" />
                  </p>
               </td>
               <td>
                  <p class="form-field">
                     <label>Medida 2</label>
                     <input name="medidas_2['.$loop.']" value="'.get_post_meta( $variation->ID, 'medidas_2', true ).'" />
                  </p>
               </td>
               <td>
                  <p class="form-field">
                     <label>Medida 3</label>
                     <input name="medidas_3['.$loop.']" value="'.get_post_meta( $variation->ID, 'medidas_3', true ).'" />
                  </p>
               </td>
            </tr>
            <tr>
               <td>
                  <p class="form-field">
                     ...
                     <input name="medidas_4['.$loop.']" value="'.get_post_meta( $variation->ID, 'medidas_4', true ).'" />
                  </p>
               </td>
               <td>
                  <p class="form-field">
                     <label>Medida 5</label>
                     <input name="medidas_5['.$loop.']" value="'.get_post_meta( $variation->ID, 'medidas_5', true ).'" />
                  </p>
               </td>
               <td>
                  <p class="form-field">
                     <label>Medida 6</label>
                     <input name="medidas_6['.$loop.']" value="'.get_post_meta( $variation->ID, 'medidas_6', true ).'" />
                  </p>
               </td>
            </tr>
         </table>
   ...
   ';
	...
	woocommerce_wp_text_input(
		[
			'id' => 'ordenado_fab[' . $loop . ']',
			'class' => 'ordenado_fab',
			'label' => __( 'Ordenado fábrica:<br>', 'woocommerce' ),
			'placeholder' => '',
			'value' => get_post_meta( $variation->ID, 'ordenado_fab', true )
		]
	);
	...
	woocommerce_wp_text_input(
		[
			'id' => 'scale_drop[' . $loop . ']',
			'class' => 'scale_drop',
			'label' => __( 'Scale Drop<br>', 'woocommerce' ),
			'placeholder' => '',
			'value' => get_post_meta( $variation->ID, 'scale_drop', true )
		]
	);	
	echo '</div>';
	echo '<div style="width: 33%;">';
	woocommerce_wp_text_input(
		[
			'id' => 'scale_stock[' . $loop . ']',
			'class' => 'scale_stock',
			'label' => __( 'Scale Stock<br>', 'woocommerce' ),
			'placeholder' => '',
			'value' => get_post_meta( $variation->ID, 'scale_stock', true )
		]
	);	
	echo '</div>';
	echo '<div style="width: 33%;">';
	woocommerce_wp_text_input(
		[
			'id' => 'scale_ok[' . $loop . ']',
			'class' => 'scale_ok',
			'label' => __( 'Scale OK<br>', 'woocommerce' ),
			'placeholder' => '',
			'value' => get_post_meta( $variation->ID, 'scale_ok', true )
		]
	);	
	echo '</div>';
	echo '</div>';
	...
	woocommerce_wp_text_input(
		[
			'id' => 'cliente[' . $loop . ']',
			'class' => 'cliente',
			'label' => __( 'Cliente:<br>', 'woocommerce' ),
			'placeholder' => '',
			'value' => get_post_meta( $variation->ID, 'cliente', true )
		]
	);
...
function bbloomer_save_custom_field_variations( $variation_id, $i ) {
   ...
   $supplier_code = $_POST['supplier_code'][$i];

   if ( isset( $supplier_code ) ) update_post_meta( $variation_id, 'supplier_code', esc_attr( $supplier_code ) );

   $pi_numeral = $_POST['pi_numeral'][$i];

   if ( isset( $pi_numeral ) ) update_post_meta( $variation_id, 'pi_numeral', esc_attr( $pi_numeral ) );

   $sourcing_office = $_POST['sourcing_office'][$i];

   if ( isset( $sourcing_office ) ) update_post_meta( $variation_id, 'sourcing_office', esc_attr( $sourcing_office ) );
   ...
   $codigo_sat = $_POST['codigo_sat'][$i];

   if ( isset( $codigo_sat ) ) update_post_meta( $variation_id, 'codigo_sat', esc_attr( $codigo_sat ) );
   
   $warehouse = $_POST['warehouse'][$i];
   if ( isset( $warehouse ) ) update_post_meta( $variation_id, 'warehouse', esc_attr( $warehouse ) );
   
   $medidas_1 = $_POST['medidas_1'][$i];
   if ( isset( $medidas_1 ) ) update_post_meta( $variation_id, 'medidas_1', esc_attr( $medidas_1 ) );

   $medidas_2 = $_POST['medidas_2'][$i];
   if ( isset( $medidas_2 ) ) update_post_meta( $variation_id, 'medidas_2', esc_attr( $medidas_2 ) );

   $medidas_3 = $_POST['medidas_3'][$i];
   if ( isset( $medidas_3 ) ) update_post_meta( $variation_id, 'medidas_3', esc_attr( $medidas_3 ) );

   $medidas_4 = $_POST['medidas_4'][$i];
   if ( isset( $medidas_4 ) ) update_post_meta( $variation_id, 'medidas_4', esc_attr( $medidas_4 ) );

   $medidas_5 = $_POST['medidas_5'][$i];
   if ( isset( $medidas_5 ) ) update_post_meta( $variation_id, 'medidas_5', esc_attr( $medidas_5 ) );

   $medidas_6 = $_POST['medidas_6'][$i];
   if ( isset( $medidas_6 ) ) update_post_meta( $variation_id, 'medidas_6', esc_attr( $medidas_6 ) );
   ...
   if(isset($_POST["ordenado_fab"]) && isset($_POST["ordenado_fab"][$i])){
	$ordenado_fab=$_POST["ordenado_fab"][$i];
	update_post_meta( $variation_id, 'ordenado_fab', esc_attr( $ordenado_fab ) );
   }

   $scale_drop = $_POST['scale_drop'][$i];
   if ( isset( $scale_drop ) ) update_post_meta( $variation_id, 'scale_drop', esc_attr( $scale_drop ) );

   $scale_stock = $_POST['scale_stock'][$i];
   if ( isset( $scale_stock ) ) update_post_meta( $variation_id, 'scale_stock', esc_attr( $scale_stock ) );

   $scale_ok = $_POST['scale_ok'][$i];
   if ( isset( $scale_ok ) ) update_post_meta( $variation_id, 'scale_ok', esc_attr( $scale_ok ) );
   ...
   $cliente = $_POST['cliente'][$i];

   if ( isset( $cliente ) ) update_post_meta( $variation_id, 'cliente', esc_attr( $cliente ) );
...
function action_checkout_create_order_line_item_callback( $item, $cart_item_key, $cart_item, $order ) {
   ...
   }elseif(isset($cart_item["type_stock"]) && $cart_item["type_stock"]=="available_china"){

		$item->update_meta_data( 'type_stock', $cart_item['type_stock']);
		$stock_avalible_china=(int)get_post_meta($cart_item["variation_id"],"_stock_present_china",true);
		$new_stock_china = $stock_avalible_china-$cart_item["quantity"];
		update_post_meta( $cart_item["variation_id"], '_stock_present_china',$new_stock_china );
...
function action_woocommerce_before_order_itemmeta( $item_id, $item, $product ) { 
   ...
   $is_presale = wc_get_order_item_meta( $item_id, 'is_presale', true );
   if(!empty($is_presale) && $is_presale == "1"){
	echo '<span class="cart-tag" style="display: inline-block;padding: 2px 10px;background: #000;color: #fff;border-radius: 10px;font-size: 12px;vertical-align: bottom;margin-bottom: 2px; margin-right:10px;">Presale</span>';
	}
   if(!empty($type_stock) && $type_stock == "future"){
   ...
   if(!empty($type_stock) && $type_stock == "available_china"){
	echo '<span class="cart-tag" style="display: inline-block;padding: 2px 10px;background: #000;color: #fff;border-radius: 10px;font-size: 12px;vertical-align: bottom;margin-bottom: 2px; margin-right:10px;">Immediate Stock China</span>';
	}
	 
	if(empty($type_stock) && empty($is_presale)){
		echo '<span class="cart-tag" style="display: inline-block;padding: 2px 10px;background: #000;color: #fff;border-radius: 10px;font-size: 12px;vertical-align: bottom;margin-bottom: 2px; margin-right:10px;">Immediate Stock Panamá</span>';
		}
   ...
   echo "<div class='cart-sizes-attribute'>";
	...
	//   $row3.= '<pre style="display:none">'.json_encode($variation_size).'</pre>';
...
function xx__update_custom_roles() {
		...
		add_role( 'custom_role_kids', 'KIDS', array( 'read' => true, 'level_0' => true ) );
...
function add_order_again_status($array){
   $status = array(
	...
	$option_config = json_decode(get_option("wi_order_status_list"),true);
   	if(is_array($option_config) && count($option_config)>0){
		$status_config = array_column($option_config,"code");
		$array = array_merge($array,  $status_config);
	}
...
function bbloomer_add_edit_order_my_account_orders_actions( $actions, $order ) {
    ...
    if ( $order->has_status( 'pending' ) || $order->has_status( 'processing' )  || $order->has_status( 'presale3' ) || $order->has_status( 'presale4' )|| $order->has_status( 'presale5' )  || $order->has_status( 'presale6' )  || $order->has_status( 'presale7' ) || $order->has_status( 'ss24-q2' )  || $order->has_status( 'fw24-q3' )) {
...
add_filter( 'wc_order_is_editable', 'wc_make_processing_orders_editable', 10, 2 );
 ...
 function wc_make_processing_orders_editable( $is_editable, $order ) {
    ...
    if ( $order->get_status() == 'pending' || $order->get_status() == 'processing' || $order->get_status() == 'presale3'  || $order->get_status() == 'presale4'  || $order->get_status() == 'presale5' || $order->get_status() == 'presale6' || $order->get_status() == 'presale7' || $order->get_status() == 'ss24-q2' || $order->get_status() == 'fw24-q3') {
...
add_action( 'woocommerce_order_status_cancelled', 'action_function_notificacion_cancel_order', 10, 1 );
...
add_action( 'woocommerce_order_status_presale7', 'action_function_name_stock_update', 10, 1 );

add_action( 'woocommerce_order_status_ss24-q2', 'action_function_name_stock_update', 10, 1 );
...
function action_function_notificacion_cancel_order($order_id) {
    // Obtener el pedido
    $order = wc_get_order($order_id);
    if (!$order) {
        error_log('Error: Pedido #' . $order_id . ' no encontrado.');
        return;
    }

    // Obtener el sistema de correos de WooCommerce
    $mailer = WC()->mailer();
    $emails = $mailer->get_emails();

    // Verificar si la notificación de "Orden cancelada" existe
    if (isset($emails['WC_Email_Cancelled_Order'])) {
        $email = $emails['WC_Email_Cancelled_Order'];
        // No modificamos $email->recipient, usamos el configurado en los ajustes
        $email->trigger($order_id, $order);
        error_log('Notificación de Orden cancelada enviada para el pedido #' . $order_id . ' a: ' . $email->recipient);
    } else {
        error_log('Error: Notificación WC_Email_Cancelled_Order no encontrada.');
    }
}
...
add_filter('woocommerce_register_shop_order_post_statuses', 'woocommerceOrderPresale6OrderStatus',10,1);
 ...
 // Presale 7
/*
function woocommerceOrderPresale7OrderStatus($order_statuses){

	$order_statuses['wc-presale7'] = array(

	   'label'                     => _x( 'Presale7', 'Order status', 'woocommerce' ),
	   ...
	   'label_count'               => _n_noop( 'Presale7<span class="count">(%s)</span>', 'Presale7<span class="count">(%s)</span>', 'woocommerce' ),
...
add_filter('woocommerce_register_shop_order_post_statuses', 'woocommerceOrderPresale7OrderStatus',10,1);

 

 function show_custom_order_status_presale7( $order_statuses ) {

 

	$order_statuses['wc-presale7'] = _x( 'Presale7', 'Order status', 'woocommerce' );
 ...
 add_filter( 'wc_order_statuses', 'show_custom_order_status_presale7',10,1);

 

 

 function get_custom_order_status_bulk_presale7( $bulk_actions ) {

	$bulk_actions['mark_presale7'] = 'Change status to Presale7 Status';
 ...
 add_filter( 'bulk_actions-edit-shop_order', 'get_custom_order_status_bulk_presale7',10,1);
*/

 // SS24-Q2

/* function woocommerceOrderSS24Q2OrderStatus($order_statuses){

   $order_statuses['wc-ss24-q2'] = array(

      'label'                     => _x( 'SS24-Q2', 'Order status', 'woocommerce' ),
      ...
      'label_count'               => _n_noop( 'SS24-Q2<span class="count">(%s)</span>', 'SS24-Q2<span class="count">(%s)</span>', 'woocommerce' ),
...
add_filter('woocommerce_register_shop_order_post_statuses', 'woocommerceOrderSS24Q2OrderStatus',10,1);

 

 function show_custom_order_status_ss24q2( $order_statuses ) {

 

   $order_statuses['wc-ss24-q2'] = _x( 'SS24-Q2', 'Order status', 'woocommerce' );
 ...
 add_filter( 'wc_order_statuses', 'show_custom_order_status_ss24q2',10,1);

 

 

 function get_custom_order_status_bulk_ss24Q2( $bulk_actions ) {

   $bulk_actions['mark_ss24-q2'] = 'Change status to SS24-Q2 Status';
 ...
 add_filter( 'bulk_actions-edit-shop_order', 'get_custom_order_status_bulk_ss24Q2',10,1); */

 // Stock Order Status only (Inventory) 

function woocommerceOrderStockOrderStatus($order_statuses){

	$order_statuses['wc-stock'] = array(
 
	   'label'                     => _x( 'Stock', 'Order status', 'woocommerce' ),
	   ...
	   'label_count'               => _n_noop( 'Stock<span class="count">(%s)</span>', 'Stock<span class="count">(%s)</span>', 'woocommerce' ),
 ...
 add_filter('woocommerce_register_shop_order_post_statuses', 'woocommerceOrderStockOrderStatus',10,1);
 
 
 
 function show_custom_order_stock_status( $order_statuses ) {
 
 
 
	$order_statuses['wc-stock'] = _x( 'Stock', 'Order status', 'woocommerce' );
 ...
 add_filter( 'wc_order_statuses', 'show_custom_order_stock_status',10,1);
 
 
 
 
 
 function get_custom_order_stock_status_bulk( $bulk_actions ) {
 
	$bulk_actions['stock'] = 'Change status to Stock Status';
 ...
 add_filter( 'bulk_actions-edit-shop_order', 'get_custom_order_stock_status_bulk',10,1);
...

{
    ...
    foreach( $columns as $key => $column){
        ...
        if( $key ==  'order_total' ){
            ...
            $reordered_columns['export-sage-status'] = __( 'Export Sage Status','theme_domain');
        }
...

{
...

    {
        ...
        case 'my-column1' :
            ...
            echo '<a class="button button-primary customOrderExport" data-orderId="'.$post_id.'" style="margin-top:1em;" href="javascript:void(0);">Export XML</a>';

            break;
            case 'export-sage-status' :

            // Get custom post meta data

            $export_status = get_post_meta( $post_id, '_export_sage_status', true );

            echo $export_status==1?"Exported":"-";
            break;
     ...
     if($order->get_billing_country() == 'MX')

      {
         ...
         foreach ( $order->get_items() as $item_id => $item ) {
            ...
            $result->setAttribute('Fecha_Entrega', $fechaEntrega); //$orderCreateDate
...
add_action( 'wp', 'check_client_is_logged' );


function wi_mail($mail,$subject,$html,$from_email="no-reply@fexpro.com",$from="Shop Fexpro"){
	if(!defined('SMTP_HOST')){
		return false;
	}
	$config=array(
		'host'        => SMTP_HOST,//"smtp.gmail.com",
		'username'    => SMTP_USER,//"envios.webinfosv@gmail.com",
		'password'    => SMTP_PASS,//"bnpndjxginicvdbt",
		'port'        => SMTP_PORT,
		'smtp_secure' => "tls"
	);
	
 
	include("PHPMailerCI.php");
	$phpmailerci = new PHPMailerCI($config);
	 
	$phpmailerci->setFrom($from_email,$from);
	$phpmailerci->AddReplyTo($from_email,$from);
	

	/* if($attach!=null){
		$phpmailerci->addStringAttachment($attach[0], $attach[1])	;
	} */
	
	$message="";
	$r=$phpmailerci->send_mail($mail,$subject,$html,$message);
	return $r;
}
...
add_action( 'wp', 'debug_hook',9 );

function add_category_meta_fields($term=null,$taxonomy=null) {
      $category_type = "";
      if(isset($term->term_id)){
      ...
      <div class="form-field term-display-type-wrap">
         ...
         <select name="category_type" id="" class="postform">
            <option value="">-Select-</option>
            <option value="department" <?=$category_type == 'department' ? "selected" : ''?>>Department</option>
            <option value="apparel" <?=$category_type == 'apparel' ? "selected" : ''?>>Apparel</option>
         </select>
      </div>
      <?php
   }

   function save_category_meta_fields($term_id, $tt_id = '', $taxonomy = '') {
      if (isset($_POST['category_type'])) {
         update_term_meta($term_id, 'category_type', sanitize_title($_POST['category_type']));
      }
   }
add_action('product_cat_add_form_fields', 'add_category_meta_fields', 10, 2);
add_action('product_cat_edit_form_fields', 'add_category_meta_fields', 10, 2);
