...
foreach($splitArr as $sk => $sv){
    ...
    foreach($sv as $value)
    {
		...
		$root->appendChild($result);
		if($dom->save('barcode/IMG_' . $ak . '.xml'))
		{
			...
			echo "FEXpro Style Images XML is created please click <a href='". site_url() ."/wp-content/themes/porto-child/fw23_images/barcode/IMG_" . $ak . ".xml' target='_blank'>Click Here " . $ak ."</a> | <a href='". site_url() ."/wp-content/themes/porto-child/fw23_images/images/" . $zip_file . "'>".$zip_file."</a><br><hr>"; 
		}
		else
		{
