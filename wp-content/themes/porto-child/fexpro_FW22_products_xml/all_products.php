...
foreach ($allProducts as $key => $value) {
	...
	if (!has_term('ss23', 'product_cat', $parentID)) {
		continue;
	} else {
		...
		$description_spanish = isset($meta['composition'][0]) ? $meta['composition'][0] : '';
		$description_english = $customProduct->get_description();


		$Composicion = $customProduct->get_attribute('pa_fabric-composition');
		$Composicion_esp = $customProduct->get_attribute('pa_composicion');
		...
		$result->setAttribute('Group', $meta['grupo'][0]);
...
if ($dom->save('INV_all_fexpro_ss23_products.xml')) {
	echo "Fexpro SS23 Product XML is created please click <a href='" . site_url() . "/wp-content/themes/porto-child/fexpro_ss23_products_xml/INV_all_fexpro_ss23_products.xml' target='_blank'>Click Here All products </a> <br><hr>";
} else {
