...
foreach ($allProducts as $key => $value) {
	...
	if (!has_term('fall-winter-22', 'product_cat', $parentID)) {
		...
		$Composicion = $customProduct->get_attribute('Fabric-composition');
		$Composicion_esp = get_post_meta($parentID, 'composition', true);
		//$brand = strtoupper($customProduct->get_attribute( 'pa_brand' ));
		//echo $brand . "<br>";
		$brand = $customProduct->get_attribute('Brand');
		...
		$logo_Application_tmp = $customProduct->get_attribute( 'pa_logo-application' );
		if($logo_Application_tmp != ''){
			$logo_Application = $logo_Application_tmp;
		} else if (get_post_meta($variationID, 'logo_application', true) != '') {
		...
		$result->setAttribute('Reference', "$parentSKU");
...
if ($dom->save('INV_all_fexpro_fw22_products.xml')) {
	echo "FEXpro FW22 Product XML is created please click <a href='" . site_url() . "/wp-content/themes/porto-child/fexpro_FW22_products_xml/INV_all_fexpro_fw22_products.xml' target='_blank'>Click Here All products </a> <br><hr>";
} else {
