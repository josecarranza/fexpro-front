...
function import_order(){
    ...
    //ini_set('display_startup_errors', 1);
    //error_reporting(E_ALL);
    $inventory_origin = isset($_POST["inventory_origin"]) ? addslashes(trim($_POST["inventory_origin"])) : "presale"; 
    ...
    if(isset($_FILES["file"]) && $_FILES["file"]["error"]==0){
        ...
        if(count($excel)>0){
            ...
            foreach($excel as $row){
                ...
                $stock_present       = (int)$metas["_stock_present"][0] ?? 0;
                $stock_present_china = (int)$metas["_stock_present_china"][0] ?? 0;
                $stock_future = (int)$metas["_stock_future"][0] ?? 0;
                 
                if(isset($product->ID) && $qty > 0){
                    $data_extra=array();
                    switch ($inventory_origin) {
                        case 'presale':
                        ...
                        case 'available_future':
                            ...
                            break;
                        default:
                            if($qty<=$stock_present){
                                $woocommerce->cart->add_to_cart($product->post_parent,$qty,$product->ID,null,$data_extra);
                            }else{
                                if($qty<=$stock_present_china){
                                    $data["type_stock"] = "available_china";
                                    $woocommerce->cart->add_to_cart($product->post_parent,$qty,$product->ID,null,$data_extra);
                                }else{
                                    $no_added[]=$sku;
                                }
            ...
            if(count($no_added)>0){
                wc_add_notice( "These products have not been added to cart: ".implode(", ",$no_added) ,"error");
            }
            $json["error"]=0;
        }
    }else{
...
function ExcelGet($fileName,$sheet=-1,$index=true){
	...
	$excelObj = $excelReader->load($fileName);
    	...
    	PHPExcel_Calculation::getInstance($excelObj)->setCalculationCacheEnabled(false);
	...
	foreach($worksheetNames as $key => $sheetName){
		...
		$return[$key] = $excelObj->getActiveSheet()->toArray(null, false,true,$index);
	}
	if($sheet!=-1){
		return $return[$sheet];
	}
...
function get_ajax_products_stock() {
    ...
    $filter_warehouse = isset($_GET["f_warehouse"]) ? preg_replace("/[^a-zA-Z0-9\-,_\+]+/", "",trim($_GET["f_warehouse"])) : "";
    ...
    $_cat_in=explode(",",$slug_category);
    if($_cat_in[0]=="presale"){
        $_cat_in[]="stock-inmediato"; // for Core's
    }
    ...
    if($division !=""){
        ...
        WHERE t.slug IN ('".implode("','",$_cat_in)."') AND t3.slug REGEXP '".(str_replace(",","|",$division))."' ";
        //echo $sql;
    ...
    if($apparel !=""){
        ...
        WHERE t.slug IN ('".implode("','",$_cat_in)."')
    ...
    if($slug_category!="" && count($categories)>0){
        if($categories[0] && $categories[0]=="presale"){
            $categories[]="core";
        }
        $args['tax_query'][] = array(
            'taxonomy'        => 'product_cat',
    ...
    //var_dump($filter_apparel_accesories); exit;
    ...
    if ($date!=""  && $slug_category!="stock-inmediato") {
        $filter_date = explode(',', $date);
        $args['tax_query'][] = array(
            'taxonomy'        => 'pa_date',
            'field'           => 'slug',
    ...
    if($filter_warehouse!=""){
        $filter_variation["warehouse"] = $filter_warehouse;
    }
    if($slug_category=="stock-inmediato" && isset($_GET["meta_delivery_date"]) && $_GET["meta_delivery_date"]!="now" ){
        $filter_variation["only_stock_future"] = true;
    ...
    if($orderby!="default" && in_array($orderby,array("newest_asc","newest_desc"))){
        ...
        $extra_args["order"] = $orderby=="newest_asc"?"DESC":"ASC";
    }
  
    $extra_args["posts_per_page"] = $per_page;
    $extra_args["paged"] = $pag;
    $extra_args['orderbyselection'] = $orderby;
    ...
    if ($loop!=null && $loop->have_posts()) {
        foreach ($loop->posts as $key => $value) {
            ...
            $gender = isset($cats[0]) && isset($cats[0]["children"])  && isset($cats[0]["children"][0]) ? $cats[0]["children"][0]["name"]  : "";
            if($rol!="administrator"){
                $price_ = role_price_get_by_parent_id($value->ID,$rol);
                $price = count($price_)>0?$price_:$price;
            }
            ...
            $show_custom_image = isset($meta["show_custom_image"]) && isset($meta["show_custom_image"][0]) && $meta["show_custom_image"][0]=="1" ?1:0;
            $custom_image = null;
            if( $show_custom_image == '1'){
                $custom_image =  wp_get_attachment_image_src(get_post_thumbnail_id($value->ID), 'thumbnail');
            }
...
function filter_variations($loop,$filter_variation=array(),$extra_args=array()){
    if ($loop->have_posts()) {
        ...
        $only_stock_future = isset($filter_variation["only_stock_future"]) && $filter_variation["only_stock_future"] ? true:false;
        ...
        $sql="SELECT pp.ID, pp.post_title, pp.post_name, pp.post_date, pp.post_parent, pp.post_excerpt ".$extra_field." FROM wp_posts p INNER JOIN wp_posts pp ON p.post_parent=pp.ID";

        // apparel column
/*	
        $sql.=" INNER JOIN wp_term_relationships tr_apparel ON pp.ID = tr_apparel.object_id 
                inner join wp_terms t_apparel on tr_apparel.term_taxonomy_id =t_apparel.term_id 
                inner join wp_termmeta tm_apparel on t_apparel.term_id=tm_apparel.term_id AND tm_apparel.meta_key='category_type' AND tm_apparel.meta_value='apparel'
            ";
 */
	$sql.=" LEFT JOIN (
		wp_term_relationships tr_apparel 
                inner join wp_terms t_apparel on tr_apparel.term_taxonomy_id =t_apparel.term_id 
		inner join wp_termmeta tm_apparel on t_apparel.term_id=tm_apparel.term_id 
		AND tm_apparel.meta_key='category_type' AND tm_apparel.meta_value='apparel'
		) ON pp.ID = tr_apparel.object_id 
            ";
        ...
        if(isset($filter_variation["warehouse"]) && $filter_variation["warehouse"]!=""):
            $meta_key = $filter_variation["warehouse"]=="china"?"_stock_present_china" : "_stock_present";
            $sql.=" INNER JOIN wp_postmeta pm_wh ON p.ID=pm_wh.post_id  AND (pm_wh.meta_key='".$meta_key."' AND CONVERT(pm_wh.meta_value,int) > 0 ) ";
        endif;
        if($only_stock_future){
            $sql.=" INNER JOIN wp_postmeta pm_sf ON p.ID=pm_sf.post_id  AND (pm_sf.meta_key='_stock_future' AND CONVERT(pm_sf.meta_value,int) > 0 ) ";
        }
        ...
        if(isset($extra_args['orderbyselection']) && $extra_args['orderbyselection'] == 'default'){
            $sql.=" ORDER BY FIELD(t_apparel.name, 'hoodie','jersey') DESC, t_apparel.name ASC ";
        }        
...
function category_search($level,$cat_id, $_c=""){
    ...
    if($is_presale){
        $mains_cats=array(6372,7884,9939,11155);
    }
...
function report_purchase(){
    ...
    //6497=core,5931=presale,3259=stock inmediato(inventory)
    //$current_presale = get_option("current_presale_status");
    $current_presale = get_order_status_default();
    $presale_status  = "wc-".(isset($filtros->presale_status) && $filtros->presale_status!="" ? $filtros->presale_status : $current_presale);
    ...
    ( SELECT group_concat(concat(tt.taxonomy,'||',t.name,if(tt.taxonomy='product_cat',CONCAT(':',tt.parent,':',tt.term_id),''),if(tt.taxonomy='pa_player',CONCAT(':',t.slug),'')) SEPARATOR '///')
        ...
        if(     
           (isset($filtros->filter_suggestion) && count($filtros->filter_suggestion)>0) 
        || (isset($filtros->filter_factory_status) && count($filtros->filter_factory_status)>0)
        ){
            $sql.=", 
            pm4.meta_value _drop,
            pm5.meta_value ordenado_fab,
            (if(pm4.meta_value=1,'drop',if(pm5.meta_value>0,'ordenado fabrica','pendiente'))) status_fab,
            pm6.meta_value units_per_pack,
            pm7.meta_value scale_drop,
            pm8.meta_value scale_stock,
            pm9.meta_value scale_ok,
            (pm6.meta_value*qty) total_units,
            (if((pm6.meta_value*qty)<=pm7.meta_value,'DROP',if((pm6.meta_value*qty)>pm7.meta_value AND (pm6.meta_value*qty)<pm9.meta_value,'STOCK',if((pm6.meta_value*qty)>=pm9.meta_value,'OK','')))) suggestion
            ";
        }
    ...
    $sql.=" FROM (
        ...
        WHERE pp.post_type='shop_order' AND pp.post_status='{$presale_status}' AND pp.ID >=126125
        )
        group by
        p.order_item_id
        ) AS ttmp
        ...
        if(isset($filtros->filter_sku) && count($filtros->filter_sku)>0){
            $_sku = $filtros->filter_sku[0];
            $sql.=" INNER JOIN wp_postmeta tr10 ON tr10.post_id=ttmp.variationID AND tr10.meta_key='_sku' AND tr10.meta_value like '{$_sku}%'";
        }
        if(isset($filtros->filter_global_brand) && count($filtros->filter_global_brand)>0){
            $ids_global_brand = term_id_global_brand($filtros->filter_global_brand);
            $sql.=" INNER JOIN wp_term_relationships tr11 ON tr11.object_id=ttmp.productID AND tr11.term_taxonomy_id IN (".implode(",",$ids_global_brand).") ";
        }
    ...
    if(     
     ...
     || (isset($filtros->filter_factory_status) && count($filtros->filter_factory_status)>0)
        ...
        $sql.="
        LEFT JOIN wp_postmeta pm4 ON (pm4.post_id=tmp.variationID AND pm4.meta_key='drop')
        LEFT JOIN wp_postmeta pm5 ON (pm5.post_id=tmp.variationID AND pm5.meta_key='ordenado_fab')
        LEFT JOIN wp_postmeta pm6 ON (pm6.post_id=tmp.variationID AND pm6.meta_key='units_per_pack')
        LEFT JOIN wp_postmeta pm7 ON (pm7.post_id=tmp.variationID AND pm7.meta_key='scale_drop')
        LEFT JOIN wp_postmeta pm8 ON (pm8.post_id=tmp.variationID AND pm8.meta_key='scale_stock')
        LEFT JOIN wp_postmeta pm9 ON (pm9.post_id=tmp.variationID AND pm9.meta_key='scale_ok')
        ";
     }
    if($sort_col!=""){
        ...
        if($sort_col=="cat"){
            $sql.=" LEFT JOIN (wp_term_relationships tr1  INNER JOIN wp_term_taxonomy tt1 ON tr1.term_taxonomy_id=tt1.term_taxonomy_id AND tt1.taxonomy='product_cat' AND tt1.term_id IN (6497,5931,3259) INNER JOIN wp_terms trm1 ON trm1.term_id= tt1.term_id) ON tr1.object_id=tmp.productID ";
        }
        if($sort_col=="lob"){
    ...
    if(     
     ...
     || (isset($filtros->filter_factory_status) && count($filtros->filter_factory_status)>0)
        ...
        $sql.=" HAVING 1=1 ";
        if(isset($filtros->filter_suggestion) && count($filtros->filter_suggestion)>0){
            $sql.=" AND suggestion IN ('".implode("','",$filtros->filter_suggestion)."') ";
        }
        if(isset($filtros->filter_factory_status) && count($filtros->filter_factory_status)>0){
            $sql.=" AND status_fab IN ('".implode("','",$filtros->filter_factory_status)."') ";
        }
     }
    ...
    foreach($r as $var_data){
        ...
        for($i=1;$i<=10;$i++){
            if(isset($metas_variation["custom_field".$i]) && $metas_variation["custom_field".$i]!=""){
                ...
                if(isset($metas_variation["medidas_".$i]) &&  $metas_variation["medidas_".$i]!=""){
                     $medidas[] = $metas_variation["medidas_".$i];
                }
        ...
        $item['group'] = isset($main_atts["pa_group"])? $main_atts["pa_group"] : $item['group'];
        ...
        if(!$new_columns){
            ...
            $price_mx = $wpdb->get_var(" SELECT price FROM wp_wusp_role_pricing_mapping p WHERE p.role = 'custom_role_mexico1' AND p.product_id= ".$var_data->variationID);

            $item["price_mx"] = !empty($price_mx)? $price_mx : "";
            $item["suggestion"]      = $suggestion;
            $item["moq"]             = ((int)$scale_ok)-1;
            $item["stock_panama"]    = isset($metas_variation["_stock_present"])?$metas_variation["_stock_present"]*$units_per_pack  : "0";
            $item["stock_china"]    = isset($metas_variation["_stock_china"])?$metas_variation["_stock_china"]*$units_per_pack  : "0";
            $item["stock_future"]    = isset($metas_variation["_stock_future"])?$metas_variation["_stock_future"]*$units_per_pack  : "0";
            ...
            $item["supplier_code"]        = isset($metas_variation["supplier_code"])?$metas_variation["supplier_code"]  : "";
            ...
            $item["open_units"] = $open_units;
            $item["percent_units"] = ($_ordenado_fab > 0 ? round(($open_units<0?$open_units*-1 : $open_units)/$_ordenado_fab * 100,2) :0)."%";
...
function unflattenArray($flatArray){
  ...
  function getGroup($string){
    $list=array("SPORTS","POP","UNIVERSITIES","GIRLS","WOMENS","MENS","KIDS","TEENS");
    $string=str_replace($list,"",$string);
    if($string==""){
        return "";
    }
    $string = strpos(strtolower($string),"apparel")>-1?"APPAREL":"ACCESORIES";
    return trim($string);
}
...
function report_purchase_users(){
    ...
    $sql.=" FROM (
        ...
        if(isset($filtros->filter_global_brand) && count($filtros->filter_global_brand)>0){
            $ids_global_brand = term_id_global_brand($filtros->filter_global_brand);
            $sql.=" INNER JOIN wp_term_relationships tr2 ON tr2.object_id=ttmp.productID AND tr2.term_taxonomy_id IN (".implode(",",$ids_global_brand).") ";
        }
        $sql.=" GROUP BY ttmp.order_item_id
        ) ttmp
        GROUP BY ttmp.variationID
    ...
    $sellers = get_all_sellers($group_by,$presale_status);
...
function get_all_sellers($group_by="user",$presale_status=""){
    global $wpdb;
    ...
    $country_order=array(
        ...
        "PA"=>'Panama',"PR"=>'Puerto Rico',"MF"=>'Saint Martin (Dutch part)',"SX"=>'Sint Maarten (Dutch part)',
        "CO"=>'Colombia',"VE"=>'Venezuela',"EC"=>'Ecuador',"PE"=>'Peru',"BO"=>'Bolivia',"BR"=>'Brasil',"CL"=>'Chile',"AR"=>'Argentina',"UY"=>'Uruguar',"PY"=>'Paraguay',
        "PH"=>'Filipinas'
    );
    ...
    WHERE pm.post_id IN (".(implode(",",$_orders_id)).") AND pm.meta_key ='_customer_user'
    GROUP BY ".$col_group. " ".
    "ORDER BY FIELD(um4.meta_value,'".(implode("','",array_keys($country_order)))."')";
    ...
    $final = array();
    foreach($r as $i=> $s){
        // $r["seller_".$i]["nombre"] = $s["customer_name"]." ".$s["customer_lastname"];
        // $r["seller_".$i]["country"] = isset($countries[$s["country"]])?$countries[$s["country"]]:"";
        // $r["seller_".$i]["label"] = ($group_by == "user" ? ($s["customer_name"]." ".$s["customer_lastname"]." - ") :"") .
        //                   ($group_by == "user" || $group_by=="company" ? ($s["company"]." - ") :"").
        //                    (isset($countries[$s["country"]])?$countries[$s["country"]]:"");
        // $r["seller_".$i]["detail"] = purchases_seller($s["orders"]);
        $final["seller_".$i] = array(
        // $final[$i] = array(
            'nombre' => $s["customer_name"]." ".$s["customer_lastname"],
            'country' => isset($countries[$s["country"]])?$countries[$s["country"]]:"",
            'country_code' =>$s['country'],
            'company'=> $s['company'],
            'user_id'=> $s['user_id'],
            'orders' => $s['orders'],
            'detail' => purchases_seller($s["orders"]),
            'label'  => ($group_by == "user" ? ($s["customer_name"]." ".$s["customer_lastname"]." - ") :"") .
                        ($group_by == "user" || $group_by=="company" ? ($s["company"]." - ") :"").
                        (isset($countries[$s["country"]])?$countries[$s["country"]]:"")
        );
    }
...
function filtros_reportes_get(){
    ...
    $sql="select ttmp.productID,group_concat(ttmp.variationID) variationID FROM (
        ...
        AND p.order_id IN (
        SELECT pp.ID FROM wp_posts pp
        WHERE pp.post_type='shop_order' AND pp.post_status='wc-presale7' AND pp.ID >=126125
        )
        group by
        p.order_item_id
        ) AS ttmp
    ...
    $json["filter_global_brand"] = filter_global_brand($products_id);
    ...
    $json["filter_suggestion"] = [["value"=>"DROP","text"=>"DROP"],["value"=>"STOCK","text"=>"STOCK"],["value"=>"OK","text"=>"OK"]];
    $json["filter_factory_status"] = [["value"=>"pendiente","text"=>"Pendiente"],["value"=>"ordenado fabrica","text"=>"Ordenado fabrica"],["value"=>"drop","text"=>"Drop"]];

    $json["filter_supplier"] = filter_supplier();
    
    

    $json["order_status"] = load_order_status_list();

    echo json_encode($json);
    die;

}
function load_order_status_list(){
...
function get_order_status_default(){
    $default = "presale7";
    $list = load_order_status_list();
    foreach($list  as $os){
...
function filter_category(){
    $cat=array( array("value"=>"presale","text"=>"Presale"),array("value"=>"core","text"=>"Core"),array("value"=>"stock-inmediato","text"=>"Stock"));
    return $cat;
}
function filter_lob(){
...
function filter_group_old($products_id){
    global $wpdb;
    $sql="SELECT * FROM (
        ...
        WHERE tt2.taxonomy='product_cat' and (name!='' and name!='Uncategorized' ) AND tt2.count>0 
            AND  tt2.term_id IN (
				...
				WHERE tt3.parent IN (9002,4104,8661,8657,6372,7884,9939,9493)
				)
...
function filter_group($products_id){
    ...
    $sql = "SELECT * from wp_term_taxonomy tt inner join wp_terms t on tt.term_id=t.term_id where tt.taxonomy= 'pa_group'";
    $r=$wpdb->get_results($sql);
    $groups=array();
    foreach ($r as $value) {
        $item=array();
...
function filter_product_type($products_id){
    ...
    INNER JOIN wp_term_relationships tr ON tt2.term_taxonomy_id=tr.term_taxonomy_id
    WHERE tt2.taxonomy='product_cat' and (name!='' and name!='Uncategorized' ) AND tt2.count>0 
        AND  tt2.parent IN (SELECT tt3.term_id FROM wp_term_taxonomy tt3 WHERE tt3.parent IN (9002,4104,8661,8657,6372,7884,9939,9493))
        AND tr.object_id IN (".implode(",",$products_id).")
    ) tmp
    GROUP BY tmp.name
    ORDER BY NAME ASC";
...
function term_id_cat($cats){
    ...
    if(in_array("core",$cats)){
        $id_cats[]=6497;
    };
    if(in_array("stock-inmediato",$cats)){
        $id_cats[]=3259;
    }
...
function term_id_lob($lobs){
    ...
    INNER JOIN wp_term_taxonomy tt2 ON tt1.term_id=tt2.term_id
    WHERE tt2.taxonomy='product_cat'
    AND  tt2.term_id IN (SELECT tt3.term_id FROM wp_term_taxonomy tt3 WHERE tt3.parent IN (9002,4104,8661,8657,6372,7884,9939,9493))
    AND (".$includes.")";
    // echo $sql;
    // die;
    $r=$wpdb->get_results($sql,ARRAY_A);
...
function term_id_global_brand($brands){
    global $wpdb;
    ...
    FROM wp_terms tt1
    INNER JOIN wp_term_taxonomy tt2 ON tt1.term_id=tt2.term_id
    WHERE tt2.taxonomy='pa_global-brand'
...
function term_id_group_old($groups){
    global $wpdb;
    //$types = explode(",",$lobs);
...
function term_id_group($groups){
    ...
    where tt.taxonomy = 'pa_group'
    and t.slug in ('".implode("','",$groups)."')";
...
function unflattenArray2($flatArray){
  ...
  function get_user_collections_old(){
...
add_action('wp_ajax_get_ajax_seasons_list', 'get_ajax_seasons_list');
add_action('wp_ajax_nopriv_get_ajax_seasons_list', 'get_ajax_seasons_list');

function get_ajax_seasons_list(){
    global $wpdb;
    $sql="SELECT a.term_id,b.name, b.slug FROM wp_term_taxonomy a
    INNER JOIN wp_terms b ON a.term_id=b.term_id
    WHERE a.taxonomy='pa_season' AND a.count>0
    ORDER BY b.name asc";
    $r=$wpdb->get_results($sql);
    $json["data"]=$r;
    $json["error"] = 0;
...
add_action('wp_ajax_season_products_report', 'season_products_report');
add_action('wp_ajax_nopriv_season_products_report', 'season_products_report');

function season_products_report(){
    set_time_limit(120);
    ...
    $season_term_id = (int)$filtros->season??0;
    if($season_term_id==0){
        $json["error"] = 1;
        $json["message"] = "Select a season";
        echo json_encode($json);
        exit;
    }
    $pag= $filtros->page??1;
    ...
    $sql="SELECT pv.ID,pv.post_title,
    pm2.meta_value image ,
    pm3.meta_value miniaturas,
    (SELECT group_concat(concat(b.meta_key,'||',b.meta_value) SEPARATOR '///') FROM wp_postmeta b WHERE b.post_id=pv.ID ) metas, 
    (SELECT group_concat(concat(tt.taxonomy,'||',t.name,if(tt.taxonomy='product_cat',CONCAT(':',tt.parent,':',tt.term_id),''),if(tt.taxonomy='pa_player',CONCAT(':',t.slug),'')) SEPARATOR '///') 
        FROM wp_term_relationships tr 
        INNER JOIN wp_term_taxonomy tt 
        ON tr.term_taxonomy_id=tt.term_taxonomy_id 
        INNER JOIN wp_terms t ON tt.term_id=t.term_id WHERE tr.object_id=p.ID ) attributes
    FROM wp_term_relationships tr
    INNER JOIN wp_posts p ON tr.object_id=p.ID
    INNER JOIN wp_posts pv ON pv.post_parent=p.ID
    LEFT JOIN wp_postmeta pm1 ON (pm1.post_id=pv.ID AND pm1.meta_value IS NOT NULL AND pm1.meta_key='_thumbnail_id') 
    LEFT JOIN wp_postmeta pm2 ON (pm2.post_id=pm1.meta_value AND pm2.meta_key='_wp_attached_file') 
    LEFT JOIN wp_postmeta pm3 ON (pm3.post_id=pm1.meta_value AND pm3.meta_key='_wp_attachment_metadata') 
    WHERE tr.term_taxonomy_id=".$season_term_id." 
    AND p.post_status IN ('publish','private')
    AND pv.post_type='product_variation' 
    AND pv.post_status IN ('publish')
    ";
    ...
    foreach($r as $var_data){
        ...
        $item['price'] =  $price;
        $item["sold"] = isset($metas_variation["sold"]) && $metas_variation["sold"]=="1"?"Yes":"";
    ...
    if(!$is_export){
        ...
        //build_excel_catalog_report($products);
        include("build-excel.php");
         $headers=["Selling Price","Sold"];
         export_phpspreadsheet($products,$headers,true);
    }
...
add_action('wp_ajax_save_meta_variation', 'save_meta_variation');
add_action('wp_ajax_nopriv_save_meta_variation', 'save_meta_variation');

function save_meta_variation(){
    $post = json_decode( file_get_contents('php://input') );
    $_metas=['fob'=>'fob','supplier'=>'supplier','pi'=>'pi_numeral','sourcing_office'=>'sourcing_office','ordenado_fab'=>'ordenado_fab'];

    $id_variation = (int)$post->id ?? 0;

    if(isset($post->fob)){
        $value = trim($post->fob?? '');
        update_post_meta($id_variation,'fob',$value);
    }
    if(isset($post->supplier)){
        $value = trim($post->supplier?? '');
        update_post_meta($id_variation,'supplier',$value);
    }
    if(isset($post->supplier_code)){
        $value = trim($post->supplier_code?? '');
        update_post_meta($id_variation,'supplier_code',$value);
    }
    if(isset($post->pi)){
        $value = trim($post->pi?? '');
        update_post_meta($id_variation,'pi_numeral',$value);
    }
    if(isset($post->sourcing_office)){
        $value = trim($post->sourcing_office?? '');
        update_post_meta($id_variation,'sourcing_office',$value);
    }
    if(isset($post->ordenado_fab)){
        $value = trim($post->ordenado_fab?? '');
        update_post_meta($id_variation,'ordenado_fab',$value);
    }
...
add_action('wp_ajax_test_email1', 'test_email1');
add_action('wp_ajax_nopriv_test_email1', 'test_email1');
function test_email1(){
   /*  ini_set('display_errors', 1);
    ini_set('display_startup_errors', 1);
    error_reporting(E_ALL);

    $html = "New order registered<br> <h4>ORDER: 1000</h4>";
	wi_mail("","New stock order registered",$html);
    $json['error'] = 0;
    $json['message'] = 'send';
    echo json_encode($json); */
