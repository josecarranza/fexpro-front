	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
		...
		if($taxonomy == 'pa_color')
		{
				...
				foreach ( $terms as $term ) 
				{
					...
					$words = strtolower(preg_replace('/[0-9]+/', '', $term->name));
					$final_result1 = $words;
					array_push($return_arr,$final_result1);
				}
				$getUniqueColors = array_unique($return_arr);
				//print_r($getUniqueColors);
								/* print_r($return_arr);
								echo "<br>";
								echo "<br>";
								//print_r(array_merge($final_result1));
				...
				foreach($getUniqueColors as $abc)
				{
					$return_arr2 = array();	
					foreach ( $terms as $keyterm => $term ) {

						if(strtolower(preg_replace('/[0-9]+/', '', $term->name)) == $abc)
						{
							
							if(!in_array($keyterm, $return_arr2, true)){
							array_push($return_arr2,$keyterm);
							}
							$row = array("$abc" => implode(",",$return_arr2));
						}
			...
			echo "</pre>"; */
			$ka = $return_arr1;
			foreach($ka as $akColor => $ak)
			{
			$av = [];

				foreach($ak  as $keyColor => $ap)
				{

					//print_r($terms[$ap]->slug);
					...
					foreach($kp as $qa)
					{
							...
							$current_values = isset( $_chosen_attributes[ $taxonomy ]['terms'] ) ? $_chosen_attributes[ $taxonomy ]['terms'] : array();
								...
								$option_is_set  = in_array( $terms[$qa]->slug, $current_values, true );
								$count          = isset( $term_counts[ $terms[$qa]->term_id ] ) ? $term_counts[ $terms[$qa]->term_id ] : 0;

								// Skip the term for the current archive.

								if ( $this->get_current_term_id() === $terms[$qa]->term_id ) {
									continue;
								}
								...
								if($_GET['filter_color']){
									$current_filter = array();
									$active_filter = $_GET['filter_color'];
								}
								if ( ! in_array( $terms[$qa]->slug, $current_filter, true ) ) {
									$current_filter[] = $terms[$qa]->slug;
									$av[] = $terms[$qa]->slug;
								}


								$link = remove_query_arg( $filter_name, $base_link );
								$link1 = remove_query_arg( $filter_name, $base_link );
								...
								foreach ( $current_filter as $key => $value ) {
									...
									if ( $option_is_set && $value === $terms[$qa]->slug ) {
										unset( $current_filter[ $key ] );
									}
								}
								...
								if ( ! empty( $current_filter ) ) {
									...
									if ( 'or' === $query_type && ! ( 1 === count( $current_filter ) && $option_is_set ) ) {
										$link = add_query_arg( 'query_type_' . wc_attribute_taxonomy_slug( $taxonomy ), 'or', $link1 );
									}
								...
								if ( $count > 0 || $option_is_set ) {
									...
									$term_html = '<a rel="nofollow" href="' . esc_url( $link ) . '">' . esc_html( $terms[$qa]->name ) . '</a>';
								} else {
									$link      = false;
									$term_html = '<span>' . esc_html( $terms[$qa]->name ) . '</span>';
								}
					...
					/* $av = implode(",", $option_is_set);
					echo $av; */

					//for Active call pre selected 
					$activeClass = '';
					if($_GET['filter_color']){
						if( strpos($_GET['filter_color'], $ac) !== false ) {
						     $activeClass = 'chosen';
						}
					}
					
					echo '<li class="woocommerce-widget-layered-nav-list__item wc-layered-nav-term '  . ( $option_is_set ? 'woocommerce-widget-layered-nav-list__item--chosen chosen ' : '' ) . (!empty($ac) ? ' have_elements ' : ' empty_counts' ) . $activeClass .' ">';


					//echo $link1;
					if (strpos($link1, '/?') !== false || strpos($link1, '?query_type') !== false) {
						echo '<a rel="nofollow" href="' . $link . '&filter_color='.$ac.'" data-filterColor="'.$ac.'" data-crrentUrl="'.$link.'">' . ucfirst($keyColor)  . '</a>';	
					}
					else
					{	
						echo '<a rel="nofollow" href="' . $link . '&filter_color='.$ac.'" data-filterColor="'.$ac.'" data-crrentUrl="'.$link.'">' . ucfirst($keyColor)  . '</a>';	
			...
			foreach ( $terms as $term ) {
				...
				// Custom Stock filter based on range
				if($filter_name == 'filter_stock'){
					if($getArrayElement){
						$maxValue = $this->getMax($getArrayElement);
						$minValue = $this->getMin($getArrayElement);
					}

					
					if($maxValue && $minValue){
						echo "<form id='submitCustomStockFilterForm'>";
						echo '<div class="overly-count" style="display:none;"><div id="custom-preloader"></div></div>';
						echo '<div class="customStockFilter">';
						echo '<div class="customStockFilterInput">';
						echo 'From :- <input type="text" size="5" name="minValue" id="minValue" data-minValue="'.$minValue.'" value="'.$minValue.'" /> '; 
						echo '</div>';
						echo '<div class="customStockFilterInput">';
						echo 'To :- <input type="text" size="5" name="maxValue" id="maxValue" data-maxValue="'.$maxValue.'" value="'.$maxValue.'" /> '; 
						echo '</div><BR>';
						echo '<button type="button" class="button" id="submitCustomStockFilter"> FIlter Stock</button>';
						echo '<p class="stockErrorMsg"></p>';
						echo '<div class="stock_label" style=""> Stock: <span class="from">'.$minValue.'</span> — <span class="to">'.$maxValue.'</span></div> </div>';
						echo '<input type="hidden" name="filterArr" value="'.implode(',',$getArrayElement).'"  id="filterArr">';
						echo "</form>";
					}
