...
$sql_variaciones="SELECT a.*,pp.post_excerpt post_excerpt_main, pm1.meta_value thumbnail_id, pm2.meta_value image , pm3.meta_value miniaturas,pm4.meta_value delivery_date,
...
$player =  is_array($player)?array_column($player,"name"):[];
...
foreach ($variaciones as $key => $var) {
    ...
    $product_player = isset($metas_variation["product_player"]) ? $metas_variation["product_player"]:"";
    ...
    $item['product_title'] = $var->post_excerpt_main!=""? $var->post_excerpt_main." - ".$color.", ".$product_team.($player!=""?$", ".$player:""): $var->post_title;// . " - " . $color ;
