...
foreach ( $items as $item_id => $item ) :
	...
	<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $order ) ); ?>">
		...
		echo "<div class='cart-sizes-attribute'>";
			...
			$row3 .= '<table class="make-table">';
			$row3 .= '<thead><tr>';
			foreach ($variation_size as $key => $size) {
				$row3 .= "<th'>" . $size['label']  . "</th>"; 
			}
			$row3 .= '</tr></thead>';
			$row3 .= '<tbody><tr>';
			foreach ($variation_size as $key => $size) {
				$row3 .= "<td>". $size['value'] * $item->get_quantity() . "</td>";
				$c += $size['value']; 
			}
			$row3 .= '</tr></tbody>';
			$row3 .= '</table>';	
			$row3 .= "<table class='make-table'><thead><tr><th>Total</th></tr></thead><tbody><tr><td>" . $c * $item->get_quantity()  . "</td></tr></tbody></table>";
			echo $row3;
		echo "</div>";
		echo "</div>";
