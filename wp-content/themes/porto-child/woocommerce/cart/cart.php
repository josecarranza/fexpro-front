...
foreach($items as $key => $item){
    $tmp=array();
    $tmp["key"] = $key;
    $tmp["type_stock"] =  isset($item["type_stock"]) ? ($item["type_stock"] == "future"?"future":"present_china"):"present";
	...
	$post_excerpt = $product->get_data()["attribute_summary"];
	$color = preg_match('/Color:\s*([^,]+)/',$post_excerpt, $coincidencias) && isset($coincidencias[1]) ?trim($coincidencias[1]):"";
	$team = preg_match('/Team:\s*([^,]+)/',$post_excerpt, $coincidencias) && isset($coincidencias[1]) ?trim($coincidencias[1]):"";

	$main_prod = wc_get_product($item["product_id"]);
	...
	$title2 = $main_prod->get_short_description()!=""?$main_prod->get_short_description()." - ".$color.", ".$team:$product->get_name();
	$tmp["title"] = $title2;//$product->get_name();
	...
	$tmp["stock_present_china"] = isset($metas["_stock_present_china"][0])?(int)$metas["_stock_present_china"][0]:0;
			...
			<div class="accordions">
				<div class="item-accordion show">
					...
					<div class="item-accordion-body">
						...
						<div ng-if="has_stock_present>0">
							<label for="" class="cart-group">Inventory <span>- Immediate | <b>From Panamá</b></span></label>
							<table class="cart-items">
								<tr ng-repeat="item in cart" ng-if="item.type_stock=='present'">
									<td>
										<a href="{{item.link_product}}">
										<div class="cart-item-image">
											<img ng-src="{{item.image}}" alt="">
									...
									<td>
										<util-number-stepper ng-model="item.qty" max="item.stock_present"></util-number-stepper>
									</td>
									<td class="col-total">
										{{item.total | currency:'$'}}
									</td>
						...
						<div ng-if="has_stock_present_china>0">
							<label for="" class="cart-group">Inventory <span>- Immediate | <b>From China</b></span></label>
							<table class="cart-items">
								<tr ng-repeat="item in cart" ng-if="item.type_stock=='present_china'">
									<td>
										<a href="{{item.link_product}}">
										<div class="cart-item-image">
											<img ng-src="{{item.image}}" alt="">
									...
									<td>
										<util-number-stepper ng-model="item.qty" max="item.stock_present_china"></util-number-stepper>
									</td>
									<td class="col-total">
										{{item.total | currency:'$'}}
									</td>
						...
						<div ng-if="has_stock_future>0">
							<label for="" class="cart-group">Inventory <span>- Future</span></label>
							<table class="cart-items">
								<tr ng-repeat="item in cart" ng-if="!item.is_presale && item.type_stock=='future'">
									<td>
										<a href="{{item.link_product}}">
										<div class="cart-item-image">
											<img ng-src="{{item.image}}" alt="">
									...
									<td>
										<util-number-stepper ng-model="item.qty" max="item.stock_future"></util-number-stepper>
									</td>
									<td class="col-total">
										{{item.total | currency:'$'}}
									</td>
...
<script>
	...
	.controller("ctrl",function($scope,$http,$timeout){
		...
		$scope.has_presale       = 0;
		$scope.has_stock_future  = 0;
		$scope.has_stock_present = 0;
		$scope.has_stock_present_china   = 0;
		...
		$scope.cart=cart.map((x)=>{
			...
			$scope.has_stock_future   += (!x.is_presale && x.type_stock=='future')?1:0;
			$scope.has_stock_present   += (!x.is_presale && x.type_stock=='present')?1:0;
			$scope.has_stock_present_china   += (!x.is_presale && x.type_stock=='present_china')?1:0;
			x.units_per_pack=total;
			return x;
		});
		console.log($scope.cart);
		console.log($scope.has_presale,
$scope.has_stock_future,
$scope.has_stock_present,
$scope.has_stock_present_china);
...
<script>
	$(".accordions .item-accordion-header").click(function(){
		let has_class=$(this).parent().hasClass("show");
