...
<h2>FW21 & Fexpro pop Customers Role -- Purchase List With Open Stock</h2>
   ...
   <thead>
	...
	<tbody id="myTable">
		<?php 
			...
			$thefTotal = $wpdb->get_row( $wpdb->prepare( "SELECT `forderunits` FROM {$wpdb->prefix}factory_order_confirmation_list WHERE vid = $key" ), ARRAY_A );
			if($thefTotal['forderunits'])
			{
				$fOrder = $thefTotal['forderunits'];
			}
			else
			{
				$thefTotal = $wpdb->get_row( $wpdb->prepare( "SELECT `forderunits` FROM {$wpdb->prefix}pop_factory_order_confirmation_list WHERE vid = $key" ), ARRAY_A );
				$fOrder = $thefTotal['forderunits'];
			}
				...
				echo "<tr>";
					...
					echo "<td class='".$_product->get_sku()."'>" . $fOrder . "</td>";
