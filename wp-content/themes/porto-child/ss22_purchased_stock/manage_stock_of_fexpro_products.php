...
$wpdb->query("TRUNCATE TABLE {$wpdb->prefix}show_fexpro_products_ss22");
foreach($final_result4 as $key => $value)
{
	$e = get_post_meta($key, '_stock', true);
	$wpdb->insert( 
		"{$wpdb->prefix}show_fexpro_products_ss22",
		array( 
			'vid' => $key, 
			'fexpro_stock' => array_sum($value), 
			'current_stock' => $e, 
		), 
		array( 
			'%d',
