...
foreach($getallOrdersList as $abc)
{
	...
	foreach($allData as $bk)
	{
		...
		else
		{
			...
			{
				$return_array1[$abc['vid']]['PA'][] = $bk['order_item_id'];
			}
   ...
   <thead>
	...
	<tbody id="myTable">
		...
		foreach($getallOrdersList as $key => $value)
		{
			...
			if(empty($merge5[$value['vid']]['PA']))
			{
				$pa = '';
			}
			else
			{
				$pa = array_sum($merge5[$value['vid']]['PA']);
			}
			
			if(empty($merge5[$value['vid']]['MX']))
			{
				$mx = '';
			}
			else
			{
				$mx = array_sum($merge5[$value['vid']]['MX']);
			}
