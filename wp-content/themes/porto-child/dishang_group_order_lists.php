...
$getZenlineOrdersList = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}factory_order_confirmation_list WHERE `factoryname` = 'Dishang Group/Weihai Textile Group Import & Export Co,. Ltd'", ARRAY_A );
...
<h2>Dishang Group/Weihai Textile Group Import & Export Co,. Ltd</h2>
	...
	<tbody id="myTable">
		...
		foreach($getZenlineOrdersList as $key => $value)
		{
			...
			echo "<tr class='$classs'>";
				...
				echo "<td class='".$_product->get_sku()."'>" . $value['deliverydate'] . "</td>";
				echo "<td class='".$_product->get_sku()."'>" . $value['costprice'] . "</td>";				
