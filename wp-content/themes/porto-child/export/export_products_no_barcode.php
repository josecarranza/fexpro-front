...
WHERE pp.post_type='shop_order' AND pp.post_status='wc-fw24-q4' AND pp.ID >=126125
 )
 GROUP BY pm.meta_value";
*/
$qdate="Q4";

$sql_variations_selled = "SELECT tmp.variation_id FROM (
	SELECT  pm.meta_value variation_id, COUNT(ppm.meta_key) cant_barcode
	FROM wp_woocommerce_order_items p 
	INNER JOIN wp_woocommerce_order_itemmeta pm ON p.order_item_id=pm.order_item_id AND pm.meta_key='_variation_id'
	...
	INNER JOIN wp_postmeta ppm0 ON ppm0.post_id=pm.meta_value AND ppm0.meta_key = '_sku'
...
$chunks =array_chunk($allProducts_list,500);
/*
echo "<pre>";
// print_r($allProducts);
...
//}
	...
	if ($dom->save('ss24_products_xml/INV_all_fexpro_ss24_products_no_barcode_'.($i_file+1).'.xml')) {
    	echo "Fexpro SS24 Product XML is created please click <a href='" . site_url() . "/wp-content/themes/porto-child/export/ss24_products_xml/INV_all_fexpro_ss24_products_no_barcode_".($i_file+1).".xml' target='_blank'>Click Here All products part ".($i_file+1)." </a> <br><hr>";
	} else {
