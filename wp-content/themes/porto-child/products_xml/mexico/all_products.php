...
$multipleSizeArray = Array("SIZES" => Array("S-M-L-XL" => "25", "4-6-8-10-12-14-16" => "3", "7-8-9-10-11" => "SD", "28-29-30-31-32-33" => "CL", "OS" => "OS", "8-10-12-14-16" => "YH"));

//$multipleCategoryArray = Array("CATEGORIES/GROUP" => Array("ACCESORIES" => "AC", "APPAREL" => "AP", "FOOTWEAR" => "FT", "UNDERWEAR AND SLEEPWEAR" => "UN", "DRINKWEAR" => "DK", "DRINKWARE" => "DK"));

$multipleCategoryArray = Array("CATEGORIES/GROUP" => Array("ACCESORIES" => "ACC", "APPAREL" => "AP", "FOOTWEAR" => "FT", "UNDERWEAR AND SLEEPWEAR" => "UN",  "LARGE EQUIPMENT" => "LEQ", "SMALL EQUIPMENT" => "SEQ", "HOODIE" => "HD", "JERSEY" => "JS",  "SHORTS" => "SH",  "TANK TOP" => "TT", "TRAINING JERSEY" => "JT", "TSHIRTS" => "TS", "DRINKWEAR" => "DK", "DRINKWARE" => "DK"));

$multipleSeasonArray = Array("SEASON" => Array("Fall Winter 21" => "212", "Basics" => "BAS"));

//$multipleSubCategoryArray = Array("SUB-CATEGORIES/SUB-GROUP" => Array("T-Shirts" => "TS", "Tank Top" => "TT", "Jerseys" => "JS", "Jackets" => "JK", "Hoodies" => "HD", "Long Sleeves" => "LS", "Sweatshirts" => "SW", "Training Jerseys" => "JT", "Pants" => "PT", "Polos" => "PL", "Shorts" => "SH", "Boxers and Underwear" => "BX", "Socks" => "SO", "Pijamas" => "PJ", "Slides" => "SL", "Flip Flops" => "FF", "Clogs" => "CLO", "Caps" => "CA", "Beanies" => "BN", "Scarf" => "SC", "Sport Bra" => "SBR", "Crop Top Tees" => "CT", "ACCESSORIES" => "AC"));

$multipleSubCategoryArray = Array("SUB-CATEGORIES/SUB-GROUP" => Array("Caps" => "CA", "Cap" => "CA", "Hoodie" => "HD", "Hoodies" => "HD", "Jacket" => "JK", "Jackets" => "JK", "Jerseys" => "JS", "Pant" => "PT", "Pants" => "PT", "Polo" => "PL", "Short" => "SH", "Shorts" => "SH", "T-Shirt" => "TS", "T-Shirts" => "TS", "Tank Top" => "TT", "Clogs" => "CLO", "Pijamas" => "PJ", "Benches" => "BC", "Bikes" => "BIKE", "Ellipticals" => "ELLI", "Mini Gyms" => "MGYM", "Treadmills" => "TRDM", "ACCESSORIES" => "AC", "Bands" => "BAN", "Jump Ropes" => "JUM", "Mats" => "MAT", "Weights" => "WEI", "Beanies" => "BN", "Scarf" => "SC", "Long Sleeves" => "LS", "Polos" => "PL",  "Sweater" => "SW","Sweaters" => "SW", "Sweatshirts" => "SW", "Tank Tops" => "TT", "Training Jerseys" => "JT", "Flip Flops" => "FS", "Slides" => "SL", "Boxers and Underwear" => "BX", "Socks" => "SO", "Crop Top Tees" => "CT", "Sport Bra" => "SBR", "Windbreaker" => "WB", "Canteen" => "CANT", "Coffee Mug" => "COFF", "Sport Bottle" => "SPB", "Tumbler" => "TUMB", "Water Bottles" => "WATB", "Wine Tumbler" => "WNT"));
...
foreach($allProducts as $key => $value)
{
	...
	if($css_slugGender[0] == 'Fitness' || $css_slugGender[0] == 'Fexpro POP')
	{
		continue;
	}
	else
	{
		...
		$codigodetalla[] = get_post_meta( $variationID, 'custom_field1', true );
		$codigodetalla[] .= get_post_meta( $variationID, 'custom_field2', true );
		$codigodetalla[] .= get_post_meta( $variationID, 'custom_field3', true );
		$codigodetalla[] .= get_post_meta( $variationID, 'custom_field4', true );
		$codigodetalla[] .= get_post_meta( $variationID, 'custom_field5', true );
		$codigodetalla[] .= get_post_meta( $variationID, 'custom_field6', true );
		$codigodetalla[] .= get_post_meta( $variationID, 'custom_field7', true );
		$codigodetalla[] .= get_post_meta( $variationID, 'custom_field8', true );
		$codigodetalla[] .= get_post_meta( $variationID, 'custom_field9', true );
		$codigodetalla[] .= get_post_meta( $variationID, 'custom_field10', true );		
		$codigodetallaCombine = implode("-", array_filter($codigodetalla));

		$codigo_de_talla = $multipleSizeArray['SIZES'][$codigodetallaCombine];
		...
		foreach($variationSize as $vSize)
		{
			...
			if(!empty($vSize))
			{
				...
				$result1->setAttribute('Sizes', "$vSize");
		...
		/* if($dom->save('INV_' . $value->ID . '.xml'))
		{
		   echo "FEXpro Product XML is created please click <a href='". site_url() ."/wp-content/themes/porto-child/products_xml/INV_" . $value->ID . ".xml' target='_blank'>Click Here " . $value->ID ."</a> <br><hr>"; 
		}
		else
		{
...
if($dom->save('INV_all_products_MEX.xml'))
	{
	   echo "FEXpro Mexico Product XML is created please click <a href='". site_url() ."/wp-content/themes/porto-child/products_xml/mexico/INV_all_products_MEX.xml' target='_blank'>Click Here All products </a> <br><hr>"; 
	}
	else
	{
		die('XML Create Error');
