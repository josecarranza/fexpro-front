...
$getallOrdersList = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}factory_order_confirmation_list WHERE `forderunits` != '0'", ARRAY_A );
/* echo "<pre>";
...
foreach($r1 as $r1value)
{
	...
	$getallOrdersList1 = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}factory_order_confirmation_list WHERE `forderid` = '$r1value' AND `forderunits` != '0'", ARRAY_A );
	...
	if($dom->save('CNF_' . $newName . '_MEX.xml'))
	{
	   echo "FEXpro Factory Mexico XML is created please click <a href='". site_url() ."/wp-content/themes/porto-child/factory_order/CNF_" . $newName . "_MEX.xml' target='_blank'>Click Here " . $newName ."</a> <br><hr>"; 
	} 
	else
	{
