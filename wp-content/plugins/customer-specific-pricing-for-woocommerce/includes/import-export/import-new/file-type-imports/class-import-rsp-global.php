...
class RspGlobalImport extends \cspImportExport\Import\FileTypeImport\CSPImportBase {
	...
	public function importRules( $rules, $headerMap = array()) {
		...
		foreach ($rules as $key => $aRule) {
			...
			$dbKey = $aRule['role'] . '_' . $aRule['min_quantity'];
	...
	public function getUniqueueEntitiesFromValidRules( $rules, $headerMap = array()) {
		...
		foreach ($rules as $aRule) {
			...
			if (!in_array($aRule['role'], $uniqueRoles, true) && is_numeric($aRule['role'])) {
		...
		return array('roles'=>$uniqueRoles, 'quantities'=>$uniqueueMinQuantities);
	}
	...
	public function getRelatedRulesFromDB( $distictEntities ) {
		...
		if (empty($distictEntities['quantities']) || empty($distictEntities['roles'])) {
			return $rules;	
		}

		$prepareArgs = array_merge($distictEntities['roles'], $distictEntities['quantities']);

		$result = $wpdb->get_results($wpdb->prepare('SELECT * From ' . $wpdb->prefix . 'wcsp_global_discount_rule_mapping WHERE rule_type = "RSP" AND type_id IN (' . implode(', ', array_fill(0, count($distictEntities['roles']), '%s')) . ') AND min_qty IN (' . implode(', ', array_fill(0, count($distictEntities['quantities']), '%d')) . ')', $prepareArgs));
	...
	public function updateRules( $rules = array()) {
		...
		foreach ($rules as $aRule) {
			...
			$updated = $wpdb->update($wpdb->prefix . 'wcsp_global_discount_rule_mapping', 
				...
				array('type_id'=>$aRule['role'], 'rule_type'=>'RSP', 'min_qty'=>$aRule['min_quantity']), 
				array('%s','%d'), 
				array('%s','%s','%d')
			);
