...
class UspCategoryImport extends \cspImportExport\Import\FileTypeImport\CSPImportBase {
	...
	public function importRules( $rules, $headerMap = array()) {
		...
		foreach ($rules as $key => $aRule) {
			...
			$dbKey = $aRule['user'] . '_' . $aRule['category_slug'] . '_' . $aRule['min_quantity'];
	...
	public function validateRules( $rules ) {
		...
		$validateProductIds = apply_filters('wdm_csp_import_validte_categories', true);
	...
	public function getUniqueueEntitiesFromValidRules( $rules, $headerMap = array()) {
		...
		return array('users'=>$uniqueUsers, 'category_slugs'=>$uniqueueCatSlugs, 'quantities'=>$uniqueueMinQuantities);
	}
	...
	public function getRelatedRulesFromDB( $distictEntities ) {
		...
		if (empty($distictEntities['category_slugs']) || empty($distictEntities['quantities']) || empty($distictEntities['users'])) {
			return $rules;	
		}
		...
		$result = $wpdb->get_results($wpdb->prepare('SELECT * From ' . $wpdb->prefix . 'wcsp_user_category_pricing_mapping WHERE user_id IN (' . implode(', ', array_fill(0, count($distictEntities['users']), '%d')) . ') AND cat_slug IN(' . implode(', ', array_fill(0, count($distictEntities['category_slugs']), '%s')) . ') AND min_qty IN (' . implode(', ', array_fill(0, count($distictEntities['quantities']), '%d')) . ')', $prepareArgs));

		foreach ($result as $aRule) {
			$key 		 = $aRule->user_id . '_' . $aRule->cat_slug . '_' . $aRule->min_qty;
	...
	public function insertRules( $rules = array()) {
		...
		foreach ($rules as $aRule) {
			$flatOrPercent = ( !empty($aRule['%_discount']) && $aRule['%_discount']>0 )?2:1;
			$price         = 2==$flatOrPercent?$aRule['%_discount']:$aRule['flat_price'];
			$insertStatus  =$wpdb->insert($wpdb->prefix . 'wcsp_user_category_pricing_mapping', array(
				'price'                     => $price,
	...
	public function updateRules( $rules = array()) {
		...
		foreach ($rules as $aRule) {
			...
			$updated = $wpdb->update($wpdb->prefix . 'wcsp_user_category_pricing_mapping', 
				array('price'=>$price, 'flat_or_discount_price'=>$flatOrPercent),
				array('user_id'=>$aRule['user'], 'cat_slug'=>$aRule['category_slug'], 'min_qty'=>$aRule['min_quantity']), 
				array('%s','%d'), 
				array('%d','%s','%d')
			);
