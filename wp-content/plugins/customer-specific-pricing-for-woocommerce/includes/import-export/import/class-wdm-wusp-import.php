...
if (!class_exists('WdmWuspImport')) {
	...
	class WdmWuspImport {
		...
		public $requiredHeaders  = array(
			'Products' => array(
				'usp' => array('sku', 'product id', 'user', 'min qty', 'price'), 
				'rsp' => array('sku', 'product id', 'role', 'min qty', 'price'),
				'gsp' => array('sku', 'product id', 'group name', 'min qty', 'price')
			),
			'Categories' => array(
				'usp' => array('user_id', 'category_slug', 'min_qty', 'flat_price', '%_discount'), 
				'rsp' => array('role', 'category_slug', 'min_qty', 'flat_price', '%_discount'),
				'gsp' => array('group_id', 'category_slug', 'min_qty', 'flat_price', '%_discount'),
			),
			'GlobalDiscounts' => array(
				'usp' => array('user_id', 'min_qty', 'flat_price', '%_discount'), 
				'rsp' => array('role', 'min_qty', 'flat_price', '%_discount'),
				'gsp' => array('group_id', 'min_qty', 'flat_price', '%_discount'),
			),
		...
		*/
		...
		public function wdmShowImportOptions() {
			$this->enqueueScripts();
			include_once plugin_dir_path(CSP_PLUGIN_FILE) . 'templates/admin/import-page.php';
			?>
		...
		private function createBatches( $post) {
			 ...
			 * 
			 * @param string|bool $nonce_verification default _wpnonce(nonce_name).
			 */
			...
			if (isset($post['wdm_import_csp'])) {
				...
				$importRulesFor = $post['wdm_csp_import_rules_for'];
				...
				switch ($importRulesFor) {
					...
					case 'Categories':
						include_once 'class-category-rule-import.php';
						$typeId    		  = self::getTypeIdFromInput($post['dd_show_import_options']);
						$validFileHeaders = \cspImportExport\cspImport\CSPCategoryRuleImport::isValidFile($foundHeaders, $typeId);
					
						if ($validFileHeaders) {
							$batchDir  = $this->wdmGetOrCreateUploadDirectory('importCsv');
							$batchDir  = $this->wdmClearDirectory($batchDir);
							$batchData = \cspImportExport\cspImport\CSPCategoryRuleImport::generateBatches($batchDir, $csvFile, $batchSize, $typeId);
							
							//Show progress bar on the page for import
							$filePointer = file($csvFile);
							$noOfRecords = count($filePointer) - 1;
							$fileType	 = 'category_' . $typeId;
							$this->displayImportProcessStatus($fileType, '', $noOfRecords);
							//Create a directory to store the reports if not exist, remove all the reports from existing directory. 
							...
							$this->wdmcallImportAjaxRequests($batchData);
							$foundHeaders[] = 'Import Status';
							$recordsToStore[] = $foundHeaders;
							global $cspFunctions;
							$cspFunctions->wdmSaveCSV('batch0', $recordsToStore, 'cspReports');
						} else {
							$requiredHeaders = implode(', ', \cspImportExport\cspImport\CSPCategoryRuleImport::getValidHeaderListFor($typeId));
							$currentHeaders  = implode(', ', $foundHeaders);
							include_once CSP_PLUGIN_PATH . '/templates/admin/import-csv-error.php';
						}
						break;
					case 'GlobalDiscounts':
						include_once 'class-global-rule-import.php';
						$typeId    		  = self::getTypeIdFromInput($post['dd_show_import_options']);
						$validFileHeaders = \cspImportExport\cspImport\CSPGlobalRuleImport::isValidFile($foundHeaders, $typeId);
						if ($validFileHeaders) {
							$batchDir  = $this->wdmGetOrCreateUploadDirectory('importCsv');
							$batchDir  = $this->wdmClearDirectory($batchDir);
							$batchData = \cspImportExport\cspImport\CSPGlobalRuleImport::generateBatches($batchDir, $csvFile, $batchSize, $typeId);

							//Show progress bar on the page for import
							$filePointer = file($csvFile);
							$noOfRecords = count($filePointer) - 1;
							$fileType	 = 'global_' . $typeId;
							$this->displayImportProcessStatus($fileType, '', $noOfRecords);
							//Create a directory to store the reports if not exist, remove all the reports from existing directory. 
							$reportsDir = $this->wdmGetOrCreateUploadDirectory('cspReports');
							$reportsDir = $this->wdmClearDirectory($reportsDir);
							...
							$requiredHeaders = implode(', ', \cspImportExport\cspImport\CSPGlobalRuleImport::getValidHeaderListFor($typeId));
							$currentHeaders  = implode(', ', $foundHeaders);
							include_once CSP_PLUGIN_PATH . '/templates/admin/import-csv-error.php';
						}
						break;
		...
		private function wdmGetBatchSize() {
			 ...
			 * 
			 * @param int $batchSize default 1000.
			 */
		...
		private function wdmcallImportAjaxRequests( $batchJsCallArray) {
			...
			/**
			 * Filter number of simultanious batches to use while importing the CSP rules.
			 * 
			 * @param int $noOfSimultaneousBatchImport default 2.
			 */
		...
		private function wdmGetFileIfValid() {
			/**
			 * This hook can be used to disable file type check for the import file.
			 * by default file type checking is enabled.
			 * 
			 * @param bool $fileTypeCheck
			 */
			$fileTypeCheck = apply_filters('wdm_csp_enable_import_file_type_check', false);
			$fileName = '';
		...
		*/
		public function checkFileHeaders( $foundHeaders, $requiredHeaders2, $requiredHeaders1) {
			...
			if ($foundHeaders !== $requiredHeaders2 && $foundHeaders !== $requiredHeaders1) {
				$requiredHeaders = implode(', ', $requiredHeaders2);
				$currentHeaders  = implode(', ', $foundHeaders);
				include_once CSP_PLUGIN_PATH . '/templates/admin/import-csv-error.php';
				return false;
			}
		...
		*/
		...
		/**
		 * This function is used to simplify the input received
		 * from the import form, the function returns
		 * usp for Wdm_User_Specific_Pricing_Import
		 * rsp for Wdm_Role_Specific_Pricing_Import
		 * gsp for Wdm_Group_Specific_Pricing_Import
		 *
		 * @since 4.6.0
		 * @param string $typeFromInput
		 * @return string usp|rsp|gsp
		 */
		public static function getTypeIdFromInput( $typeFromInput ) {
			$typeId = 'usp';
