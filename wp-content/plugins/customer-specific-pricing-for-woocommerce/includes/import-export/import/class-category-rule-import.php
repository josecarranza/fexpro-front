 ...
 * category based pricing rules in CSP.
 *
 * @since 4.6.0
 */
if (! class_exists('CSPCategoryRuleImport')) {

	class CSPCategoryRuleImport {
		...
		public static function getValidHeaderListFor( $importType) {
			...
			return array( $entityIdHeader, 'category_slug', 'min_qty', 'flat_price', '%_discount');
		}
		...
		public static function getBatchEntryByRuleTypeHeaders( $data, $validFileHeaders, $entityIdHeader) {
			...
			$category 		 = isset($data[$validFileHeaders['category_slug']]) ? $data[$validFileHeaders['category_slug']] : '';
		...
		public static function getEntityIdHeaderByRuleType( $ruleType) {
			...
			/**
			 * Filter for the heading of the category rule export CSV.
			 * 
			 ...
			 */
			return apply_filters('wdm_csp_import_cat_entity_header', $entityIdHeader, $ruleType);
		}
		...
		public static function importUSPRules( $batchFile, $headerMap, $entityIdHeader) {
			...
			while (false !== ( $data = fgetcsv($batchFile, 0, ',') )) {
				...
				$ruleValid = self::isValidCategoryRule($rule);
				
				if (!$ruleValid['status'] || false===$rule['user_id']) {
					...
					$status = self::updateUspRule($rule);
					if ('inserted'===$status) {
						$insertCnt++;
		...
		public static function importRSPRules( $batchFile, $headerMap, $entityIdHeader) {
			...
			while (false !== ( $data = fgetcsv($batchFile, 0, ',') )) {
				...
				if (!$ruleValid['status'] || false===$rule['role']) {
					...
					$status = self::updateRspRule($rule);
					if ('inserted'===$status) {
						$insertCnt++;
						$rowData[] = esc_html__('Inserted', 'customer-specific-pricing-for-woocommerce');
					} elseif ('updated' ===$status) {
		...
		public static function importGSPRules( $batchFile, $headerMap, $entityIdHeader) {
			...
			while (false !== ( $data = fgetcsv($batchFile, 0, ',') )) {
				...
				if (!$ruleValid['status'] || false===$rule['group_id']) {
					...
					$status = self::updateGspRule($rule);
					if ('inserted'===$status) {
						$insertCnt++;
						$rowData[] = esc_html__('Inserted', 'customer-specific-pricing-for-woocommerce');
					} elseif ('updated' ===$status) {
		 ...
		 */
		public static function isValidCategoryRule( $rule) {
			if (empty($rule['category_slug']) || empty($rule['min_qty']) || ( empty($rule['flat_price']) && empty($rule['%_discount']) )) {
				return array('status'=>false, 'message'=>esc_html__('Invalid or empty rule values', 'customer-specific-pricing-for-woocommerce'));
			}	

			if (! term_exists($rule['category_slug'], 'product_cat')) {
				return array('status'=>false, 'message'=>esc_html__('Category does not exists', 'customer-specific-pricing-for-woocommerce'));
			}
		 ...
		 */
		public static function updateUspRule( $rule) {
			global $wpdb;
			$userId    	= $rule['user_id'];
			...
			$result    	= $wpdb->get_results($wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'wcsp_user_category_pricing_mapping WHERE user_id=%d AND cat_slug=%s AND min_qty=%d', $userId, $category, $minQty));
			if (empty($result)) {
				$status = $wpdb->insert($wpdb->prefix . 'wcsp_user_category_pricing_mapping', array(
					'user_id'               => $userId,
					'cat_slug'              => $category,
					'min_qty'               => $minQty,
				...
				$status = $wpdb->update($wpdb->prefix . 'wcsp_user_category_pricing_mapping', array(
					'price'                     => $priceValue,
					'flat_or_discount_price'    => $priceType,
					), array(
					'user_id'  => $userId,
		 ...
		 * * Insert if the rule does not already exists
		 *
		 * @param array $rule CSP category RSP rule
		 * @return srting status of the rule import
		 */
		public static function updateRspRule( $rule) {
			global $wpdb;
			$role    	= $rule['role'];
			$category  	= $rule['category_slug'];
			$minQty    	= $rule['min_qty'];
			$priceType 	= !empty($rule['flat_price'])?1:2;
			$priceValue	= 2===$priceType?$rule['%_discount']:$rule['flat_price'];
			$result    	= $wpdb->get_results($wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'wcsp_role_category_pricing_mapping WHERE role=%s AND cat_slug=%s AND min_qty=%d', $role, $category, $minQty));
			if (empty($result)) {
				$status = $wpdb->insert($wpdb->prefix . 'wcsp_role_category_pricing_mapping', array(
					'role'               	=> $role,
					'cat_slug'              => $category,
					'min_qty'               => $minQty,
					'flat_or_discount_price'=> $priceType,
				...
				$status = $wpdb->update($wpdb->prefix . 'wcsp_role_category_pricing_mapping', array(
					'price'                     => $priceValue,
					'flat_or_discount_price'    => $priceType,
					), array(
					'role'     => $role,
		 ...
		 * * Insert if the rule does not already exists
		 *
		 * @param array $rule CSP category GSP rule
		 * @return srting status of the rule import
		 */
		public static function updateGspRule( $rule) {
			global $wpdb;
			...
			$priceType 	= !empty($rule['flat_price'])?1:2;
			$priceValue	= 2===$priceType?$rule['%_discount']:$rule['flat_price'];
			$result    	= $wpdb->get_results($wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'wcsp_group_category_pricing_mapping WHERE group_id=%d AND cat_slug=%s AND min_qty=%d', $groupId, $category, $minQty));
			if (empty($result)) {
				$status = $wpdb->insert($wpdb->prefix . 'wcsp_group_category_pricing_mapping', array(
					'group_id'              => $groupId,
					'cat_slug'              => $category,
					'min_qty'               => $minQty,
					'flat_or_discount_price'=> $priceType,
				...
				$status = $wpdb->update($wpdb->prefix . 'wcsp_group_category_pricing_mapping', array(
					'price'                     => $priceValue,
					'flat_or_discount_price'    => $priceType,
					), array(
					'group_id' => $groupId,
