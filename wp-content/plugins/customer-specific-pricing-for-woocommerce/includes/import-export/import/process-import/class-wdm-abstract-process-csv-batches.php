...
abstract class WdmAbstractProcessCSVBatches {
	...
	public function processBatch() {
		 ...
		 * 
		 * @param string|bool $nonce_verification
		 */
		...
		if (! $nonce_verification) {
			...
			/**
			 * Allow only admin to import csv files, this filter can be used to change the user capability required to access import feature.
			 * 
			...
			/**
			 * This filter can be used to conditionally enabled disable the access to the import feature.
			 * 
			 * @param string $currentUserCanImport Current User can import
			 */
		...
		if (false !== $getfile) {
			...
			while (false !== ( $data      = fgetcsv($getfile, 0, ',') )) {
				...
				if (isset($post['unique_id']) && !empty($primaryId)) {
					...
					/**
					 * Filter to check if the current product has the translations created with WPML, the same rules will be replicated for the translations as well.
					 * 
					 * @param bool|array $translatedProductIds translation product ids if exist or false.
					 */
					$hasTranslations = $productId!=$primaryId ? apply_filters('wdm_check_for_product_translations', false, $productId):false;
	...
	protected function getProductIdOfImportById( $uniqueId, $productId) {
		/**
		 * This filter can be used to filter the product Id fetched by the SKU for import feature.
		 * 
	...
	*/
	...
	/**
	 * Import nonce verification, checks if the user has a permission to
	 * imoprt the files & halts the execution on failure.
	 *   
	 * @since 4.6.0
	 */
	public function permissionCheck() {
		 ...
		 * Override nonce verification for extending import functionality in any third party extension
		 * 
		 * @param bool $nonce_verification Is the nonce valid.
		 */
		$nonce_verification = apply_filters('csp_import_nonce_verification', $nonce_verification);
		if (! $nonce_verification) {
			 echo esc_html__('Nonce verification failed', 'customer-specific-pricing-for-woocommerce');
			 exit;
		} else {
			/**
			 * Allow only admin to import csv files, this filter can be used to change the user capability required to access import feature.
			...
			$can_user_import    = apply_filters('csp_can_user_import_csv', current_user_can($capabilityToImport));
			if (!$can_user_import) {
				echo esc_html__('Sorry, please check if you have right permissions to proceed with this operation', 'customer-specific-pricing-for-woocommerce');
				exit;
			}
		}
	}
	...
	/**
	 * This function checks if the import file exists & is a valid batch file, 
	 * if the file present returns anarray of fileName & Batch Number else halts
	 * the execution with an error message
	 *
	 * @return array
	 */
	public function importFileTypeCheck( $postArray) { 
		$upload    = wp_upload_dir();
		$batchDir  = $upload['basedir'] . '/importCsv';
		$csvFile   = '';
		$fileName  = !empty($postArray['file_name'])?sanitize_file_name($postArray['file_name']):'';
		if (!empty($fileName) && file_exists($batchDir . '/' . $fileName)) {
			$csvFile = $batchDir . '/' . $fileName;
		} else {
			echo esc_html__('Please check the csv file', 'customer-specific-pricing-for-woocommerce');
			exit();
		}
		$batchNumber = 0;
		$batchNumber = isset($postArray['batch_number'])? sanitize_text_field($postArray['batch_number']):'';
		if (absint($batchNumber) <= 0) {
				echo esc_html__('Invalid batch number', 'customer-specific-pricing-for-woocommerce');
				exit();
		}
		return array('validFile'=> $csvFile, 'batchNumber'=>$batchNumber);
	}
}
