...
class WdmProcessUserSpecificCSVBatches extends WdmAbstractProcessCSVBatches {
	...
	*/
	public function __construct() {
		...
		add_action('wp_ajax_import_cat_usp_file', array($this, 'processCategoryUSPImport'));
		add_action('wp_ajax_import_global_usp_file', array($this, 'processGlobalUSPImport'));
	}
	...
	protected function processRow( $productId, $sku, $flag, $price, $priceType, $status, $batchNumber, $rowData, $skipCnt, $updateCnt, $insertCnt) {
		...
		if (( $isPriceEmpty && 2==$priceType )) {
			$skipCnt++;
			$status = $rowStatusMessages['discountOnEmptyPrice'];
	...
	public function processCategoryUSPImport() {
		...
		$responseData = \cspImportExport\cspImport\CSPCategoryRuleImport::importBatch($fileDetails['validFile'], $fileDetails['batchNumber'], 'usp');
	 ...
	 * @return void
	 */
	public function processGlobalUSPImport() {
		set_time_limit(0);
		wp_suspend_cache_addition(true);
		$this->permissionCheck();
		$postArray   = filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);
		...
		$responseData = \cspImportExport\cspImport\CSPGlobalRuleImport::importBatch($fileDetails['validFile'], $fileDetails['batchNumber'], 'usp');
		wp_send_json($responseData);
	}
