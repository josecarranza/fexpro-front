...
class WdmProcessGroupSpecificCSVBatches extends WdmAbstractProcessCSVBatches {
	...
	*/
	public function __construct() {
		...
		add_action('wp_ajax_import_cat_gsp_file', array($this, 'processCategoryGSPImport'));
		add_action('wp_ajax_import_global_gsp_file', array($this, 'processGlobalGSPImport'));
	}
	...
	public function processCategoryGSPImport() {
		...
		$responseData = \cspImportExport\cspImport\CSPCategoryRuleImport::importBatch($fileDetails['validFile'], $fileDetails['batchNumber'], 'gsp');
		wp_send_json($responseData);
	}

	/**
	* An ajax callback for the action import_global_gsp_file,
	* Initiates the process of global CSP pricing rules import for
	* group specific pricing rules
	*
	* @return void
	*/
	public function processGlobalGSPImport() {
		set_time_limit(0);
		wp_suspend_cache_addition(true);
		$this->permissionCheck();
		$postArray   = filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);
		...
		$responseData = \cspImportExport\cspImport\CSPGlobalRuleImport::importBatch($fileDetails['validFile'], $fileDetails['batchNumber'], 'gsp');
		wp_send_json($responseData);
	}
}
