...
class WdmProcessRoleSpecificCSVBatches extends WdmAbstractProcessCSVBatches {
	...
	*/
	public function __construct() {
		...
		add_action('wp_ajax_import_cat_rsp_file', array($this, 'processCategoryRSPImport'));
		add_action('wp_ajax_import_global_rsp_file', array($this, 'processGlobalRSPImport'));
	}
	...
	/**
	 * An ajax callback for the action import_cat_rsp_file,
	 * Initiates the process of category based CSP pricing rules import for
	 * user-role specific pricing rules
	 *
	 * @return void
	 */
	public function processCategoryRSPImport() {
		set_time_limit(0);
		wp_suspend_cache_addition(true);
		$this->permissionCheck();
		$postArray   = filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);
		$fileDetails = $this->importFileTypeCheck($postArray);

		include_once plugin_dir_path(CSP_PLUGIN_FILE) . 'includes/import-export/import/class-category-rule-import.php';
		$responseData = \cspImportExport\cspImport\CSPCategoryRuleImport::importBatch($fileDetails['validFile'], $fileDetails['batchNumber'], 'rsp');
		wp_send_json($responseData);
	}

	/**
	* An ajax callback for the action import_global_rsp_file,
	* Initiates the process of global CSP pricing rules import for
	* role specific pricing rules
	*
	* @return void
	*/
	public function processGlobalRSPImport() {
		set_time_limit(0);
		wp_suspend_cache_addition(true);
		$this->permissionCheck();
		$postArray   = filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);
		$fileDetails = $this->importFileTypeCheck($postArray);	
		include_once plugin_dir_path(CSP_PLUGIN_FILE) . 'includes/import-export/import/class-global-rule-import.php';
		$responseData = \cspImportExport\cspImport\CSPGlobalRuleImport::importBatch($fileDetails['validFile'], $fileDetails['batchNumber'], 'rsp');
		wp_send_json($responseData);
	}
}
