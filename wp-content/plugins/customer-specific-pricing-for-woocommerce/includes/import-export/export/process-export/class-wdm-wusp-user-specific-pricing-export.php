	...
	*/
	class WdmWuspUserSpecificPricingExport extends \cspImportExport\cspExport\WdmWuspExport {
		...
		public function wdmFetchData() {
			...
			/**
			 * Filter for the heading of the user based rule export CSV.
			 * 
			 * @param array $user_headings array of csv headers.
			 * @param string $exportType  type of the CSP rules.
			 */
			$user_headings = apply_filters('wdm_csp_filter_product_usp_export_headers', $user_headings, $exportType);
			if (!is_multisite()) {
		...
		public function processResult( $user_product_result, $exportType = 'product_id') {
			...
			/**
			 * Filter for changing the rules in the role based rules export file, this filter can be used to add/remove new columns from the export files.
			 * 
			 * @param array $formattedResult list of csp rule arrays.
			 * @param string $exportType type pf the CSP rules
			 */
			$formatedResult = apply_filters('wdm_csp_filter_product_usp_export_fields', $formatedResult, $exportType);
