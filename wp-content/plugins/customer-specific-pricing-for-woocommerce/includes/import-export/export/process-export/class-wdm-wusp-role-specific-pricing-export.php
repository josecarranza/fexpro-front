...
if (! class_exists('WdmWuspRoleSpecificPricingExport')) {
	class WdmWuspRoleSpecificPricingExport extends \cspImportExport\cspExport\WdmWuspExport {
		...
		public function wdmFetchData() {
			...
			/**
			 * Filter for the heading of the role based rule export CSV.
			 * 
			 * @param array $role_heading array of csv headers.
			 * @param string $exportType  type of the CSP rules.
			 */
			$role_heading        = apply_filters('wdm_csp_filter_product_rsp_export_headers', $role_heading, $exportType);
		...
		public function processResult( $role_product_result, $exportType = 'product_id') {
			 ...
			 */
			$formatedResult = apply_filters('wdm_csp_filter_product_rsp_export_fields', $formatedResult, $exportType);
