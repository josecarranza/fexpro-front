...
include_once 'class-wdm-usp-product-price-commons.php';
...
if (! class_exists('WuspCSPProductPrice')) {
	...
	class WuspCSPProductPrice extends \WuspSimpleProduct\WuspCSPProductPriceCommons {
		...
		/**
		 * An array to implement the catching in the
		 * method applyCustomPrice() this array holds the CSP prices
		 * for the product based on its quantity & user & returns if the
		 * the functiong is repeatedly called with the same arguements.
		 *   
		 * @since 4.6.0
		 * @var array
		 */
		public static $applyCustomPriceCache = array();
		/**
		...
		*/
		public function __construct() {
			...
			/**
			 * By default the product total will be displayed in the product summary, if one is 
			 * not showing the product summary on the product page & wants to show product total
			 * on some other action they can filter out the hook name.
			 * 
			 * @param string $hookName Show in product summary.
			 */
			$productTotalDisplayHook =  apply_filters('wdm_csp_filter_product_total_position', 'woocommerce_single_product_summary');
			...
			add_action($productTotalDisplayHook, array($this,'cspQuantityBasedProductTotal',), 10);
		 ...
		 */
		public function cspValidateAddToCart( $passed, $productId, $quantity = 1, $variationId = '') {
			$regularPrice    = get_post_meta($productId, '_regular_price', true);
			/**
			 * Filter the regular price being used for the CSP price calculations.
			 * 
			 * @param float $regularPrice - Regular price of the product.
			 * @param int $productId - Product ID for the product.
			 * @return float Filtered regular price.
			 */
		...
		public function getProductUrl( $url) {
			/**
			 * When a product is added to the cart user gets redirected to the product page, in case of variation the variation
			 * selection on the variable product page, this functionality may cause some conflicts with some themes allowing to add
			 * variations from the shop page, this filter can be use in such cases to prevent default CSP behaviour. 
			 * 
			 * @param string $unset - default 'unset' will apply CSP custom redirect any other value will ignore CSPs link.
			 * @param string $url - Current redirection url.
			 */
		...
		public function cspLimitCartQuantity( $cartItemKey, $quantity, $oldQuantity, $cart) {
			...
			$minQty =$this->cspGetDefaultMinimumQty($productId);
			/**
			 * This filter can be used to filter out the minimum urchase quantity applied for the product by CSP.
			 * 
			 * @param int $minQty minimum quantity defined by CSP Rules.
			 * @param int $productID woocommerce productId.
			 */
			$minQty = apply_filters('csp-filter-min-purchsable-quantity', $minQty, $productId);
			/**
			 * Filter 'wdm_csp_apply_csp_based_minimum_qty_for_product_in_cart' can be used to prevent CSP minQty from getting applied in the cart
			 * 
			 * @param bool $apply true|false.
			 * @param int $quantity Current quantity.
			 * @param object $cartItem.
			 */
			if ($quantity < $minQty && apply_filters('wdm_csp_apply_csp_based_minimum_qty_for_product_in_cart', true, $quantity, $cart->cart_contents[ $cartItemKey ])) {
		...
		public function wdmAvailabilityMessage( $product) {
			...
			/**
			 * This filter can be used to change the messages being displayed when quantity in the stock is lesser than
			 * the minimum purchase quantity set by CSP rules.
			 * 
			 * @param string $message - html content of the message to be displayed.
			 * @param int $stock - remaining stock quantity.
			 */
		...
		public function cspSetDefaultMinimumQty( $min, $product) {
			...
			/**
			 * This filter can be used to prevent CSP minQty from getting applied on the product
			 * 
			 * @param bool $apply true|false.
			 * @param int $min current minimum quantity.
			 * @param object $product WooCommerce Product object.
			 */
			if ($productPriceSet || !isset($product) || !apply_filters('wdm_csp_apply_csp_based_minimum_qty_for_product', true, $min, $product)) {
				return $min;
			}
			$productId =$product->get_id();
			$userId    =get_current_user_id();
		...
		*/
		public function applyQuantityPriceInCart() {
			...
			foreach ($woocommerce->cart->get_cart() as $cart_item_key => $cart_item) {
				...
				/**
				 * This filter can be used to preven CSP from applying CSP price to the cart item
				 * 
				 * @param bool $apply to apply CSP for the cart item.
				 * @param int $cart_product_id product id.
				 * @parma object WC cart item object.
				 */
				...
				/**
				 * This filter can be used to filter/chnge the already applied CSP price in the cart.
				 * 
				 * @param float $price CSP calculated price for the product.
				 * @param object $cart_item WC Cart item.
				 */
				$price = apply_filters('wdm_csp_filter_cart_price', $price, $cart_item);
		...
		*/
		public function cspQuantityBasedProductTotal() {
			...
			/**
			 * This filter can be ueful to conditionally show the product total on the product page.
			 * 
			 * @param bool $current_price_status Currently showing the price total for the product.
			...
			/**
			 * This action gets executed before the generation of the 'Product Total' html on the product pages.
			 *
			 * @since 1.0.0
			 * @param object $product WooCommerce Product Object
			 */
			...
			if ($product->is_type('variable')) {
				...
				$db_price     = $this->getUnitPrice($product_id, $db_price);
				$displayPrice = self::getDisplayPrices($product, $db_price);
				$minQty       = $this->cspGetDefaultMinimumQty($product_id);
				$display      = 1 == $minQty? 'none':'block';

				$priceSuffix        = get_option('woocommerce_price_display_suffix');
				$taxedPriceInSuffix = false;
				if (!empty($priceSuffix)) {
					$taxedPriceInSuffix = ( false!==strpos($priceSuffix, '{price_including_tax}') )?'includeTax':$taxedPriceInSuffix;
					$taxedPriceInSuffix = ( false!==strpos($priceSuffix, '{price_excluding_tax}') )?'excludeTax':$taxedPriceInSuffix;
				}

				$priceSuffix = false==$taxedPriceInSuffix ? $product->get_price_suffix($displayPrice, $minQty):'';
				$priceTotalHtml =  sprintf('<div id="product_total_price" style="display:' . $display . ';">%s %s</div>', __('Product Total:', 'customer-specific-pricing-for-woocommerce'), '<span class="price">' . wc_price($displayPrice) . '</span><span class="price-suffix">' . $priceSuffix . '</span>');
			}

			/* Note: Use only allowed html fields from array $allowedHtml*/
			/**
			 * Filter the price total html to be displayed for the product generated by CSP according to the CSP prices.
			 * 
			 * @param string $priceTotalHtml Current HTML code or the price total.
			 * @param object $prouct WooCommerce Product Object.
			 * @param float $displayPrice Unit Price to be considered for product total.
			 */
			echo wp_kses(apply_filters('wdm_csp_price_html', $priceTotalHtml, $product, $displayPrice), $allowedHtml);

			/**
			 * This action gets executed after the generation of the 'Product Total' html on the product pages.
			 *
			 * @since 1.0.0
			 * @param object $product WooCommerce Product Object
			 */
		...
		*/
		public function cspFrontEndScript() {
			...
			/**
			 * This filter can be used to conditionally restrict CSP assets from loading on the product pages.
			 * 
			 * @param bool $load Load CSP Assets or not.
			...
			if (is_user_logged_in() && is_product() && apply_filters('wdm_csp_load_single_product_assets', true, $post)) {
				...
				/**
				 * This filter can be used to conditionally enable CSP Strikethrough setting on the simple product page.
				 * 
				 * @param bool $isStrikethroughEnabled Strikethrough enabled or not.
				 * @param object $product WooCommerce Product Object.
				 */
		...
		*/
		...
		/**
		 * Accepts quantity based CSP prices array & rturned the tax
		 * applied CSP prices according to the taxation settings in 
		 * woocommerce tax settings 
		 *
		 * @since 4.5.0
		 * @param array $cspPrices
		 * @return array $taxedCSPPrices
		 */
		public static function getTaxedCSPPrices( $cspPrices) {
			$taxedCSPPrices = array();
			foreach ($cspPrices as $vid => $variationCSPs) {
				$product = wc_get_product($vid); 
				foreach ($variationCSPs as $qty => $price) {
						$taxedCSPPrices[$vid][$qty] = $product->get_price_suffix($price, 1);
				}
			}
			return $taxedCSPPrices;
		}
		...
		*/
		public function getVariableProductsArrayTobePassed( $product) {
			...
			foreach ($variation_ids as $variation_id) {
				...
				$regular_price             = $productVariation->get_regular_price();
				...
				if (!empty($regular_price) && $regular_price!=$csp_prices[$variation_id][1]) {
					...
					$regularPriceHtml[$variation_id] = $this->wdmAddRegularPriceNearPriceTable($cspSettings, $productVariation);
				}

				$cspPriceDiscriptions[$variation_id] =$this->wdmGetCSPDiscriptionIfAvailable($cspSettings, $variation_id);
			}
			
			if (false!=$taxedPriceInSuffix) {
				$taxedSuffixes = self::getTaxedCSPPrices($csp_prices);
			}

			if (!empty($csp_prices)) {
				$sameCspForAllVariants = self::doesAllPricesForQtyOneAreEqual($csp_prices);
				/**
				 * Filter for the marker $sameCSPForAllVariations, this marker is used
				 * to determine whether to show the price range or a single price or price range
				 * when the price for all the variations is same.
				 * 
				 * @param bool $sameCspForAllVariants if CSP is same for all the variations.
				 * @param array $csp_prices aray of CSP prices for variations.
				 * @param object $product WooCommerce Product Object.
				 */
				$sameCspForAllVariants = apply_filters('wdm_csp_same_csp_for_all_variations', $sameCspForAllVariants, $csp_prices, $product);
			}
			return array(
				...
				/**
				 * This filter can be used to disable quantity management on product page when external plugin for the same is used.
				 * 
				 * @param bool $isQuantityManagedExternally Wether plugins like WC Min Max Quantity are in use.
				 * @param object $product WC Product Object. 
				 */
				'is_wp_min_max_active'      => apply_filters('wdm_csp_external_qty_limits', defined('WC_MIN_MAX_QUANTITIES'), $product),
				'show_regular_price'        => $showRegularPrice,
				'regular_price_html'        => $regularPriceHtml,
				'csp_price_discriptions'    => $cspPriceDiscriptions,
				'taxed_suffixes'			=> empty($taxedSuffixes)?'':$taxedSuffixes,
				'same_csp_for_all_varients' => $sameCspForAllVariants,
			);
		}
		...
		public function applyCSPVariationPrice( $prices_array, $product) {
			/**
			 * The filter can be used to specify Wether to show the CSP prices in variable product price ranges on the shop & archieve pages.
			 * 
			 * @param bool $includeCSP default True.
			 * @param object $product WC product object.
			 */
			...
			if (!empty($uspRules) || !empty($rspRules) || !empty($gspRules)) {
				$uspPrices        = $this->cspCalculatePrices($uspRules, $prices, $product);
				$prices           = $this->replaceWithCspPrices($prices, $uspPrices);
				$variationsToSkip = array_keys($uspPrices);
				$rspPrices        = $this->cspCalculatePrices($rspRules, $prices, $product, $variationsToSkip);
				$prices           = $this->replaceWithCspPrices($prices, $rspPrices);
				$variationsToSkip = array_merge($variationsToSkip, array_keys($rspPrices));
				$gspPrices        = $this->cspCalculatePrices($gspRules, $prices, $product, $variationsToSkip);
			...
			/**
			 * This filter can be used to filter out the calculated CSP prices for the variable product variations.
			 * 
			 * @param array $prices array of CSP price pairs for variations
		...
		public function applyCustomPrice( $price, $product) {
			...
			$supportedProductTypes = array('simple', 'variable', 'variation', 'course');
			if (!\in_array($product->get_type(), $supportedProductTypes, true)) {
				return $price;
			}
			...
			/**
			 * This filter can be used to conditionally disable CSP price calculation for the simple product.
			 * 
			 * @param bool $calculateCSP default True.
			 * @param object $product WC product object.
			 */
			...
			if (!is_admin()) {
				...
				$key = $product_id . '_' . $userId . '_' . $minQty;
				if (isset(self::$applyCustomPriceCache[$key])) {
					return self::$applyCustomPriceCache[$key];
				} else {
				...
				if (isset($db_price) && $db_price>=0) {
					/**
					 * Filter Calculated CSP price conditionaly for the application in the cart.
					 * 
					 * @param float $db_price CSP price.
					 * @param int $product_id WooCommerce productID
					 */
					$cspPrice 						   = apply_filters('wdm_apply_csp_product_price', $db_price, $product_id);
					self::$applyCustomPriceCache[$key] = $cspPrice;
					return $cspPrice;
				}
		...
		*/
		public static function getDBPrice( $product_id, $price, $qty = 1, $user_id = null) {
			...
			/**
			 * Filter the calculated CSP product price for the specified quantity.
			 * 
			 * @param float $cspPrice CSP calculated price for the quantity.
			 * @param int $product_id Id of the woocommerce product.
			 * @param float $price actual price of the product.
			 * @param int $quantity Quantity of the product for which the CSP price is being calculated.
			 * @param array $csp_prices List of CSP quantity-price pairs.
			 * @param array $qtyList List of quantities for which CSP prices are present.
			 */
		...
		*/
		public static function getQuantityBasedPricing( $product_id, $user_id = null) {
			if (is_user_logged_in() || ! empty($user_id)) {
				...
				/**
				 * This filter can be useful when it is required to copy CSP pricing of the one product for another,
				 * This filter can be used for WPML translations of the product.
				 * 
				 * @param int $product_id product id of the WooCommerce Product.
				 */
				...
				/**
				 * This filter can be used to filter the Role specific pricing rules fetched for the product.
				 * 
				 * @param array $rsp_price role specific pricing rules.
				 * @param array $csp_price customer/user specific pricing rules.
				 */
				$rsp_price	= apply_filters( 'wdm_csp_filter_rsp_quantity_prices', $rsp_price, $csp_price );

				$gsp_price = false;
				$gsp_price =self::wdmGetGspPrice($user_id, $product_id);
				/**
				 * This filter can be used to filter the Group specific pricing rules fetched for the product.
				 * 
				 * @param array $gsp_price group specific pricing rules.
				 * @param array $csp_price customer/user specific pricing rules.
				 */
				$gsp_price = apply_filters( 'wdm_csp_filter_gsp_quantity_prices', $gsp_price, $csp_price );
				...
				if (!isset($qtyList) || count($qtyList) <= 0) {
					...
					if (!empty($catPrices) && $product->get_type() != 'variable') {
						...
						/**
						 * This filter can be used to filter the CSP prices for the product
						 * 
						 * @param array $catPrices CSP quantity-price pairs.
						 * @param object $product WC product object. 
						 */
						 ...
						 * This filter can be used to filter the CSP prices for the product
						 * 
						 * @param array $qtyList CSP quantity-price pairs.
						 * @param object $product WC product object. 
						 */
				 ...
				 * This filter can be used to filter the CSP prices for the product
				 * 
				 * @param array $mergedPrices CSP quantity-price pairs
				 * @param object $product WC product object. 
				 */
		...
		*/
		public function showQuantityBasedPricing( $price, $product) {
			...
			/**
			 * The filter can be used to determine wether to show the CSP quantity based prices on the product pages.
			 * 
			 * @param bool $show Show CSP prices or not.
			 * @param object $product WC product object.
			 */
			if (!is_product() || $wp_query->queried_object_id != $product_id || !apply_filters('wdm_csp_show_quantity_based_price', true, $product)) {
				/**
				 * This filter can be used to show the CSP quantity based pricing on the archieve pages.
				 * 
				 * @param bool $show default False.
				 * @param object $product WC Product Object.
				 */	
				$showPriceTableOnShopPage = apply_filters('wdm_csp_show_quantity_based_price_on_shop', false, $product);
				if (!$showPriceTableOnShopPage) {
					return $price;
				}
			}
			...
			/**
			 * Conver the CSP price as per the requirement of the multicurrency extension.
			 * 
			 * @param float $regularPrice Regular price of the product.
			 */
			...
			if (isset($csp_prices) && $csp_prices) {
				if (1 === count($csp_prices)) {
					...
					if (1 == $keys[0]) {
						...
						if ($isStrikeThroughEnabled && $cspPrice < $regularPrice) {
							...
							$price = self::getDisplayPrice($product, $cspPrice);
							$price = wc_price($price) . $product->get_price_suffix();
						}
						$regPriceDisplay = $cspPrice < $regularPrice ? $this->wdmAddRegularPriceNearPriceTable($cspSettings, $product):'';
						$price           = $this->wdmGetCSPDiscriptionIfAvailable($cspSettings, $product_id) . $price;
						$price           = $regPriceDisplay . $price;
						/**
						 * Filter CSP price html
						 * 
						 * @param string $price Current CSP price HTML
						 * @param float $cspPrice CSP price.
						 * @param object $product wc product object.
						 */
				...
				$table = $this->wdmAddRegularPriceNearPriceTable($cspSettings, $product) . $table;
				/**
				 * Filter CSP price table html
				 * 
				 * @param string $table Current CSP price HTML
				 * @param float $cspPrice CSP price list.
				 * @param object $product wc product object.
				 */
				return apply_filters('wdm_csp_html_price_format', $table, $csp_prices, $product);
			}
			return $price;
		}
		...
		public static function wdmGetQtyDiscountTable( $product, $csp_prices) {
			...
			foreach ($csp_prices as $qty => $price) {
				$table .= '<tr>';
				$table .= '<td class = "qty-num">' . $qty . ' ' . $moreText . '</td><td class = "qty-price">' . wc_price(self::getDisplayPrice($product, $price)) . $product->get_price_suffix($price, 1) . '</td>';
				$table .= '</tr>';
			}
			$table .= '</table></div></div>';
			return $table;
		...
		*/
		...
		public function showCSPAsSalePrice( $onSale, $product) {
			...
			/**
			 * Filter to conditionally enable/diable CSP as a sale price feature. 
			 * 
			 * @param bool $cspDontCheckSale Wheter to check if CSP as sale price
			 * @param object $product Wc product object.
			 * @param int $userId id of the signed in user.
			 */
			...
			/**
			 * A filter which can be used to conditionally manage showing CSP as a sale price.
			 * 
			 * @param bool $onSale Is product on sale according to the CSP.
			 * @param object $product wc product object.
			 * @param int $userId id of the signed in user. 
			 */
