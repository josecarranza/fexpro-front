...
abstract class CspMappings extends \WC_REST_Controller {
	...
	public function generateResponseData( $response, $request) {
		...
		/**
		 * This filter can be used to modify the APi response body.
		 * 
		 * @param object $response responce generated by API callback.
	...
	/**
	 * Checks if the request sender has the capability to delete.
	 *
	 * @since 4.5.3
	 * @param  WP_REST_Request $request Full details about the request.
	 * @return bool|WP_Error
	 */
	public function isUserAllowedToDelete( $request) {
		$user = wp_get_current_user();
		/**
		 * This filter can be used to allow/disallow users with specific capabilitys to perform delete operations using CSP API.
		 * 
		 * @param array $userRoles array of alloweded user roles.
		 */
		$allowed_roles = apply_filters('wdm_csp_api_roles_with_rule_delete_permission', array('administrator', 'shop_manager'));
		if ( 1 > count( array_intersect( $allowed_roles, $user->roles )) ) {
			return new \WP_Error('cspapi_rest_cannot_delete', __('Sorry, you are not allowed to delete these resources.', 'customer-specific-pricing-for-woocommerce'), array('status' => rest_authorization_required_code()));
		}
		return true;
	}
	...
	public function getFailedMessage( $key, $item = array()) {
		...
		/**
		 * This filter can be used to modify the failed message responce from API.
		 * 
		 * @param string $failedMessage for the request.
		 * @param string $key key of the request Item.
		 * @param array $item rule data for which the failed message is generated.
		 */
	...
	public function getSuccessMessage( $key, $item) {
		...
		/**
		 * This filter can be used to modify the success message responce from API.
		 * 
		 * @param string $successMessage for the request.
		 * @param string $key key of the request Item.
		 * @param array $item rule data for which the success message is generated.
		 */
