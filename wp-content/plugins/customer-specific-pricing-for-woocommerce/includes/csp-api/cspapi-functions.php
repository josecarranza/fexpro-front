...
function cspapiGenerateProductHashByEntity( $productID, $entity, $qty, $discountType, $price) {
	/**
	 * Filter for productId in the API request
	 * 
	 * @param int $productId WC product Id.
	 */
	$productID    = apply_filters('cspapi_product_id_for_hash', (int) $productID);
	/**
	 * Filter for entity in the API request
	 * 
	 * @param int $entity user|role|group.
	 */
	$entity       = apply_filters('cspapi_customer_entity_for_hash', $entity);
	/**
	 * Filter for quantity in the API request
	 * 
	 * @param int $qty quantity in the CSP rule.
	 */
	$qty          = apply_filters('cspapi_quantity_for_hash', (int) $qty);

	/**
	 * Filter for discountType in the API request
	 * 
	 * @param int $discountType falt|%.
	 */
	$discountType = apply_filters('cspapi_product_id_for_hash', $discountType);

	/**
	 * Filter for discounted price or % discount value in the API request
	 * 
	 * @param int $price flat price or % discount value.
	 */
...
function cspapiIsAPIHashValid( $clientHash, $serverHash, $item) {
	...
	/**
	 * This filter can be used to enable/disable hash validation in CSP rest API.
	 * 
	 * @param bool $isHashValid true|false
	 * @param string $clientHash MD5 hash value shared by the client.
	 * @param string $serverHash MD5 hash generated by the server to validate the rule.
	 * @param array $item CSP rule data submited in API request.
	 */
...
function cspapiGetDefaultCspData( $item, $key) {
	 ...
	 * 
	 * @param array $cspData Data submited by API request.
	 * @param array $item CSP Rule data.
