	...
	class WdmWuspVariableProductsUsp {
		...
		*/
		...
		public function variableFields( $loop, $variation_data, $variation) {
			global $wpdb, $wp_version, $cspFunctions;
			...
			/**
			 * This filter can be used to fill all the CSP rules on the variation edit section with the rules set of its base product.
			 *
			 *  @param mixed $variationId WooCommerce Product Id.
			 */
							...
							if (! empty($user_price_result)) {
								foreach ($user_price_result as $key => $rows) {
									...
									<tr>
										...
										<td><input type="text" name="wdm_woo_variation_price[<?php echo esc_attr($variation_data['variation_post_id']); ?>][]" size="5" class="<?php echo esc_attr($cspPercentClass); ?>" value="<?php echo esc_attr(wc_format_localized_price(wc_format_decimal($rows->price, '', true))); ?>" /></td>
		...
		*/
		private function addVariationUserPriceMappingInDb( $variable_csp_user, $variable_price_field, $variable_qty_field, $variable_price_type, $var_id) {
			...
			if (isset($variable_csp_user[$var_id]) && ! empty($variable_csp_user[$var_id]) && isset($variable_qty_field[$var_id]) && ! empty($variable_qty_field[$var_id])) {
				...
				/**
				 * This action gets triggerd on saving the customer specific product pricing rules for variations
				 * 
		...
		public function variationPriceFormat( $price, $product) {
			...
			/**
			 * This filter can be used to enable disable strikethrough on the variation prices when CSP strikethrough setting is enabled.
			 * 
		...
		*/
		...
		public function getCustomPriceOfVariation( $price, $product) {
			...
			$user_id = get_current_user_id();
			/**
			 * This filter can be used to enable/disable custom price html format for variations
			 * 
			 * @param bool $enableCustomHtml Default True.
			 */
