	...
	class WdmWuspGetCategoryData {
		...
		*/
		public function getUsersCategoryPricingPairs( $currentUserId, $catArray, $productId) {
			...
			$userPrice = array();
			$price     = array();
			$price     = $this->getCatUSPFromDatabase($currentUserId, $catArray, $productId);
		 ...
		 * @return void
		 */
		public function getCatUSPFromDatabase( $currentUserId, $catArray, $productId = '') {
			global $wpdb;
			$price = array();
			...
			$price       = $wpdb->get_results($wpdb->prepare('SELECT user_id, price, min_qty,
			...
			/**
			 * This filter can be used to filter the category based user specific pricing rules applicable to the product for the user.
			 * this filter is also used internally.
			 * 
			 * @param array $price array of CSP rules fetched from the database.
			 * @param int $currentUserId Id of the WP user for which the rules are fetched.
			 * @param array $catArray Array of category slugs.
			 */
			$price	= apply_filters('wdm_csp_filter_usp_rules_for_categories', $price, $currentUserId, $catArray, $productId);
		...
		*/
		public function getRolesCategoryPricingPairs( $currentUserId, $catArray, $productId, $roleList = array()) {
			...
			$price               = $this->getCatRSPFromDatabase($userRoles, $catArray, $productId);
			if (null==$price) {
				return;
			}
		 ...
		 *
		 * @since 2.4.5
		 * @param array $userRoles array of roles to which the user belongs.
		 * @param array $catArray array of category slugs to fetch CSP rules for.
		 * @param int $productId product Id for which the category rules are being fetched.
		 * @return array Arrar Of RSP rules for the categories provided
		 */
		public function getCatRSPFromDatabase( $userRoles, $catArray, $productId = '') {
			global $wpdb;
			$price = array();
			...
			$price       = $wpdb->get_results( $wpdb->prepare(
										'SELECT role, price, min_qty, cat_slug,
										...
										ORDER BY min_qty', $prepareArgs));
			/**
			 * This filter can be used to filter the category based role specific pricing rules applicable to the product for the user.
			 * this filter is also used internally.
			 * 
			 * @param array $price array of CSP rules fetched from the database.
			 * @param array $userRoles array of user role slugs for which the rules are fetched.
			 * @param array $catArray Array of category slugs.
			 */
			$price = apply_filters('wdm_csp_filter_rsp_rules_for_categories', $price, $userRoles, $catArray, $productId);
		...
		*/
		public function getGroupsCategoryPricingPairs( $currentUserId, $catArray, $productId, $groupIds = array()) {
			...
			if ($userGroupId && 'NOT_ADDED_TO_ANY_GROUP'!=$userGroupId) {
				...
				$price 		  = $this->getCatGSPFromDatabase($groupIds, $catArray, $productId);
				if ( null != $price ) {
					for ($i = 0; $i < count($price); $i++) {
						$currentMinQty       = $price[$i]->min_qty;
						$category            = get_term_by('slug', $price[$i]->cat_slug, 'product_cat');
		 ...
		 * @param array $catArray aray of categories for which the CSP rules are required
		 * @return array array of GSP category rules
		 */
		public function getCatGSPFromDatabase( $userGroupIds, $catArray, $productId = '') {
			global $wpdb;
			$price = array();
			...
			$price       = $wpdb->get_results($wpdb->prepare('SELECT group_id, price, min_qty, cat_slug,
																	 flat_or_discount_price as price_type, 
			...
			/**
			 * This filter can be used to filter the category based group specific pricing rules applicable to the product for the user.
			 * this filter is also used internally.
			 * 
			 * @param array $price array of CSP rules fetched from the database.
			 * @param array $userGroupIds array of user group Ids for which the rules are fetched.
			 * @param array $catArray Array of category slugs.
			 */
			$price = apply_filters('wdm_csp_filter_gsp_rules_for_categories', $price, $userGroupIds, $catArray, $productId);
