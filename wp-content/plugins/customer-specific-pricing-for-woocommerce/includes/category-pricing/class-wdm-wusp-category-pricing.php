	...
	class WdmWuspCategoryPricing {
		...
		*/
		public function cspCategoryEnqueueScripts() {
			if (isset($_GET['page']) && 'wisdm-csp-cat-pricing'==$_GET['page']) {
		...
		* Displays the group-specific-pricing section in category-pricingz
		* tab.
		* For the last row add an option for adding new pricing pair.
		*/
		...
		*/
		public function generateRoleDropdown( $role = '') {
			...
			<?php
				...
				echo wp_kses(csp_dropdown_roles($role), $allowedHTMLInDropdown);
				?>
			</select>
			<?php
		}
		...
		public function generateCategoryOptions( $catSlug = null) {
			...
			foreach (self::$product_categories as $category) {
				if ($catSlug == $category->slug) {
					echo "<option value = '" . esc_attr($category->slug) . "' selected>" . esc_html(apply_filters('wdm_csp_filter_dropdown_category_name', $category->name, $category)) . '</option>';
				} else {
					echo "<option value = '" . esc_attr($category->slug) . "'>" . esc_html(apply_filters('wdm_csp_filter_dropdown_category_name', $category->name, $category)) . '</option>';
				}
			}
		}
	}
