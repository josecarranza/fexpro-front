...
if (! class_exists('WdmWuspGetData')) {
	class WdmWuspGetData {
		...
		public static function getAllPricesForSingleProduct( $product_id) {
			global $wpdb;
			/**
			 * This filter will be useful when it is required to get CSP pricing rules for the WPML translated product,
			 * return the base product Id for the product Id using this filter for the same.
			 * 
		...
		public static function getPriceOfProductForUser( $user_id, $product_id) {
			...
			$get_price = $wpdb->get_results($wpdb->prepare('SELECT user_id, price, min_qty, flat_or_discount_price as price_type FROM ' . $wpdb->prefix . 'wusp_user_pricing_mapping WHERE user_id=%d AND product_id=%d ORDER BY min_qty', $user_id, $product_id));
			/**
			 * This filter can be used to filter the CSP user specific rules fetched for the product from the database.
			 * 
			 * @param array $get_price array of CSP rule objects fetched from the database.
			 * @param int $user_id Id of the WP user for whom these rules are fetched.
			 * @param int $product_id Id of the woocommerce product for which these rules are fetched.
			 */
		...
		public static function getQtyPricePairsOfProductForGroup( $user_id, $product_id, $groupIds = array()) {
			...
			if ($user_groupid) {
				...
				foreach ($user_groupid as $single_user_groupid) {
					...
					$price = $wpdb->get_results($wpdb->prepare('SELECT group_id, price, min_qty, product_id, flat_or_discount_price as price_type FROM ' . $wpdb->prefix . 'wusp_group_product_price_mapping WHERE group_id=%d AND product_id=%d ORDER BY min_qty', $single_user_groupid->group_id, $product_id));
					
					
					/**
					 * This filter can be used to filter the CSP user specific rules fetched for the product from the database.
					 * 
					 * @param array $price array of CSP rule objects fetched from the database.
					 * @param int $group_id Id of the WP user group for which these rules are fetched.
					 * @param int $product_id Id of the woocommerce product for which these rules are fetched.
					 * */
