...
if (!class_exists('WdmCustomerSpecificPricingNewOrder')) {
	class WdmCustomerSpecificPricingNewOrder {
		...
		public function cspChangeTemplateTotalSubtotal( $product, $orderItem, $orderItemId) {
			/**
			 * This filter can be used to enable disable the CSP pricing the WooCommerce Manual orders
			 * 
			 * @param bool $changeTotalAsPerTheCSP true|false (default: true).
			 * @param object $product WooCommerce product object.
			 * @param object $orderItem Item to be included in the manual order.
			 * @param int $orderItemId Id of the Item being added in the cart.
			 */
			if (apply_filters('wisdm_csp_filter_woocommerce_admin_order_item_values', true, $product, $orderItem, $orderItemId)) {
			...
			if (!empty($post) && 'shop_order'==$post->post_type) {
				...
				wp_localize_script(
								...
								array(
										...
										'order_id' => $post->ID,
										/**
										 * This filter can be used to enable using wc order item customized price as the regular price while applying CSP prices, this will be
										 * helpful to enable custom prices for 
										 */
										'allow_custom_price_for_wc_admin_order_item' => apply_filters('wdm_csp_allow_custom_price_for_wc_admin_order_item', false, $post->ID),
									));
			}
		}
