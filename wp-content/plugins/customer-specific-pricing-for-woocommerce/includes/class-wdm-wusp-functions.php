	...
	class WdmWuspFunctions {
		...
		*/
		public function getProductCategories( $product) {
			...
			if ('simple' === $product->get_type() || 'variable' === $product->get_type()) {
				...
				$catArray = wp_get_post_terms($productId, 'product_cat');
			}

			$cspSettings                = get_option('wdm_csp_settings');
			$getParentCategories 		= isset($cspSettings['csp_enable_category_hierarchy']) && 'enable' == $cspSettings['csp_enable_category_hierarchy'] ? true : false;
			if ( $getParentCategories) {
					$categoryIds = $this->getHierarchyOfCategories($catArray);
					$catArray   = $this->getCategoriesByIds($categoryIds);
			}
			return $catArray;
		}
		...
		*/
		...
		public function callRuleCreatedHook( $selection_entry_list, $selection_details) {
			...
			if (!empty($selection_entry_list)) {
				...
				include_once CSP_PLUGIN_PATH . "/includes/rules/wdm-csp-rule-$ruleType.php";
				...
				/**
				 * This action gets executed on saving the CSP rules to the database.  
				 * 
				 * @since 4.3.0
				 * @param string  $ruleCreationType CSP rule type
				 * @param array $wdmSavedRules {
				 *			@type object CustomerBasedRule|RoleBasedRule|GroupBasedRule
				 * }
				 */
		...
		*/
		...
		public function wdmGetSalePrice( $productId) {
			...
			/**
			 * This filter can be used to filter the sale price being used by the CSP for price calculations.
			 * 
			 * @param float $salePrice Current sale price of the product.
			 * @param int $productId Id of the WooCommerce product for which the sale price is fetched.
			 */
		...
		*/
		...
		public function getProductSku( $product_id) {
			...
			/**
			 * This filter can be used to filter/change the product SKU fetched for the product using product Id
			 * 
			 * @param string $sku Current SKU for the product.
		...
		public function getSiteUserIdNamePairs() {
			...
			/**
			 * This filter will allow to filter the list of username, userid pairs retrieved from the database.
			 * 
			 * @param array $usersList
			 */
			return apply_filters('wdm_csp_filter_site_user_id_name_pairs', $usersList);
		}
		...
		/**
		 * * This method accepts the array of categories as a parameter,
		 * * Finds all the parent categories of each of the categories.
		 * * Returns Ids of all the categories found.
		 *
		 * @since 4.5.0
		 * @param array $category - array of term objects
		 * @return array array of term ids
		 */
		private function getHierarchyOfCategories( $catArray) {
			$catIds = array_column($catArray, 'term_id');
			$uniqueCategoryIds = $catIds;
			if (!empty($catArray)) {
				foreach ($catIds as $catId) {
					$idsOfCatsInHierarchy = $this->getParentHierarchy($catId);
					$uniqueCategoryIds = array_unique(array_merge($uniqueCategoryIds, $idsOfCatsInHierarchy));
				}	
			}
		...
		/**
		 * * This method accepts the term id
		 * * Finds all of its parent categories in a hierarchy
		 * * Returns an array of all the ids of the categories found in a hierarchy.
		 *
		 * @since 4.5.0
		 * @param int $catId
		 * @return array - array of category ids.
		 */
		private function getParentHierarchy( $catId) {
			$catIds = array();
			while (isset(self::$categories[$catId]) && 0!=self::$categories[$catId]->parent) {
				$catIds[] = self::$categories[$catId]->parent;
				$catId	= self::$categories[$catId]->parent;
			}
			return $catIds;
		}
		...
		/**
		 * This method retuns the array of wp_term object 
		 * for the ids of the categories mentione din the parameter
		 *
		 * @since 4.5.0
		 * @param array $categoryIds
		 * @return array - array of wp-term objects
		 */
		private function getCategoriesByIds( $categoryIds) {
			$catArray = array();
			foreach ($categoryIds as $catId) {
				if (isset(self::$categories[$catId])) {
					$catArray[] = self::$categories[$catId];
				}
			}
			return $catArray;
		}
		...
		/**
		 * This function should be called when the groups plugin is not installed
		 * or not active, the function will output the text with the direct installation & 
		 * direct activation links for the the plugin groups by ItThinks
		 *
		 * @since 4.6.0
		 * @return void
		 */
		public function showGroupPluginDirectInstallLinks() {
			include_once CSP_PLUGIN_PATH . '/includes/class-wp-org-plugin-installer.php';
			$WpOrgPluginInstaller = new \WdmCSP\WpOrgPluginInstaller('groups', 'groups', 'groups.php' );
			if ( ! file_exists(WP_CONTENT_DIR . '/plugins/groups/groups.php')) {
				$installLink = $WpOrgPluginInstaller->getInstallUrlHtml('Install' );
				echo wp_kses_post($installLink . ' & ' . __("Activate the 'Groups' Plugin to enjoy the benefits of Group Specific Pricing.", 'customer-specific-pricing-for-woocommerce'));
			} else {
				$ActivationLink = $WpOrgPluginInstaller->getActivateUrlHtml('Activate' );
				echo wp_kses_post( $ActivationLink . __(" the 'Groups' Plugin to enjoy the benefits of Group Specific Pricing.", 'customer-specific-pricing-for-woocommerce'));
			}
		}


		/**
		 * This function has been added for the admin to be able to stop manipulation of
		 * the CSP rules from the product edit pages, this function simple contains the filter
		 * 'wdm_csp_usp_update_from_simple_product_edit_page' which could be used to determine
		 * wether it is alloweded to update CSP rules from the product edit page.
		 *
		 * @return boolean
		 */
		public function isUSPUpdateEnabledFromProductEditPage() {
			return apply_filters('wdm_csp_usp_update_from_product_edit_page', true);
		}

		/**
		 * This function has been added for the admin to be able to stop manipulation of
		 * the CSP role based rules from the product edit pages, this function simple contains the filter
		 * 'wdm_csp_rsp_update_from_simple_product_edit_page' which could be used to determine
		 * wether it is alloweded to update CSP rules from the product edit page.
		 *
		 * @return boolean
		 */
		public function isRSPUpdateEnabledFromProductEditPage() {
			return apply_filters('wdm_csp_rsp_update_from_product_edit_page', true);
		}

		/**
		 * This function has been added for the admin to be able to stop manipulation of
		 * the CSP group based rules from the product edit pages, this function simple contains the filter
		 * 'wdm_csp_gsp_update_from_simple_product_edit_page' which could be used to determine
		 * wether it is alloweded to update CSP rules from the product edit page.
		 *
		 * @return boolean
		 */
		public function isGSPUpdateEnabledFromProductEditPage() {
			return apply_filters('wdm_csp_gsp_update_from_product_edit_page', true);
		}

		/**
		 * Converts the given snake case string to the camel case string
		 * and returns the same
		 *
		 * @since 4.6.3
		 * @param string $name string in a camel case
		 * @return string
		 */
		public static function getCamelCaseOfDashCase( $name ) {
			if (!empty($name)) {
				$name = ucfirst($name);
		...
		/**
		 * This function will convert the time provided to the UTC time
		 * bu calculating the differance between the UTC time & local time
		 * 
		 * @since 4.6.3
		 * @param int $eventTime a non UTC timestamp to be converted to the UTC time
		 * @return int timestamp converted to UTC
		 */
		public function convertToUTC( $eventTime ) {
			$utc   = date_i18n( 'U', false, true );    
			$local = date_i18n( 'U' );
			$diff  = abs($local - $utc);
			if ($local>$utc) {
				$eventTime = $eventTime - $diff;	
			} else {
				$eventTime = $eventTime + $diff;
			}
		...
		/**
		 * This function will convert the time provided to the Local time
		 * by calculating the differance between the UTC time & local time
		 * 
		 * @since 4.6.3
		 * @param int $eventTime a UTC timestamp to be converted to the Local time
		 * @return int timestamp converted to Local time
		 */
		public function convertToLocalTime( $eventTime ) {
			$utc   = date_i18n( 'U', false, true );    
			$local = date_i18n( 'U' );
			$diff  = abs($local - $utc);
			if ($local>$utc) {
