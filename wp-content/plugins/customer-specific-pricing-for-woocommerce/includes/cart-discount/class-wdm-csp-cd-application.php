	...
	class WdmCSPCartDiscountApplication {
		...
		public function applyOfferIfApplicable() {
			...
			foreach ($items as $item) {
				...
				/**
				 * This filter will let the user enable including taxes in the cart total.
				 * 
				 * @param bool $includeTax.  include tax while computing the cart total
				 */
				if (apply_filters('wdm_csp_cd_include_tax_for_eligible_items', false)) {
					$item['subTotalTax'] = empty($item['subTotalTax'])?0:$item['subTotalTax'];
					$cartTotalEligibleForDiscount += $item['subTotalTax'];	
				}
			...
			foreach ($discountRules as $aRule) {
				if ($cartTotalEligibleForDiscount >= $aRule['min']) {
					if (!is_numeric($aRule['max']) || $aRule['max']>=$cartTotalEligibleForDiscount) {
						...
						/**
						 * Filter can be used to change the cart discount text.
						 * 
						 * @param string $text default 'Cart Discount'.
						 * @param string $cdOfferText text containing details of the order.
						 * @param array $aRule Rule which is used for applygin the cart discount.
						 */
		...
		public function showCongratulationsMessage() {
			...
			/**
			 * 
			 * This filter can be used to filter the Cart offer text html
			 * 
			 * @param string $offerHtml Existing Html content for the offer display.
			 * @param string $buttonText Text to be shown on the button.
			 * @param string $shopMoreLink The link to redirect user to shop more products(default: WC shop page link).  
			 */
			$offerHtml		= apply_filters('wdm_csp_filter_cd_congratulations_section', $offerHtml, $buttonText, $shopMoreLink);
			echo wp_kses_post($offerHtml);
			?>
		...
		public function showOfferMessage() {
			...
			$offerHtml      =   '<div class="woocommerce-message csp-cd-offer" role="notice">
								...
								$offerHtml		= apply_filters('wdm_csp_filter_cd_offer_section', $offerHtml, $buttonText, $shopMoreLink);
			echo wp_kses_post($offerHtml);
			?>
				<?php $this->printOfferModalContent(); ?>
			</div>
		...
		private function printOfferModalContent() {
			...
			<div class="csp-cd-modal-content">
				  <div>
				  <span class="csp-cd-modal-close" >&times;</span>
				  ...
				  <div class="csp-cd-offer-modal-content">
					...
					foreach ($this->subRules as $aSubRule) {
						...
						$max = is_numeric($aSubRule['max'])? esc_html__('to ', 'customer-specific-pricing-for-woocommerce') . wc_price($aSubRule['max']): esc_html__( 'and more', 'customer-specific-pricing-for-woocommerce');
					...
					echo '</ul>';
					/**
					 * This filter can be used to disable showing the maximum applicable discount amount & text in the offers modal
					 * 
					...
					<span>
						<?php 
						/**
						 * This hook executes after the grneration of the cart modal & can be used to 
						 * add custom notice/message/html in the cart discounts modal.
						 * 
						 * @since 4.3.0
						 */
						do_action('csp_add_custom_message_in_cart_discount_modal'); 
						?>
					</span>
				  </div>
				</div>
		...
		private function extractCartItemsDataForDiscountAnalysis( $itemsInACart) {
			...
			foreach ($itemsInACart as $key => $cartItem) {
				...
				$item['subTotalTax'] = $cartItem['line_subtotal_tax'];
				$items[]=$item;
			}
			return $items;
		}
		...
		private function getApplicableSubRules( $userType) {
			...
			/**
			 * This filter can be used to filter the cart discount rules applicable for the user conditionally.
			 * 
			 * @param array $rules array of applicable subrules.
			 * @param string $rulesType Type of the subrules.
			 */
			$rules = apply_filters('csp_filter_cart_discount_limits', $rules, 'csp_cd_' . $userType . '_subrules');
			$this->subRules = $rules;
			return $rules;
		}
		...
		private function getMaxDiscount( $userType) {
			...
			/**
			 * This filter can be used to filter the value of maximum allowed discount,
			 * ex. Convert value to different currency, change the value conditionally
			 * according to the user.
			 * 
			 * @param float $maxDiscount Maximum cart discount amount to be applicable for the user.
			 */
