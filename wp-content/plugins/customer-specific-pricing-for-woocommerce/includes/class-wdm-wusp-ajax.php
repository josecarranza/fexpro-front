		...
		public function getProductPriceMapping( $option_selected, $product_list, $selection_values, $query_log_details) {
			...
			foreach ($selection_values as $SingleUser => $SingleName) {
				...
				foreach ($product_list as $product_id => $product_name) {
					...
					$minQty = '<input type="number" min = "1" value="' . esc_attr($existing_qty) . '" placeholder="1" name="csp_qty_' . esc_attr($product_id) . '_' . esc_attr($userId) . '" id="csp_qty" class="csp_single_view_qty" data-oldval = "' . esc_attr($existing_qty) . '" step="1"/>';
