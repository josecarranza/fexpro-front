...
require_once CSP_PLUGIN_PATH . '/includes/rules/wdm-csp-rule-role.php';
		...
		*/
		public function adminScripts() {
			...
			if (in_array($screen->id, array('product', 'edit-product' ))) {
					...
					$wdm_roles_dropdown_v = csp_dropdown_roles();
		...
		public function cspDisplayUserRoleForVariableProduct( $variation_data, $variation) {
			...
			if (! empty($role_price_products)) {
				foreach ($role_price_products as $key => $rows) {
					add_filter('editable_roles', array($this,'wdmListAllRoles'), 99, 1);
					$wdm_roles_dropdown_v = csp_dropdown_roles($rows->role);
					?>
										...
										<tr>
											...
											<td><input type="text"  size="5" class ="<?php echo esc_attr($cspPercentClass); ?>" name="wdm_woo_variation_price_v[<?php echo esc_attr($variation_data[ 'variation_post_id' ]); ?>][]" value="<?php echo wp_kses(wc_format_localized_price(wc_format_decimal($rows->price, '', true)), array('span'=>array('class'=>true))); ?>"/></td>
		...
		*/
		private function addVariationRolePriceMappingInDb( $variable_csp_role, $variable_qty_field, $variable_price_field, $variable_price_type, $var_id) {
			...
			if (isset($variable_csp_role[$var_id]) && ! empty($variable_csp_role[$var_id])) {
				...
				/**
				 * This action gets triggerd on saving the role specific product pricing rules for product variations
				 * 
