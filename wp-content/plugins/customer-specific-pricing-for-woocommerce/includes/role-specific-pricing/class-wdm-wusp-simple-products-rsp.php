...
include_once(CSP_PLUGIN_PATH . '/includes/rules/wdm-csp-rule-role.php');
/**
	...
	class WdmWuspSimpleProductsRsp {
		...
		*/
		public function __construct() {
			...
			// License check
				add_action('wdm_csp_simple_rule_section', array($this, 'printRoleTab'), 11);
				//add_action('wdm_add_after_simple_csp', array( $this, '' ), 5);
		...
		*/
		...
		public function printRoleTab() {
			global $cspFunctions;
			if (!$cspFunctions->isRSPUpdateEnabledFromProductEditPage()) {
				return '';	
			}
			?>
		...
		/**
		* Includes the template for the Role Specific tab if it is allowed to
		* manage RSP rules from simple product edit page. 
		*
		...
		*/
		public function cspDisplayUserRole() {
			global $cspFunctions;
			if ($cspFunctions->isRSPUpdateEnabledFromProductEditPage()) {
				include CSP_PLUGIN_PATH . '/templates/print_role_specific_pricing_tab_content.php';	
			}
		}
		...
		*/
		public function addRolePriceMappingInDb( $product_id) {
			...
			if (isset($postArray[ 'wdm_woo_rolename' ]) && ! empty($postArray[ 'wdm_woo_rolename' ])) {
					...
					if (isset($wdm_woo_role_name)) {
						...
						if (! in_array($roleQtyPair, $temp_array_role_qty)) {
							...
							if (isset($postArray[ 'wdm_woo_role_price' ][ $index ]) && isset($postArray[ 'wdm_role_price_type' ][ $index ]) && isset($qty) && !( $qty <= 0 )) {
								$postArray[ 'wdm_woo_role_price' ][ $index ] = \str_replace($wcDecimalSeperator, '.', $postArray[ 'wdm_woo_role_price' ][ $index ]);
								$rulePrice = ''==$postArray[ 'wdm_woo_role_price' ][ $index ]?-1:(float) $postArray[ 'wdm_woo_role_price' ][ $index ];
								$pricing = wc_format_decimal($rulePrice);
								$price_type = $postArray[ 'wdm_role_price_type' ][ $index ];

								if (! empty($role_id) && 0<=(float) $pricing && ! empty($price_type)) {
							...
							if (!empty($pricing) && 0>(float) $pricing) {
								$wpdb->delete(
									$role_pricing_table,
									array(
									'role'       => $role_id,
				...
				/**
				 * This action gets triggerd on saving the role specific product pricing rules
				 * 
				 * @param string $ruleType Type of the rules being saved
				 * @param array $wdmSavedRules array of the saved rules.
				 */
		...
		public static function getQtyPricePairsOfProductForRole( $current_user_id, $product_id, $roleList = array()) {
			...
			$sqlPrepareArgs[]=$product_id;
			$rspRules			= $wpdb->get_results($wpdb->prepare('SELECT role, price, min_qty, flat_or_discount_price as price_type FROM ' . $wpdb->prefix . 'wusp_role_pricing_mapping WHERE role IN(' . implode(', ', array_fill(0, count($user_role), '%s')) . ') AND product_id=%d ORDER BY min_qty' , $sqlPrepareArgs));
			/**
			 * This filter can be used to filter the role specific pricing rule for the user on product, fetched from the database.
			 * 
			 * @param array $rspRules role specific pricing rules for the roles on the product.
			 * @param string $user_role WP user role slug.
		...
		public static function getAllRolePricesForSingleProduct( $product_id) {
			global $wpdb;
			/**
			 * This filter can be used if the same prices are applied for the translated products are required to fetch from base product.
			 * 
			 * @param int $product_id product Id of the product translation.
			 */
