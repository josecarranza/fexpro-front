...
include_once(CSP_PLUGIN_PATH . '/includes/rules/wdm-csp-rule-group.php');
	...
	class WdmWuspSimpleProductsGsp {
		...
		*/
		public function __construct() {
			...
			include_once(dirname(dirname(__FILE__)) . '/class-wdm-wusp-get-data.php');
				...
				add_action('wdm_csp_simple_rule_section', array($this, 'printGroupTabs'), 12);
		...
		public function printGroupTabs() {
			global $cspFunctions;
			if ($cspFunctions->isGSPUpdateEnabledFromProductEditPage()) {
		...
		public function groupSpecificPricingTabOptions() {
			global $cspFunctions;
			if ($cspFunctions->wdmIsActive('groups/groups.php') && $cspFunctions->isGSPUpdateEnabledFromProductEditPage()) {
				include CSP_PLUGIN_PATH . '/templates/print_group_specific_pricing_tab_content.php';
			}
		}
		...
		*/
		public function addGroupProductList( $product_id, $group_product_table, $post) {
			...
			if (isset($post[ 'wdm_woo_groupname' ]) && ! empty($post[ 'wdm_woo_group_qty' ])) {
				...
				/**
				 * This action gets triggerd on saving the group specific product pricing rules
				 * 
		...
		*/
		public function setGroupPricingPairs( $group_product_table, $product_id, $group_id, $index, $post) {
			...
			if (isset($post[ 'wdm_woo_group_price' ][ $index ]) && isset($post[ 'wdm_group_price_type' ][ $index ]) && isset($qty) && !( $qty <= 0 )) {
				$postArray[ 'wdm_woo_group_price' ][ $index ] = \str_replace($wcDecimalSeperator, '.', $postArray[ 'wdm_woo_group_price' ][ $index ]);
				$rulePrice = ''==$post[ 'wdm_woo_group_price' ][ $index ]?-1:(float) $post[ 'wdm_woo_group_price' ][ $index ];
		...
		*/
		public function insertGroupPricingPairs( $group_product_table, $pricing, $product_id, $group_id, $index, $qty, $post) {
			...
			if (! empty($group_id) && 0<=(float) $pricing && ! empty($price_type)) {
