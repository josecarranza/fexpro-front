							...
							if (! empty($group_price_result)) {
								foreach ($group_price_result as $key => $rows) {
									...
									<tr>
										...
										<td><input type="text"  size="5" name="wdm_woo_variation_group_price[<?php echo esc_attr($variation_data[ 'variation_post_id' ]); ?>][]" value="<?php echo esc_attr(wc_format_localized_price(wc_format_decimal($rows->price, '', true))); ?>" class="<?php echo esc_attr($cspPercentClass); ?>"/></td>
		...
		*/
		private function addVariationGroupPriceMappingInDb( $variable_csp_group, $variable_qty_field, $variable_price_field, $variable_price_type, $var_id) {
			...
			if (isset($variable_csp_group[$var_id]) && ! empty($variable_qty_field[$var_id])) {
				...
				/**
				 * This action gets triggerd on saving the group specific product pricing rules for variable products
				 * 
