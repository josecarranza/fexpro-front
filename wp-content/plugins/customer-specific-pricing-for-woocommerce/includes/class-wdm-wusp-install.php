	...
	*/
	class WdmWuspInstall {
		...
		public static function createTables() {
			...
			$global_discount_rule_table	 = $wpdb->prefix . 'wcsp_global_discount_rule_mapping';
			//Create CSP tables
			...
			//Global Discounts
			self::createGlobalDiscountsRuleTable($global_discount_rule_table, $collate);
		...
		*/
		...
		/**
		 * Creates a rule table to store the global discount rules
		 * 
		 * @since 4.5.0
		 * @param string $global_discount_rule_table - table name
		 * @param string $collate
		 * @return void
		 */
		protected static function createGlobalDiscountsRuleTable( $global_discount_rule_table, $collate) {
			global $wpdb;
			if (! $wpdb->get_var($wpdb->prepare('SHOW TABLES LIKE %s;', $global_discount_rule_table))) {
				$user_table_query = "
                CREATE TABLE IF NOT EXISTS {$global_discount_rule_table} (
                                    id bigint(20) PRIMARY KEY NOT NULL AUTO_INCREMENT,
