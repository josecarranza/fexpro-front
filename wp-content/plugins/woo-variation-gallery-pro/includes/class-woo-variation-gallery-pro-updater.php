<?php
	...
	if ( ! class_exists( 'Woo_Variation_Gallery_Pro_Updater' ) ):
		final class Woo_Variation_Gallery_Pro_Updater {
			...
			public function __construct( $plugin_file, $product_id, $license_key = '', $args = array() ) {
				...
				add_action( 'admin_action_wvg_check_update', array( $this, 'force_update_check' ) );
			...
			public function check_for_update_link( $links, $file ) {
				...
				if ( $file == $this->plugin_slug && current_user_can( 'update_plugins' ) ) {
					...
					$update_check_url = esc_url( add_query_arg( array( 'action' => 'wvg_check_update' ), admin_url( 'plugins.php' ) ) );
					
					$row_meta['wvg_check_update'] = '<a href="' . $update_check_url . '" title="' . esc_attr( esc_html__( 'Check Update', 'woo-variation-gallery-pro' ) ) . '">' . esc_html__( 'Check Update', 'woo-variation-gallery-pro' ) . '</a>';
