...
class WC_Widget_Layered_Nav_Filters extends WC_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if( has_filter('wi_widget_has_filters')){
			$has_custom_filters=apply_filters( 'wi_widget_has_filters',false);
		}
		//$has_custom_filters = has_filter('wi_widget_has_filters') ? apply_filters( 'wi_widget_has_filters'):null;

		$base_link          = $this->get_current_page_url();
	

		if ( 0 < count( $_chosen_attributes ) || 0 < $min_price || 0 < $max_price || ! empty( $rating_filter ) || $has_custom_filters ) {
			...
			if($has_custom_filters){
				if(has_filter('wi_widget_filters_selected')){
					echo apply_filters( 'wi_widget_filters_selected',$base_link,'');
				}
			}
