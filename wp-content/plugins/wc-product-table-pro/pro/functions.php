...
//-- update PRO
define( 'WCPT_PRO_URL', 'https://pro.wcproducttable.com' );
define( 'WCPT_PRO_ITEM_ID', 10 );

add_action('admin_init', 'wcpt_edd_update_pro');
function wcpt_edd_update_pro(){
  ...
  $edd_updater = new EDD_SL_Plugin_Updater(
    ...
    array(
  		'version' 	=> WCPT_VERSION, 	// current version number
  		'license' 	=> trim( get_option( 'wcpt_pro_license_key' ) ),
  		'item_id'   => WCPT_PRO_ITEM_ID, 	// id of this product in EDD
  		'author' 	  => 'Kartik Gahlaut',  // author of this plugin
  		'url'       => home_url(),
  	)
  );
}
...
//-- manage license
add_action( 'wp_ajax_wcpt_manage_license', 'wcpt_manage_license' );
function wcpt_manage_license(){
  if(
    empty( $_POST['wcpt_purpose'] ) ||
    empty( $_POST['wcpt_key'] ) ||
    empty( $_POST['wcpt_nonce'] ) ||
    ! wp_verify_nonce( $_POST['wcpt_nonce'], 'wcpt' )
  ){
    die();
  }

  $url = WCPT_PRO_URL . '/?edd_action='. $_POST['wcpt_purpose'] .'_license&item_id='. WCPT_PRO_ITEM_ID .'&license='. $_POST['wcpt_key'] .'&url='. home_url();
  ...
  if( $response['success'] ){
    if( $_POST['wcpt_purpose'] == 'activate' ){
      wcpt_set_license_status( 'active' );
    }

    if( $_POST['wcpt_purpose'] == 'deactivate' ){
      wcpt_set_license_status( 'inactive' );
    }

    // save license key
    wcpt_update_license_key($_POST['wcpt_key']);
    update_option( 'wcpt_pro_license_key', $_POST['wcpt_key'] );
  }


  echo $_POST['wcpt_purpose'] . 'd'; // activated / deactivated

  die();
}

//-- license status : active / inactive
function wcpt_get_license_status(){
  return get_option('wcpt_license_status');
}

function wcpt_set_license_status($status){
  update_option('wcpt_license_status', $status);
}

function wcpt_update_license_key($key){
  $global_settings = wcpt_get_settings_data();
  if( $global_settings ){
    $global_settings['pro_license']['key'] = $key;
  }
  update_option( 'wcpt_settings', addslashes( json_encode( $global_settings) ) );
}

//-- update license key when user changes it
add_filter('wcpt_global_settings', 'wcpt_license_key_update', 10, 1);
function wcpt_license_key_update($data){
  $_data = json_decode( stripslashes($data), true );
  if( ! empty( $_data['pro_license'] ) && ! empty( $_data['pro_license']['key'] ) ){
    update_option( 'wcpt_pro_license_key', $_data['pro_license']['key'] );
  }
  return $data;
}

// plugin activation meta link
add_filter('plugin_row_meta', 'wcpt_plugin_row_meta', 10, 2);
function wcpt_plugin_row_meta( $links, $file ){
  if( WCPT_PRO ){
    $text = wcpt_get_license_status() == 'active' ? esc_html__('Manage license', 'wc-product-table') : esc_html__('Activate license', 'wc-product-table');
    if ( $file == plugin_basename('wc-product-table-pro/main.php') ) {
  		$links[] = '<a href="' . get_admin_url() . 'edit.php?post_type=wc_product_table&page=wcpt-settings#pro_license">' . $text . '</a>';
  	}
  }
	return $links;
}
...
function wcpt_products__sort_by_taxonomy__insert_modified_results( $result ){
  ...
  $result->posts = array_slice( $sorted_ids, $offset, $posts_per_page );
...
function wcpt_required_filter_nav_missing( $query_args = array() ){
  ...
  if(
    ...
    $GLOBALS['wcpt_table_instance']->attribute_required_but_missing = $missing_attrs;
...
function wcpt_after_row_open__insert_child_row_toggle_heading(){
  ...
  ?>
  <th class="wcpt-child-row-toggle wcpt-child-row-toggle--closed wcpt-noselect">
    <?php  wcpt_icon('chevron-down', 'wcpt-child-row-toggle__control wcpt-child-row-toggle__control--open', wcpt_get_child_row_toggle_button_style()); ?>
    <?php  wcpt_icon('chevron-up', 'wcpt-child-row-toggle__control wcpt-child-row-toggle__control--close', wcpt_get_child_row_toggle_button_style()); ?>
  </th>
  <?php
...
function wcpt_wishlist( $atts ){
  ...
  if( ! empty( $icon_fill_color ) ) { // fill
    $default_style .= " fill: " . $icon_fill_color . "; ";
  }  
