...
class WiQuickEasyPurchase{
    ...
    public $only_cats     = array(4595,4104,1829);
    ...
    function shortcode_wi_quick_easy_purchase($atts=array()) {
        ...
        $products_type = $this->filter_product_type($is_presale);
    ...
    function filter_delivery_date($is_presale=false){
        ...
        if(!$is_presale){
            ...
            */
            ...
            $r=$this->db->get_results($sql);
            foreach($r as $t){
                $months[]=array("value"=>$t->slug,"text"=>$t->name,"checked"=>(in_array($t->slug,$current_filters["meta_delivery_date"])));
            }
        }
        return $months;
    }
    function filter_gender(){
        $gender=array(array("value"=>"mens","text"=>"MENS"),array("value"=>"womens","text"=>"WOMENS"),array("value"=>"kids","text"=>"KIDS"),array("value"=>"unisex","text"=>"UNISEX"));
    ...
    function filter_product_type($is_presale=false){
        $only_cats=$this->only_cats;
        if($is_presale){
            $only_cats = $this->cats_presale;
        }
    ...
    function products_get(){
        ...
        $ids=array();
        $total_rows=0;
        $posts=$this->main_products_get($total_rows);
    ...
    function main_products_get(&$total_rows=0,$paginate=true){
        ...
        if(isset($role_selected_data[$rol]) && $role_selected_data[$rol]["afpvu_enable_role"] =="yes" && is_array($role_selected_data[$rol]["afpvu_applied_products_role"]) && count($role_selected_data[$rol]["afpvu_applied_products_role"])>0 ){
    ...
    function filter_variations($loop,$filter_variation=array(),$extra_args=array(),&$total_rows=0){
        if ($loop->have_posts()) {
            ...
            $sql="SELECT p.post_parent, p.ID, p.post_title ".$extra_field." FROM wp_posts p ";
            ...
            if(isset($filter_variation["stock_min"]) && $filter_variation["stock_min"]>0){
                $sql.=" HAVING stock >= ".$filter_variation["stock_min"];
                if(isset($filter_variation["stock_max"]) && $filter_variation["stock_max"]>0){
                    $sql.=" AND stock <=".$filter_variation["stock_max"];
                }
            }

            //echo $sql;
            $r_total = $wpdb->get_results($sql);
            $total_rows = is_array($r_total)? count($r_total):0;
            
            unset($r_total);
            $start=($extra_args["pag"]-1)*$extra_args["per_pag"];
            
            if(!isset($extra_args["no_paginate"])){
                $sql.=" LIMIT ".$start.",".$extra_args["per_pag"];
            }
