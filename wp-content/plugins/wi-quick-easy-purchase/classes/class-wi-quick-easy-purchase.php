...
class WiQuickEasyPurchase{
    ...
    public $only_cats     = array(4104,8661,8657,9493); //array(4595,4104,1829,8661,8657,5826);
    ...
    public $cats_presale = array(6372,4104,7884,9939,11155);  //6372 kids-presale,7884 men,9939 women ,7912 women old
    public $is_presale=false;
    public $BASIC_CAT = "core";
    public $BASIC_CAT_ID = 6497;
    public function __construct() {
        ...
        add_shortcode('wi-lists',array($this,"shortcode_wi_lists"));
        ...
        add_action('wp_ajax_qep_list_save', array($this,"qep_list_save"));
        add_action('wp_ajax_nopriv_qep_list_save', array($this,"qep_list_save"));
        
        add_action('wp_ajax_qep_list_user', array($this,"qep_list_user"));
        add_action('wp_ajax_nopriv_qep_list_user', array($this,"qep_list_user"));

        add_action('wp_ajax_qep_list_info', array($this,"qep_list_info"));
        add_action('wp_ajax_nopriv_qep_list_info', array($this,"qep_list_info"));

        add_action('wp_ajax_qep_list_add_items', array($this,"qep_list_add_items"));
        add_action('wp_ajax_nopriv_qep_list_add_items', array($this,"qep_list_add_items"));
        
        add_action('wp_ajax_qep_list_get_all', array($this,"qep_list_get_all"));
        add_action('wp_ajax_nopriv_qep_list_get_all', array($this,"qep_list_get_all"));

        add_action('wp_ajax_qep_list_remove', array($this,"qep_list_remove"));
        add_action('wp_ajax_nopriv_qep_list_remove', array($this,"qep_list_remove"));

        add_action('wp_ajax_qep_list_items_remove', array($this,"qep_list_items_remove"));
        add_action('wp_ajax_nopriv_qep_list_items_remove', array($this,"qep_list_items_remove"));

        add_action('wp_ajax_qep_list_users_get', array($this,"qep_list_users_get"));
        add_action('wp_ajax_nopriv_qep_list_users_get', array($this,"qep_list_users_get"));

        add_action('wp_ajax_qep_search_users_get', array($this,"qep_search_users_get"));
        add_action('wp_ajax_nopriv_qep_search_users_get', array($this,"qep_search_users_get"));

        add_action('wp_ajax_qep_add_users_to_list', array($this,"qep_add_users_to_list"));
        add_action('wp_ajax_nopriv_qep_add_users_to_list', array($this,"qep_add_users_to_list"));

        add_action('wp_ajax_qep_remove_users_to_list', array($this,"qep_remove_users_to_list"));
        add_action('wp_ajax_nopriv_qep_remove_users_to_list', array($this,"qep_remove_users_to_list"));

        add_action('wp_ajax_qep_get_my_list', array($this,"qep_get_my_list"));
        add_action('wp_ajax_nopriv_qep_get_my_list', array($this,"qep_get_my_list"));
    ...
    function shortcode_wi_quick_easy_purchase($atts=array()) {
        ...
        $global_brands = $this->filter_global_brands();
        ...
        $current_filters["pa_global-brand"] = isset($_GET["pa_global-brand"])?explode(",",$_GET["pa_global-brand"]):array();
        $current_filters["pa_global-brand"] = array_map(function($item){ return addslashes($item);},$current_filters["pa_global-brand"]);


        $is_submit = isset($_GET["submit"])?1:0;

        wp_enqueue_style('wi-qep-styles', WI_PLUGIN_URL. 'assets/css/quick-easy-purchase-styles.css?v=2.5', array(),false, false);
        ob_start();
    ...
    function filter_collections($is_presale=false,$brands=[]){
        ...
        if($is_presale){
            ...
            $cats[]=$this->BASIC_CAT_ID;
        }
        $user_collections=$this->get_user_collections(1); // 1 = only presale
    ...
    function filter_gender($is_presale=false){
        $gender=array(array("value"=>"men","text"=>"MEN"),array("value"=>"women","text"=>"WOMEN"),array("value"=>"kids","text"=>"KIDS"),array("value"=>"teens","text"=>"TEENS")); //,array("value"=>"unisex","text"=>"UNISEX")
        if(!$is_presale){
            $gender[]=array("value"=>"girls","text"=>"GIRLS");
        }
        $current_filters = array();
        $current_filters["pa_gender"] = isset($_GET["pa_gender"])?explode(",",$_GET["pa_gender"]):array();
        $current_filters["pa_gender"] = array_map(function($item){ return addslashes($item);},$current_filters["pa_gender"]);
    ...
    function get_user_collections($only_presale=0){
        ...
        $sql = "SELECT DISTINCT a.id_collection from wi_collection_roles a 
        INNER JOIN wi_collection_config b ON a.id_collection=b.id
        WHERE a.role IN ('".implode("','",$user_config)."') ";
        if($only_presale==1){
            $sql.= " AND b.show_presale=1";
        ...
        if(is_array($r) && count($r)>0){
            ...
            if($only_presale == 1){
                $sql_all = "SELECT DISTINCT a.id_collection 
                from wi_collection_roles a 
                INNER JOIN wi_collection_config b ON a.id_collection=b.id 
                WHERE b.show_presale=1";
                $r = $wpdb->get_results($sql_all,ARRAY_A);
                $user_config = array_column($r,"id_collection");
            }
        }
    ...
    function products_get(){
        ...
        if ($posts) {
            ...
            foreach ($posts as $key => $var) {
                $sql_variacion="SELECT a.*,pp.post_name parent_slug, pp.post_excerpt post_excerpt_main, pm1.meta_value thumbnail_id, pm2.meta_value image ,pm3.meta_value miniaturas,pm4.meta_value delivery_date,
                (SELECT group_concat(concat(b.meta_key,'||',b.meta_value) SEPARATOR '///')  FROM wp_postmeta b WHERE b.post_id=a.ID ) metas,
                ( SELECT group_concat(concat(tt.taxonomy,'||',t.name,'||',t.slug) SEPARATOR '///')
                        FROM wp_term_relationships tr
                ...
                $main_product_atts = array_map(function($row){
                    $f = explode("||",$row);
                    return array("key"=>$f[0],"value"=>$f[1],"slug"=>isset($f[2])?$f[2]:null);
                },$main_product_atts);
                $cats = array_column(array_filter($main_product_atts,function($obj){ return $obj["key"]=="product_cat";}),"value");
                $players = array_column(array_filter($main_product_atts,function($obj){ return $obj["key"]=="pa_player";}),"value","slug");
                ...
                $color = preg_match('/Color:\s*([^,]+)/',$var_data->post_excerpt, $coincidencias) && isset($coincidencias[1]) ?trim($coincidencias[1]):"";

                //$color = preg_replace('/[0-9]+/', '',str_replace("Color: ","",$var_data->post_excerpt));
                $pa_color = $color;//strtolower(str_replace("Color: ","",$var_data->post_excerpt));
                ...
                if(isset($metas_variation["woo_variation_gallery_images"])){
                    ...
                    if(isset($_galle[0]) && is_numeric($_galle[0])){
                        $meta_galle = $this->db->get_row("SELECT meta_value FROM wp_postmeta pm3 WHERE pm3.meta_key = '_wp_attachment_metadata' AND pm3.post_id IN (".implode(",",$_galle).")");
                        if(isset($meta_galle->meta_value)){
                            ...
                            if(isset($_galle_images["file"])){
                                ...
                                $mini2 = $mini2!=""?(string)$url_image.$upload_path_galle."/".$mini2:"" ;
                            }
                        }
                    }
                }
                ...
                $team =  $item["team"] = isset($metas_variation["product_team"])?$metas_variation["product_team"]:"";
                $player = isset($metas_variation['attribute_pa_player']) && isset($players[$metas_variation['attribute_pa_player']])?$players[$metas_variation['attribute_pa_player']]:"";
                $item=array();
                ...
                $item['product_title2'] =  $var_data->post_excerpt_main. " - " . $color.", ".$team.($player!=""?", $player":"");
    ...
    function filter_global_brands(){
        ...
        foreach ($r as $value) {
            $item=array();
            $item["text"] = $value->name;
            $item["value"] = $value->slug;
            $item["checked"] = in_array($item["value"],$current_filters["pa_global-brand"]);
            $global_brands[]=$item;
        }
    ...
    function main_products_get(&$total_rows=0,$paginate=true,$_per_page=100,&$all_ids=null){
        ...
        $pag = isset($_GET["pag"])?(int)$_GET["pag"]:1;
        $global_brand = isset($_GET["pa_global-brand"]) ? trim($_GET["pa_global-brand"]):"";
        ...
        if($is_presale){
            ...
            if($division !=""){
                ...
                //all divisions, el resto de filtros de categoria limitará el resultado
                $sql="SELECT  t3.* 
                FROM  wp_terms t 
                ...
                WHERE  t3.slug REGEXP '".(str_replace(",","|",$division))."' ";
                // WHERE t.slug IN ('".implode("','",$slug_category)."') AND t3.slug REGEXP '".(str_replace(",","|",$division))."'
        ...
        //si es preventa, se incluyen los tipo core tambien
        if($is_presale){
            $slug_category[]=$this->BASIC_CAT;
        }
        ...
        if ($global_brand!="") {
            $filter_global_brand = explode(',', $global_brand);
            $args['tax_query'][] = array(
                'taxonomy'        => 'pa_global-brand',
                'field'           => 'slug',
                'terms'           =>  $filter_global_brand,
                'operator'        => 'IN',
            );
        }
        ...
        if(isset($role_selected_data[$rol]) && isset($role_selected_data[$rol]["afpvu_enable_role"]) && $role_selected_data[$rol]["afpvu_enable_role"] =="yes" && isset($role_selected_data[$rol]["afpvu_applied_products_role"]) && is_array($role_selected_data[$rol]["afpvu_applied_products_role"]) && count($role_selected_data[$rol]["afpvu_applied_products_role"])>0 ){
    ...
    function filter_variations($loop,$filter_variation=array(),$extra_args=array(),&$total_rows=0){
        if ($loop->have_posts()) {
            ...
            $pos_all = array_search("all",$delivery_dates);
       
            if($pos_all){
                $delivery_dates=array();//si solo viene ALL,reset de filtro para que no se incluya en los where 
                /*if(count($delivery_dates)==1){
                  
                }else{
                    unset($delivery_dates[$pos_all]);
            ...
            if($only_stock_present || count($delivery_dates)>0):
                ...
                $sql.=" ) ";
                if(count($delivery_dates)>0 && !$only_stock_present && !$this->is_presale):
                    $sql.=" INNER JOIN wp_postmeta pms ON p.ID = pms.post_id AND  (pms.meta_key = '_stock_future' AND CAST(pms.meta_value AS SIGNED) > 0)  ";
                endif;
                if(count($delivery_dates)>0 && $only_stock_present && !$this->is_presale):
                    $sql.=" INNER JOIN wp_postmeta pms ON p.ID = pms.post_id AND ( (pms.meta_key = '_stock_present' AND CAST(pms.meta_value AS SIGNED) > 0) OR (pms.meta_key = '_stock_future' AND CAST(pms.meta_value AS SIGNED) > 0) OR (pms.meta_key = '_stock_present_china' AND CAST(pms.meta_value AS SIGNED) > 0) )  ";
                endif;
            ...
            //echo $sql;
            $r_total = $wpdb->get_row("select COUNT(distinct p.ID) total, GROUP_CONCAT(distinct p.ID) ids ".$sql);
    ...
    function export_xlsx(){
        ...
        if(isset($post["download_list"])){
            $list = $this->get_list_info_by_id((int)$post["download_list"]);
            if($list && count($list->items)>0){
                $ids=$list->items;
                $is_presale = $list->category=="PRESALE"?1:0;
            }
        }
        ...
        if ($ids) {
            // query to sort by pa_brand
            $sql_sort = "SELECT p.ID, t.name, t.slug, p.post_title
            from wp_posts p
            inner join wp_posts pp on p.post_parent=pp.ID
            inner join wp_term_relationships tr on pp.ID = tr.object_id
            inner join wp_term_taxonomy tt on tr.term_taxonomy_id = tt.term_taxonomy_id and tt.taxonomy='pa_brand'
            inner join wp_terms t on tt.term_id =t.term_id
            ...
            ";
            // echo $sql_sort;
            $query_sort = $this->db->get_results($sql_sort,ARRAY_A);
            $ids = array_column($query_sort,'ID');
            ...
            foreach ($ids as $key => $variation_id) {
                    ...
                    if(isset($_galle[0]) && is_numeric($_galle[0])){
                        ...
                        if(isset($meta_galle->meta_value)){
                            ...
                            if(isset($_galle_images["file"])){
                                ...
                                $upload_path_galle=implode("/",$upload_path_galle);
                                $mini2 = isset($_galle_images["sizes"]["thumbnail"])?$_galle_images["sizes"]["thumbnail"]["file"]:(isset($_galle_images["sizes"]["medium"])?$_galle_images["sizes"]["medium"]["file"]:"");
                                $mini2 = $mini2!=""?(string)$upload_path_galle."/".$mini2:"" ;
                            }
                        }
                    }
                }
                ...
                //$item["main_id"] = $var->post_parent;
                $stock_present =  isset($metas_variation["_stock_present"]) ? (int)$metas_variation["_stock_present"]:0;
                $stock_present_china =  isset($metas_variation["_stock_present_china"]) ? (int)$metas_variation["_stock_present_china"]:0;
                $stock_future =  isset($metas_variation["_stock_future"]) ? (int)$metas_variation["_stock_future"]:0;
                

                $item["image"] = [$mini!=""?(string)$upload_path."/".$mini:"",$mini2];
                

                $item['sku'] =  isset($metas_variation["_sku"])?$metas_variation["_sku"]:"";

                //$item['product_title'] =  $var_data->post_title;// . " - " . $color ;
                $item['product_title'] =  $var_data->post_excerpt_main!='' ? $var_data->post_excerpt_main. " - " . $color.", ".$team.($player!=""?", $player":"") :  $var_data->post_title;// . " - " . $color ;
                ...
                $item["delivery_date"] = ($stock_present>0 || $stock_present_china > 0)?"IMMEDIATE":$delivery ;
                
                $item["available"] = $is_presale==0?($units_per_pack * ($stock_present>0?$stock_present:($stock_present_china > 0 ? $stock_present_china : $stock_future))):"";

                //$item['Division'] =  $division; 
                // warehouse
                $item['Division'] = $is_presale==0 ? ($stock_present > 0 ? "Panamá": ($stock_present_china > 0 ? "China" : "Future")) : '';

                $item['brand'] =  isset($main_atts["pa_brand"])?$main_atts["pa_brand"]:""; 
                $item['gender'] =  isset($main_atts["pa_gender"])?$main_atts["pa_gender"]:"";

                //$item['group'] = $group;
                $item['group'] = isset($main_atts["pa_collection"])?$main_atts["pa_collection"]:"";
                $item['product'] = $prod_type;
                $item['size_chart'] = implode("-",$sizes);
                ...
                $item["logo_application"] = isset($metas_variation["logo_application"])?$metas_variation["logo_application"]:"";
                $item["composition"] = isset($metas_variation["pa_compositions"])?$metas_variation["pa_compositions"]:"";
                $item["global_brand"] = isset($main_atts["pa_global-brand"])?$main_atts["pa_global-brand"]:"";
                // $item["players"] = $players;
                ...
                if($is_presale==0 && ($stock_present>0 && $stock_present_china>0)){
                    $item["delivery_date"] = "IMMEDIATE";
                    $item['Division'] = "China";
                    $item["available"] = $units_per_pack * $stock_present_china;
                    $products[] = $item;
                }

                if($is_presale==0 && (($stock_present>0 || $stock_present_china > 0 ) && $stock_future>0)){
                    $item["delivery_date"] = $delivery;
                    $item["available"] = $units_per_pack * $stock_future;
                    $item['Division'] = "";
                    $products[] = $item;
    ...
    function build_excel_catalog($data,$is_presale=0,$is_export_cart=0){
        error_reporting(0);
        ...
        $dataMainHeader = array('Image','SKU', 'Name', 'Price','Color', 'Units per Pack', 'Delivery','Available', 'Warehouse', 'Brand','Gender','Collection','Product','Size Chart','Qty','Total Units','Total Price (USD)','Team','Player','Logo application','Composition','Global brand');
          ...
          $objPHPExcel->getActiveSheet()->getStyle('A1:T1')->applyFromArray( //Q1
            ...
            while( $rowCount < count($data) ){ 
                ...
                foreach( $data[$rowCount] as $key => $value ) {
                    ...
                    switch ($key) {
                        case 'image':
                            $file = is_array($value)?$upload_path.$value[0]:$upload_path.$value;
                            if (file_exists($file)) {
                            ...
                            $file2 = is_array($value) && isset($value[1])?$upload_path.$value[1]:"";
                            if ($file2!="" && file_exists($file2)) {
                                $objDrawing = new PHPExcel_Worksheet_Drawing();
                                $objDrawing->setPath($file2);
    
                                $objDrawing->setOffsetX(68);                     //setOffsetX works properly
    ...
    function qep_add_to_cart(){
        ...
        $quantity_china = empty($_POST['quantity_china']) ? 0 : wc_stock_amount($_POST['quantity_china']);
        ...
        $type_in_cart = $this->get_first_item_type();

        $type_in_cart_options=[
            "available"       => "Inventory",
            "available_china" => "Inventory",
            "future"          => "Future Inventory",
            "presale"         => "Pre-sale"
        ];
        $type_in_cart_str = $type_in_cart_options[$type_in_cart] ?? "";
        ...
        if($quantity>0){
            ...
            if($type_in_cart!="" && ($type_in_cart != "available" && $type_in_cart != "available_china")){
                $pass_available = false;
                $message="Only {$type_in_cart_str} products are allowed in the order";
            }
        ...
        if($quantity_future>0){
            ...
            $pass_future = $this->validate_stock_add_to_cart($product_id,$variation_id,"future",$quantity_future,$message,$is_presale);

            if($type_in_cart!="" && $type_in_cart != "future" && !$is_presale){
                $pass_future = false;
                $message="Only {$type_in_cart_str} products are allowed in the order";
            }
            if($type_in_cart!="" && $type_in_cart != "presale" && $is_presale){
                $pass_future = false;
                $message="Only {$type_in_cart_str} products are allowed in the order";
            }
        ...
        if($quantity_china>0){
            $message="";
            $pass_available_china = $this->validate_stock_add_to_cart($product_id,$variation_id,"available_china",$quantity_china,$message);
        ...
        if($quantity_china>0 && $pass_available_china){
            $data_extra=array();
            $data_extra["type_stock"] = "available_china";
          
            WC()->cart->add_to_cart($product_id,$quantity_china,$variation_id,null,$data_extra);
            $added_once=true;
    ...
    function validate_stock_add_to_cart($product_id,$variation_id,$type_stock="available",$qty=1,&$message="",$is_presale=false){
        ...
        /*if( $type_stock=="future" && in_array($this->BASIC_CAT,$cats)){
            return true;
        }*/
        ...
        $stock_available_china = isset($product_metas["_stock_present_china"][0])?(int)$product_metas["_stock_present_china"][0]: 0;

        $_stock = $type_stock=="available"? $stock_available: ($type_stock=="available_china"?$stock_available_china:$stock_future);
        if($_stock==0){
        ...
        foreach ($items as $key => $item_cart) {
            ...
            if($item_cart["product_id"]==$product_id && $item_cart["variation_id"]==$variation_id && $item_type_stock == $type_stock && (!isset($item_cart["is_presale"]) || $item_cart["is_presale"]=="0")){
       ...
       /* echo "<pre>";
        ...
        if($cart!=null){
            ...
            foreach ($cart as $key => $item) {
                ...
                $quantity_china = isset($item["qty_china"])? (int)$item["qty_china"] : 0;
                ...
                if($added_once){
                    ...
                    $json["error"] = 1;
                    $json["message"] = "No products added";
                }
            }
        }else{
            $json["error"] = 1;
    ...
    function get_slugs_product_type($types,$is_presale=false){
        ...
        if($is_presale){
            //$only_cats = $this->cats_presale;
            // si es preventa se unen con las de stock por los productos tipo CORE, al final solo se filtraran las de CORE en el filtro de la categoria principal
            $only_cats = array_merge($this->only_cats,$this->cats_presale) ;
        }
        $types = explode(",",$types);
    ...
    function qep_export_cart(){
        ...
        $discounts = discount_by_rol_margin($u->ID);
            ...
            foreach ($items as $key => $var) {
                ...
                //$item['product_title'] =  $var_data->post_title;// . " - " . $color ;
                $item['product_title'] =  $var_data->post_excerpt_main!='' ? $var_data->post_excerpt_main. " - " . $color.", ".$team :  $var_data->post_title;// . " - " . $color ;
        ...
        //die();
        ob_clean();
        $file=$this->build_excel_catalog( $products,0,1);
        $json["error"] = 0;
        $json["download"] = get_site_url()."/wp-admin/admin-ajax.php?action=download_xlsx&file=".$file;
        echo json_encode($json);
        die();
    ...
    function qep_view_cart(){
        ...
        $type = $this->get_first_item_type();
        //echo $type;

        $order = new WC_Order( 181666);
       ...
       /*  $order_items = array_map(function($row){

            $row = $row->get_data();
            $row['metas'] = array_map(function($item){ return $item->get_data();},$row['meta_data']);
            return $row;
    ...
    function qep_list_save(){
        error_reporting(E_ALL);
        $post = json_decode(file_get_contents("php://input"));

        $user=wp_get_current_user();
        if(!isset($user->roles) || !in_array("administrator",$user->roles)){
            echo wp_send_json(["error"=>1,"message"=>"Not authorized"]);
            wp_die();
            return false;
        }
        $list["id_user"] = $user->ID;
        $list["id_list"] = $post->id_list??0;
        $list["name"] = addslashes(trim($post->name??""));
        $list["category"] = $post->category??"PRESALE";
        $list["image"] = $post->image??"";
        $list["status"] = $post->status??0;
        if($list["id_list"]==0){
            $list["creation_date"] = date("Y-m-d H:i:s");
        }

        $id_list = $this->list_save($list);
    ...
    function qep_list_user(){
        ...
        $sql="select * from wi_list a WHERE a.status>-1 AND a.id_user=".$user->ID." ORDER BY a.name ASC";
        $r=$this->db->get_results($sql);
    ...
    function qep_list_info(){
        error_reporting(E_ALL);
    

        $user=wp_get_current_user();
        $id_list = (int)$_GET["id_list"]??0;
        $sql="select * from wi_list a WHERE a.status>-1 AND a.id_user=".$user->ID." AND a.id_list=".$id_list;
    ...
    private function get_list_info_by_id($id_list){
        error_reporting(E_ALL);
    
 
        $sql="select * from wi_list a WHERE a.status=1 AND a.id_list=".$id_list;
        $r=$this->db->get_row($sql);
        if(isset($r->id_list)){
            $items = $this->db->get_results("select id_item from wi_list_item a WHERE a.id_list=".$r->id_list,ARRAY_A);
            $items = array_column($items,"id_item");
            $r->items = $items;
        }
        return $r;
    }
    
    function qep_list_add_items(){
        error_reporting(E_ALL);
        $post = json_decode(file_get_contents("php://input"));
        $id_products = $post->id_product??[];
        $user=wp_get_current_user();
        $id_list = (int)$_GET["id_list"]??0;
        ...
        $sql="select * from wi_list a WHERE a.status>-1 AND a.id_user=".$user->ID." AND a.id_list=".$id_list;
        $r=$this->db->get_row($sql);
        if(isset($r->id_list) && count($id_products)>0){
            $current_ids = $this->db->get_results("select id_item from wi_list_item WHERE id_list=".$id_list,ARRAY_A);
            $current_ids=array_column($current_ids,"id_item");

            $nuevos = array_diff($id_products, $current_ids);
            foreach($nuevos as $id){
                $this->db->insert("wi_list_item",["id_item"=>$id,"id_list"=>$id_list]);
            }
        }
    ...
    private function list_save($list){
        $id_list=0;
        if($list["id_list"]==0){
            $this->db->insert("wi_list",$list);
            $id_list = $this->db->insert_id;
        }else{
            $this->db->update("wi_list",$list,["id_list"=>$list["id_list"]]);
    ...
    function shortcode_wi_lists(){

        wp_enqueue_style('wi-qep-styles', WI_PLUGIN_URL. 'assets/css/quick-easy-purchase-styles.css?v=2.5', array(),false, false);
        ob_start();
        include(WI_PLUGIN_PATH."/shortcodes/view-lists.php");
        $html = ob_get_clean();
        return $html;
  
    }

    function qep_list_get_all(){
 
        $user=wp_get_current_user();
        $isAdmin =  isset($user->roles) && in_array("administrator",$user->roles);
        

        $sql="SELECT a.id_list,a.category, a.name, a.image, b.display_name
        FROM wi_list a
        LEFT JOIN wp_users b ON a.id_user=b.ID
        WHERE a.`status`=1 ";
        if(!$isAdmin){
            $sql.= " AND a.id_list IN (select id_list from wi_list_user WHERE id_user=".$user->ID.") "; 
        }
        $sql.="
    ...
    function qep_list_items_remove(){
        $post = json_decode(file_get_contents("php://input"));
        ...
        $id_list = $post->id_list??0;
        $items = $post->items ?? [];

        if(count($items)> 0){
            $sql="DELETE from wi_list_item WHERE id_list=".$id_list." AND id_item IN (".implode(',',$items).") ";
            $r=$this->db->query($sql);
        }
    ...
    function qep_search_users_get(){
        ...
        $filter = addslashes(trim($post->filter ?? ''));

        $roles = $this->db->get_row("SELECT option_value FROM wp_options WHERE option_name = 'wp_user_roles'");
        $_roles = unserialize($roles->option_value);
        $roles = [];
        foreach ($_roles as $key => $item) {
            $roles[] = ["rol"=>$key,"name"=>$item["name"]];
        }
      
        $roles = array_column($roles,'name','rol');
       
       
       

        $sql = "SELECT a.ID,a.user_email,a.display_name,b.meta_value roles FROM wp_users a 
        INNER JOIN wp_usermeta b ON a.ID=b.user_id AND b.meta_key='wp_capabilities'
        WHERE a.user_status=0 AND (a.user_email like '%".$filter."%' OR a.display_name like '%".$filter."%' )
        ORDER BY a.display_name asc ";
        $r = $this->db->get_results($sql);

        $r = array_map(function ($item) use ($roles){
            $_rol = unserialize($item->roles);
            unset($item->roles);
            $item->rol = isset($roles[array_key_first($_rol)]) ?$roles[array_key_first($_rol)] : "";
    ...
    function qep_list_users_get(){
        ...
        $sql = "SELECT a.ID,a.user_email,a.display_name,b.meta_value roles FROM wp_users a 
        INNER JOIN wp_usermeta b ON a.ID=b.user_id AND b.meta_key='wp_capabilities'
        INNER JOIN wi_list_user c ON a.ID=c.id_user AND c.id_list = ".$id_list."
    ...
    function qep_add_users_to_list(){
        $post = json_decode(file_get_contents("php://input"));
        ...
        $existe = $this->db->get_row("select * from wi_list_user a WHERE a.id_user=".$id_user." AND a.id_list=".$id_list);
        if(!isset($existe->id_user)){
            $item = [];
            $item["id_user"] = $id_user;
            $item["id_list"] = $id_list;
            $item["creation_date"] = date("Y-m-d H:i:s");

            $this->db->insert("wi_list_user",$item);
        }
    ...
    function qep_remove_users_to_list(){
        $post = json_decode(file_get_contents("php://input"));
        ...
        if(count($id_users)>0){
            $this->db->query("delete from wi_list_user WHERE id_list = ".$id_list." AND id_user IN (".implode(',',$id_users).")");
        }
    ...
    function qep_get_my_list(){
       
        $user=wp_get_current_user();
         
        $r = $this->db->get_row("select count(a.id_list) cant from wi_list_user a inner join wi_list b on a.id_list=b.id_list AND b.status=1 WHERE a.id_user=".$user->ID." AND a.read=0");
        //echo $this->db->last_query;
        $json["new_list"] = (int)$r->cant;
        $json["error"] = 0;
        echo wp_send_json($json);
        wp_die();
    }
    function get_first_item_type(){
        $items= WC()->cart->get_cart();
        $items_array = (array)$items;
        if(count($items_array)> 0 ){
            $first = current((array)$items);
            $type = $first["type_stock"] ?? "available";
            if($type == "future" && $first["is_presale"] == 1){
                $type = "presale";
            }
            return $type;
