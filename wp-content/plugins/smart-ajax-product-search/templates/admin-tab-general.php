<div id="general_tab" class="ymapp-settings__content">
	<table class="form-table">
		...
		ysm_setting( $w_id, 'product_variation_visibility', array(
			...
			'choices' => array(
				'all' => __('Parent variable product with variations', 'smart_search'),
				'parent' => __('Only parent variable product', 'smart_search'),
				'variations' => __('Only variations', 'smart_search'),
			),
		));
		...
		ysm_setting( $w_id, 'select_limit', array(
			...
			'description' => __('Set number of posts that should be retrieved from the database. This is advanced option, use it only if you know what you do.', 'smart_search'),
			'value' => '',
		));
		...
		ysm_setting( $w_id, 'search_sku_first', array(
			...
			'description' => __('Search by SKU first if this option checked and if request is numeric', 'smart_search'),
			'value' => 0,
		));
