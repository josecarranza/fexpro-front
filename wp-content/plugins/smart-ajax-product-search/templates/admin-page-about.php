...
if ( $p_code ) {
	update_option( 'saps_p_code', sanitize_text_field( $p_code ) );
	if ( $p_code ) {
		ysm_add_message( __( 'Your plugin have been activated.', 'smart_search' ) );
	} else {
		ysm_add_message( __( 'Please activate the plugin.', 'smart_search' ) );
	}
}
...
if ( $is_patch ) {
	...
	if ( $patch_key ) {
		...
		update_option( 'ysm_patch_' . $patch_key, true );
		ysm_add_message( sprintf( __( 'Patch %s applied', 'smart_search' ), $patch_key ) );
	}
}
?>
