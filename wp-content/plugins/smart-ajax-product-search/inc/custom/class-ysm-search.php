...
class Ysm_Search {
	...
	public static function search_posts() {
		...
		if ( self::get_var( 'search_sku_first' ) && is_numeric( self::get_var( 's' ) ) && ! empty( $postmeta['_sku'] ) && -1 !== $limit ) {
			...
			$res_posts = array();
			$key = md5( self::get_var( 's' ) ) . '_sku_' . self::get_widget_id() . '_' . self::get_var( 'lang' );
			...
			if ( ! $cached ) {
				...
				\YSM\Cache\set_query_cache( $key, $res_posts );
			} else {
				if ( is_array( $cached ) ) {
					$res_posts = $cached;
				}
		...
		if ( -1 === $limit || count( $posts ) < $limit ) {
			$res_posts = array();
			$key = md5( self::get_var( 's' ) ) . '_posts_' . self::get_widget_id() . '_' . self::get_var( 'lang' );
			$cached = \YSM\Cache\get_query_cache( $key );
			if ( ! $cached ) {
				Ysm_DB::init( array(
					'posts_per_page' => $limit,
		...
		if ( $postmeta && ( -1 === $limit || count( $posts ) < $limit ) ) {
			...
			$res_posts = array();
			$key = md5( self::get_var( 's' ) ) . '_postmeta_' . self::get_widget_id() . '_' . self::get_var( 'lang' );
			$cached = \YSM\Cache\get_query_cache( $key );
			if ( ! $cached ) {
				Ysm_DB::init( array(
					'posts_per_page' => $limit,
...
//if(in_array($post->ID,$ak[$_SESSION['abc']]['afpvu_applied_products_role']) || in_array($product->id,$ak[$_SESSION['abc']]['afpvu_applied_products_role']) || ($_SESSION['abc'] == 'custom_role_mexico1' &&  ($abc[0]->slug == 'nfl' || $abcd[0]->slug == 'nfl')))

		foreach ( $posts as $post ) {
			$post = get_post( $post );
			$variation = wc_get_product($post->ID);
			$product = wc_get_product( $variation->get_parent_id() );
			$abc = get_the_terms( $product->id , 'pa_brand');
			$abcd = get_the_terms( $post->ID , 'pa_brand');
			
			if(in_array($post->ID,$ak[$_SESSION['abc']]['afpvu_applied_products_role']) || in_array($product->id,$ak[$_SESSION['abc']]['afpvu_applied_products_role']) || ($_SESSION['abc'] == 'custom_role_mexico1' &&  ($abc[0]->slug == 'nfl' || $abcd[0]->slug == 'nfl')) || ($_SESSION['abc'] == 'custom_role_mexico2' &&  ($abc[0]->slug == 'nfl' || $abcd[0]->slug == 'nfl')))
			{
				continue;
			}
			else
			...
			if ( $wc_product ) {
				if ( self::get_var( 'display_sale_label' ) && $wc_product->is_on_sale() ) {
					$output .= '<span class="smart-search-sale-label">' . esc_html__('Sale', 'smart_search') . '</span>';
					$is_on_sale = true;
					$post_classes[] = 'smart-search-on_sale';
				}

				if ( self::get_var( 'display_featured_label' ) && $wc_product->is_featured() ) {
					$output .= '<span class="smart-search-featured-label">' . esc_html__('Featured', 'smart_search') . '</span>';
					$is_featured = true;
					$post_classes[] = 'smart-search-featured';
				}

				if ( self::get_var( 'display_out_of_stock_label' ) && ! $wc_product->is_in_stock() ) {
					$out_of_stock_html = '<span class="smart-search-out-of-stock-label">' . esc_html__('Out of stock', 'smart_search') . '</span>';
			...
			if ( $wc_product && $wc_product->is_in_stock() && $wc_product->is_type( 'simple' ) && $wc_product->get_price() && self::get_var( 'display_add_to_cart' ) ) {
				$is_add_to_cart = true;
	...
	public static function output() {
		...
		if ( self::get_var( 'display_view_all_link' ) || self::get_var( 'view_all_link_text' ) ) {
			...
			if ( $view_all_link_text ) {
				if ( self::get_var( 'view_all_link_found_posts' ) ) {
					$view_all_link_text = sprintf( esc_html__( $view_all_link_text, 'smart_search' ), self::$found_posts );
				} else {
					$view_all_link_text = __( $view_all_link_text, 'smart_search' );
				}
