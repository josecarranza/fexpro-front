...
<h2>Fexpro POP - Place Factory Order</h2>
...
<table class="table table-bordered" id="demo">
   <thead>
	...
	<tbody id="myTable">
		...
		foreach($merge as $key => $value)
		{
			$checkdataExist =  $wpdb->get_var("SELECT COUNT(vid) FROM {$wpdb->prefix}pop_factory_order_confirmation_list WHERE `vid`= '$key'");
			$getQtyRemaining = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}pop_factory_order_confirmation_list WHERE vid = $key" );
  ...
  jQuery(".fltrow th [ct='"+index+"']").parent().hide();
	...
	jQuery(".single-submit-it").on('click', function() {
		...
		else
		{
			...
			form_data.append('action', 'adding_pop_factory_data');
	...
	$("a.submit-in-one").on('click', function() {
		...
		form_data.append('action', 'adding_pop_factory_data_push');
