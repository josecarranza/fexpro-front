...
foreach($orders as $order_id)
{
	...
	$a = array();
	   ...
	   $variation_id = $item->get_variation_id();
	    ...
	    {
			if( has_term( array( 'youth'), 'product_cat' ,  $product_id) ) 
			{
...
foreach($final_result1 as $key => $value)
{
  foreach($value as $ak)
  {
	...
	$ur = implode(', ', $user_info->roles);
	if(!in_array($ur, $return_array1))
	{
		array_push($return_array1, $ur);
	}
	
	if(!in_array($user_meta, $return_array5))
	{
		array_push($return_array5, $user_meta);
	}
  }
}
...
<h2>Headwear Caps Purchase List</h2>
   ...
   <thead>
        <tr>
		 ...
		 global $wp_roles;
		  ...
		  {
			  echo "<th style='vertical-align : middle;text-align:center;'>" . $wp_roles->roles[$value1]['name'] . "</th>";			 
		  }
		  ?>
		  <?php
		  $countries = WC()->countries->get_countries();
		  //print_r($countries);
		  foreach($return_array5 as $key1 => $value1)
		  {
			  //echo $value1 . "<br>";
		...
		<?php 
			...
			foreach($merge as $key => $value)
			{
				...
				echo "<tr>";
					...
					echo "<td class='".$_product->get_sku()."'>" . get_post_meta($key, 'product_team', true) . "</td>";
					foreach($return_array1 as $key1 => $value1)
					{
						if($value1 == 'custom_role')
						{
							$stringdelivery_date = $main_product->get_attribute( 'pa_paris-delivery-date' );
						}
						else if($value1 == 'custom_role_fexpro_pop_dominican')
						{
							$stringdelivery_date = $main_product->get_attribute( 'pa_delivery-date' );
					...
					echo "<td class='".$_product->get_sku()."'>" . strtoupper($stringdelivery_date) . "</td>";
					}
					$apk = 0;
					foreach($return_array5 as $key1 => $value1)
					{
