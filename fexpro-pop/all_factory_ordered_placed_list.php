...
$getallOrdersList = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}pop_factory_order_confirmation_list", ARRAY_A );
...
<h2>Fexpro POP All Order Lists</h2>
  ...
  jQuery(".fltrow th [ct='"+index+"']").parent().hide();
	...
	jQuery(".single-submit-it").on('click', function() {
		...
		{ */
			...
			form_data.append('action', 'edit_pop_factory_data');
	...
	jQuery(".single-delete-it").on('click', function() {
		...
		form_data.append('getCurrentRowVariationID', getCurrentRowVariationID);	
		form_data.append('action', 'delete_pop_single_factory_data');	
