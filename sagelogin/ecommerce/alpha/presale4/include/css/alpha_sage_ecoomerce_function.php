...
function alpha_factory_order_list_datatables() {
	...
	$get_total_records = $wpdb->get_results("SELECT `id` from  alpha_wp_ss22_factory_order_confirmation_list Order by id asc ");
	$totalData = count($get_total_records);
	if($request['length'] == -1){
		$request['length'] = $totalData;
	}

	$get_variations_arr = $wpdb->get_results("SELECT * from  alpha_wp_ss22_factory_order_confirmation_list Order by id ".$request['order'][0]['dir']." LIMIT ".$request['length']." OFFSET ".$request['start']."  ");
...
function alpha_save_ss22_factory_data_order_number(){
   ...
   if($_REQUEST['action'] == 'alpha_save_ss22_factory_data_order_number'){
      ...
      if(!empty($orderNumberText)){
         $wpdb->update( 
            "alpha_wp_ss22_factory_order_confirmation_list", 
            array( 
  ...
  header("Content-Type: application/json");
    ...
    foreach($get_variations_arr as $key => $data_record){
		...
		if($getQtyRemaining->vid == $variation_id)
		{
			...
			$nestedData['total_purchased_unit' ] =  $apk;
			$nestedData['open_stock' ] =  $e;
			$nestedData['total_amount' ] =  wc_price(get_post_meta($variation_id, '_regular_price', true) * $apk);
...
// GET Alpha View Factory Lists
	...
	function alpha_view_factory_datatables() {
		...
		$get_total_records =  $wpdb->get_results("SELECT * FROM alpha_wp_ss22_factory_order_confirmation_list WHERE `factoryname` = '$namee'  UNION SELECT * FROM  alpha_wp_pop_factory_order_confirmation_list  WHERE `factoryname` = '$namee'   UNION SELECT * FROM  alpha_wp_factory_order_confirmation_list  WHERE `factoryname` = '$namee'  ", ARRAY_A );
		...
		$getZenlineOrdersList = $wpdb->get_results("SELECT * FROM alpha_wp_ss22_factory_order_confirmation_list WHERE `factoryname` = '$namee'  UNION SELECT * FROM  alpha_wp_pop_factory_order_confirmation_list  WHERE `factoryname` = '$namee'   UNION SELECT * FROM  alpha_wp_factory_order_confirmation_list  WHERE `factoryname` = '$namee' Order by id ".$request['order'][0]['dir']." LIMIT ".$request['length']." OFFSET ".$request['start']."   ", ARRAY_A );
	...
	if($_POST['exportData'] != 'All' ){
			$offset = ($_POST['exportData'] * $_POST['currentPage']) - $_POST['exportData'];
			$dataBody = $wpdb->get_results("SELECT * FROM alpha_wp_ss22_factory_order_confirmation_list WHERE `factoryname` = '$namee'  UNION SELECT * FROM  alpha_wp_pop_factory_order_confirmation_list  WHERE `factoryname` = '$namee'   UNION SELECT * FROM  alpha_wp_factory_order_confirmation_list  WHERE `factoryname` = '$namee' Order by id asc LIMIT ".$_POST['exportData']." OFFSET ".$offset."  ", ARRAY_A );

	}else{
			$dataBody = $wpdb->get_results("SELECT * FROM alpha_wp_ss22_factory_order_confirmation_list WHERE `factoryname` = '$namee'  UNION SELECT * FROM  alpha_wp_pop_factory_order_confirmation_list  WHERE `factoryname` = '$namee'   UNION SELECT * FROM  alpha_wp_factory_order_confirmation_list  WHERE `factoryname` = '$namee' Order by id asc  ", ARRAY_A );
	}
