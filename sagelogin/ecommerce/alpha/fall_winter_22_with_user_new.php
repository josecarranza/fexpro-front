...
<html lang="en">
	...
	<head>
	    ...
	    <title>Fexpro Sage - Factory Order Lists</title>
	    <!-- chartist CSS -->
	    ...
	    <link href="<?php echo SAGE_SITEURL; ?>/alpha/include/css/custom-fexpro.css?v=2.3" rel="stylesheet">
		...
		.bg-MX{
			background-color: #53eb80 !important;
		}
		.bg-GT, .bg-SV, .bg-HN, .bg-NI, .bg-CR{
			background-color:#539aeb !important;
		}
		.bg-PA,
		.bg-PR,
		.bg-MF,
		...
		.bg-UY,
		.bg-PY{
			background-color:#edf855 !important;
		}
		.bg-PH{
			background-color:#a854f7 !important;
		}
...
<script>
	...
	<body class="skin-default fixed-layout" ng-app="app" ng-controller="ctrl">
	    ...
	    <div id="main-wrapper">
	        ...
	        <div class="page-wrapper">
	            ...
	            <div class="container-fluid">
	                ...
	                <div class="row">
                        <div class="col-10">
					   		<a class="btn-dd" href="https://shop2.fexpro.com/wp-admin/admin-ajax.php?action=report_purchase_users&export=1&new_columns=1&group_by={{group_by}}&{{url_params}}" target="_blank">Export All to XLSX</a>

							    <label for="" style="margin-left:30px">Group by: </label>
									<select name="" id="" class="form-control" style="width:200px" ng-model="group_by" ng-change="filtrar()">
						...
						<div class="exporting-it">
							...
							<thead>
									...
									<th  style="vertical-align : middle;text-align:center;" ng-repeat="seller in model.sellers" class="bg-{{seller.country_code}}">{{seller.label}}</th>
								...
								<tbody id="myTable">
									<tr ng-repeat="row in report" class="odd">
										...
										<td ng-repeat="(key, value) in row" ng-if="!['id','image','image2','sending','finished'].includes(key)" class="{{key=='suggestion'? 'cell-bg-'+value :''}}">
								...
								<th style="vertical-align : middle;text-align:center;">Product image</th>
									...
									<th style="vertical-align : middle;text-align:center;min-width: 120px;">Size chart</th>
									<th style="vertical-align : middle;text-align:center;min-width: 100px;">Dimensions</th>
									<th  style="vertical-align : middle;text-align:center;" ng-repeat="seller in model.sellers">{{seller.nombre}}</th>
		...
		<script src="<?php echo SAGE_SITEURL; ?>/alpha/angular.components.js?v=2.3"></script>
		<script>
			angular.module("app",['util-components']).controller("ctrl",function($scope,$http,$timeout){
				...
				$scope.country_colors = {
					"MX":'#0f0',
					"GT":'#00f',"SV":'#00f',"HN":'#00f',"NI" :'#00f',"CR":'#00f',
					"PA":'#f00',"PR":'#f00',"MF":'#f00',"SX":'#f00',
					"CO":'#ff1',"VE":'#ff1',"EC":'#ff1',"PE":'#ff1',"BO":'#ff1',"BR":'#ff1',"CL":'#ff1',"AR":'#ff1',"UY":'#ff1',"PY":'#ff1',
					"PH":'#ccc'
				};
				...
				$scope.get_data=function(){
					...
					$http({
						url:"https://shop2.fexpro.com/wp-admin/admin-ajax.php?action=report_purchase_users&new_columns=1",
						method:"POST",
						data:_params,
						params:{group_by:$scope.group_by}
					}).then(function(response){
						
						if($scope.pag==1){
							$scope.model.sellers=response.data.sellers;
							$scope.report=response.data.data;
							console.log($scope.model.sellers);
						}else{
