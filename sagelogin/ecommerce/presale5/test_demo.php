	...
	foreach($value3 as $key4 => $abc)
	{
		...
		$last = 0;
			...
			foreach ($variation_size as $key => $size) 
			{
				...
				$merge1[$key3][$size['label']] += $size['value'];
				//$merge7[$size['label']][] = $ap * $size['value'];

				$merge45[$key3][$size['label']] = $size['value'];

				$merge50[$key3][$size['label']][] = $abc;
							...
							<table class="table table-bordered" id="demo">
							   <thead>
							        <tr>
							          ...
							          <th style="vertical-align : middle;text-align:center;">Order Item ID</th>
									  <th></th>
							        </tr>
							      </thead>
								<tbody id="myTable">
									...
									foreach($merge as $key => $value)
									{
										...
										$row3 .= '<div class="size-guide"><h5>Sizes</h5>';
											foreach ($merge1[$key] as $akkk => $akkkv) {
												...
												$merge4[$akkk] = $q;
												$OrderItmeId = $merge50[$key][$akkk][0]; 
										...
										echo "<td class='".$_product->get_sku()."'>" .$merge2[$key][0]. "</td>";
										echo "<td class='".$_product->get_sku()."'>" . $OrderItmeId . "</td>";
