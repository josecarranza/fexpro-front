...
$names = 'Dishang Group/Weihai Textile Group Import & Export Co,. Ltd';
$getZenlineOrdersList = $wpdb->get_results("SELECT * FROM alpha_wp_factory_order_confirmation_list WHERE `factoryname` = '$names'   ", ARRAY_A );
...
foreach($getZenlineOrdersList as $key => $value)
{
    ...
    {
        if ( get_post_status ( $bk['order_id'] ) != 'wc-presale5') 
        {
            continue;
        }
        else
